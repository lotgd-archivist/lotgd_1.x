...
if ($op=="save"){
	...
	if (stripslashes(httppost("villagename")) != $settings['villagename']) {
	...
	if (stripslashes(httppost("innname")) != $settings['innname']) {
...
if ($op == "") {
	...
	for ($i=0;$i<=86400;$i+=300){
        ...
        $str = sprintf_translate("In %s at %s",
                reltime($x), date("h:i a", $x));
		$enum.=",$i,$str";
	}
	...
	$setup = array(
		...
		"skinselect"=>"Should the skin selection widget be shown?,bool",
		"curtime"=>"Should the current realm time be shown?,bool",
		"newdaytime"=>"Should the time till newday be shown?,bool",
		"newestplayer"=>"Should the newest player be shown?,bool",
		...
		"autofightfull"=>"Allow fighting until fight is over,bool",
		...
		"tl_maxallowed"=>"Maximum number of players below which automatic untranslated text collection will turn on,int",
