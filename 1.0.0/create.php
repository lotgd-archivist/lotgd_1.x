...
if (getsetting("allowcreation",1)==0){ 
			...
			if (!$blockaccount){
				...
				if (db_num_rows($result)>0){
					...
					$sql = "INSERT INTO " . db_prefix("accounts") . " 
						(name, superuser, title, password, sex, login, laston, uniqueid, lastip, gold, emailaddress, emailvalidation, referer)
						VALUES 
						('$title $shortname', '".getsetting("defaultsuperuser",0)."', '$title', '".md5(md5($pass1))."', '$sex', '$shortname', '".date("Y-m-d H:i:s",strtotime("-1 day"))."', '".$_COOKIE['lgi']."', '".$_SERVER['REMOTE_ADDR']."', ".getsetting("newplayerstartgold",50).", '$email', '$emailverification', '$referer')";
					db_query($sql);
					if (db_affected_rows(LINK)<=0){
						output("`\$Error`^: Your account was not created for an unknown reason, please try again. ");
					}else{
						modulehook("process-create", httpallpost());
						if ($emailverification!=""){
							// We don't have the acctid here nor would that change anything - so we translate into default language
	...
	if ($op==""){
		...
		$refer=httpget('r');
		if ($refer) $refer = "&r=".htmlentities($refer);
		...
		rawoutput("</td><td><input type='password' name='pass1'></td></tr><tr valign='top'><td>");
		output("Re-enter it for confirmation: ");
		rawoutput("</td><td><input type='password' name='pass2'></td></tr><tr valign='top'><td>");
