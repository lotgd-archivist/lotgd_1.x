...
function page_header(){
	global $header,$SCRIPT_NAME,$session,$template, $runheaders;
...
function page_footer(){
	global $output,$nestedtags,$header,$nav,$session,$REMOTE_ADDR,
		...
		$logd_version,$copyright,$SCRIPT_NAME;
	$z = $y2^$z2;
	...
	$headscript = "";
	if (isset($session['user']['lastmotd']) && ($row['motddate']>$session['user']['lastmotd']) && $nopopups[$SCRIPT_NAME]!=1 && $session['user']['loggedin']){
...
<input type="image" src="images/paypal2.gif" border="0" name="submit" alt="Donate!">
	...
	if ($session['user']['superuser'] & SU_EDIT_PETITIONS){
		...
		db_free_result($result);
		$p = translate_inline("`0`bPetitions:`b")." `\${$petitions[5]}`0|`^{$petitions[4]}`0|`b{$petitions[0]}`b|{$petitions[1]}|`!{$petitions[3]}`0|`#{$petitions[7]}`0|`%{$petitions[6]}`0|`i{$petitions[2]}`i";
		...
		while (list($key,$val)=each($session['bufflist'])){
			...
			if ($val['name'] > "" || $session['user']['superuser'] & SU_DEBUG_OUTPUT){
				...
				if ($val['rounds']>=0){
					...
					// `% is handled.
					$n = str_replace("`%","`%%",$val['name']);
					$b = translate_inline("`#$n `7(%s rounds left)`n");
					$b = sprintf($b, $val['rounds']);
					$buffs.=appoencode($b, true);
				}else{
					$buffs.= appoencode(translate_inline("`#{$val['name']}`n"),true);
				}
				tlschema();
				$buffcount++;
			}
