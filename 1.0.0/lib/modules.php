...
$injected_modules = array(true=>array(),false=>array());
	...
	if (file_exists($modulefilename)){
		...
		if (!$force) {
			...
			if (db_num_rows($result)==0) {
				tlschema();
			 	output("`n`3Module `#%s`3 is not installed, but was attempted to be injected.`n",$modulename);
...
function module_status($modulename, $version=false) {
	...
	if (file_exists($modulefilename)) {
		...
		if (db_num_rows($result) > 0) {
			...
			$row = db_fetch_assoc($result);
			/*
			if (!array_key_exists($modulename,$injected_modules[true]))
				$injected_modules[true][$modulename] = false;
			if (!array_key_exists($modulename,$injected_modules[false]))
				$injected_modules[false][$modulename] = false;
			*/
			if ($row['active']) {
				...
				if (array_key_exists($modulename, $injected_modules[false]) &&
						$injected_modules[false][$modulename])
					$status |= MODULE_INJECTED;
				if (array_key_exists($modulename, $injected_modules[true]) &&
						$injected_modules[true][$modulename])
					$status |= MODULE_INJECTED;
			} else {
...
function mass_module_prepare($hooknames){
	...
	$sql = 
		...
		ORDER BY
			...
			$Pmodule_hooks.modulename";
	$result = db_query_cached($sql,"module_prepare");
	$modulenames = array();
...
function modulehook($hookname, $args=false, $allowinactive=false, $only=false){
	...
	if ($session['user']['superuser'] & SU_DEBUG_OUTPUT && !isset($hookcomment[$hookname])){
		rawoutput("<!--Module Hook: $hookname; allow inactive: ".($allowinactive?"true":"false")."; only this module: ".($only?"true":"false"));
		if (!is_array($args)) {
			...
			while (list($key,$val)=each($args)){
				...
				if (is_array($val)){
					...
					$arg.=htmlentities(substr($val,0,25));
				}
				rawoutput("  arg: $arg");
			}
		}
	...
	if (modulename_sanitize($module)!=$module){
		...
		if (injectmodule($module,true)) {
			...
			if (!module_check_requirements($info['requires'])){
				...
				$keys = "|".join(array_keys($info), "|")."|";
				$sql = "INSERT INTO " . db_prefix("modules") . " (modulename,formalname,moduleauthor,active,filename,installdate,installedby,category,infokeys,version,download) VALUES ('$mostrecentmodule','".addslashes($info['name'])."','".addslashes($info['author'])."',0,'{$mostrecentmodule}.php','".date("Y-m-d H:i:s")."','$name','".addslashes($info['category'])."','$keys','".addslashes($info['version'])."','".addslashes($info['download'])."')";
				db_query($sql);
...
function get_module_install_status(){
	...
	if ($handle = opendir("modules")){
		...
		while (false !== ($file = readdir($handle))){
			if ($file[0] == ".") continue;
			if (strpos($file,".php")>0 && !isset($seenmodules[$file])){
				$ucount++;
