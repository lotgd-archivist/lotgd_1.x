...
if ($onlinecount<getsetting("maxonline",0) || getsetting("maxonline",0)==0){
	...
	if ($op=="timeout"){
		...
		if (!isset($_COOKIE['PHPSESSID'])){
			$session['message'].=translate_inline(" Also, it appears that you may be blocking cookies from this site.  At least session cookies must be enabled in order to use this site.`n");
		}
	}
	if ($session['message']>"")
		output_notl("`b`\$%s`b`n", $session['message']);
	...
	output("`\$`bServer full!`b`n`^Please wait until some users logged out.`n`n`0");
	if ($op=="timeout"){
		$session['message'].= translate_inline(" Your session has timed out, you must log in again.`n");
		if (!isset($_COOKIE['PHPSESSID'])){
			$session['message'].=translate_inline(" Also, it appears that you may be blocking cookies from this site. At least session cookies must be enabled in order to use this site.`n");
	...
	if ($session['message']>"") output("`b`\$%s`b`n", $session['message']);
