...
if ($op==""){
	...
	*/
	...
	$saveclose = htmlentities(translate_inline("Save & Close"));
	$savenotclose = htmlentities(translate_inline("Save No Close"));
	rawoutput("<form action='translatortool.php?op=save' method='POST'>");
	rawoutput("$namespace <input name='uri' value=\"".htmlentities(stripslashes($uri))."\"><br/>");
	rawoutput("$texta<br>");
	rawoutput("<textarea name='text' cols='60' rows='5'>".htmlentities($text)."</textarea><br/>");
	rawoutput("$translation<br>");
	rawoutput("<textarea name='trans' cols='60' rows='5'>".htmlentities(stripslashes($trans))."</textarea><br/>");
	...
	while ($row = db_fetch_assoc($result)){
		output_notl("<option value=\"".htmlentities($row['uri'])."\">".htmlentities($row['uri'])." ({$row['c']})</option>",true);
	}
