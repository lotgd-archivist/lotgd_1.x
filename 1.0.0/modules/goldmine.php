		...
		if ($session['user']['turns']<=0) {
			...
			case 19: case 20:
				...
				if ($dead) {
					...
					addnews("%s was completely buried when ".($session['user']['sex']?"she":"he")." became greedy digging in the mines",$session['user']['name']);
				} else {
					if ($horsesave) {
						...
						if ($racemsg) output_notl($racemsg);
						else {
