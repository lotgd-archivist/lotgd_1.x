...
function dragonplace_dohook($hookname,$args){
	...
	case "newday":
		...
		if (($args['resurrection']==="true") && !get_module_setting("mvres"))
			break;
...
function dragonplace_run(){
	...
	case "scry":
		...
		if ($session['user']['gems'] < $cost) {
			...
			if ($vil == get_module_pref("dragonloc") ||
				...
				output("%sFrom high above, you see the villagers of %s walking too and fro, and even some adventurers entering and leaving the gates to go into the forest.`n", "`\$", $vil);
	...
	case "dragon":
		...
		output("Every instinct in your body wants to run, and run quickly, back to the warm inn, and the even warmer %s`\$.", ($session['user']['sex']?"`^Seth":"`5Violet"));
	...
	case "cave":
		...
		output("It may have been a dragons cave once, but now... there are only a few bones, and a flaming pile of logs.`n");
...
function dragonplace_choose() {
	...
	$key = array_rand($vloc);
	set_module_pref('dragonloc',$key);

	if (get_module_setting("dkmin") == 0) {
		set_module_pref('dragonloc2', $key);
	} elseif ($session['user']['dragonkills'] < get_module_setting("dkmin")) {
		while($key == get_module_pref("dragonloc")) {
			$key = array_rand($vloc);
		}
		set_module_pref('dragonloc2', $key);
	} else {
		// Player has too many DK -- make it rare
		set_module_pref('dragonloc2', $key);
	}
}
?>
