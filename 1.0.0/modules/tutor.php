...
function tutor_dohook($hookname,$args){
	...
	case "everyheader":
		...
		if(!$args['script']) break;
		switch($args['script']){
		case "newday":
			if ($age > 1) break;
			if ((!$session['user']['race'] || $session['user']['race']==RACE_UNKNOWN) && httpget("setrace")==""){
				if (is_module_active("racetroll")) $troll="Troll`n";
				if (is_module_active("racedwarf")) $dwarf="Dwarf`n";
				if (is_module_active("racehuman")) $human="Human`n";
				if (is_module_active("raceelf")) $elf="Elf`n";
				if ($troll || $dwarf || $human || $elf) {
					...
					tutor_talk("%s`c`b`#%s%s%s%s`b`c", $tutormsg, $troll, $elf, $human, $dwarf);
				};
			}elseif ($session['user']['specialty']=="" && !httpget("setrace")){
				if (is_module_active("specialtydarkarts")) $da="Dark Arts`n";
				if (is_module_active("specialtymysticpower")) $mp="Mystical Powers`n";
				if (is_module_active("specialtythiefskills")) $ts="Thieving Skills`n";
				if ($da || $mp || $ts){
					...
					tutor_talk("%s`c`b`#%s%s%s`b`c", $tutormsg, $da, $mp, $ts);
				}
			}
			break;
		case "village":
			...
			}elseif ($session['user']['experience'] > 100 && $session['user']['level'] == 1){
...
function tutor_run(){	
	...
	if ($op=="helpfiles") {
		...
		output("It's a good idea to get to know Seth and Violet, because they can be quite helpful to a young warrior.");
		output("You might even decide that sleeping in %s would be safer than in the fields.`n`n",$iname);
		output("Traveling can be dangerous.");
