...
function mercs_runevent($type, $link)
{
	...
	if ($op==""){
		...
		output("`@You decide not to avail yourself of their services, as some of them appear rather unsavoury and possibly even incompetent.`n`n");
	} elseif ($op == "give") {
		$cost = get_module_setting("cost");
		if ($session['user']['gems']>=$cost){
			...
			debuglog("gave $cost gems to the traveling mercenaries");
