...
function modloc_run(){
	...
	foreach ($locations as $name => $sname) {
		addnav($name,"runmodule.php?module=modloc&admin=true&loc=".$name);
		$locations[$name]=0;
	}
	...
	if ($loc==""&&$op=="") {
		...
		$t = translate_inline("Currently showing modules hooked to nonexistant locations.");
	} else {
	...
	if ($error!=0) {
		output("`n`%There are `^%s`% modules hooked to nonexistant locations.",$error);
	}
	page_footer();
}
?>
