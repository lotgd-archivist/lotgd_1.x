...
function ferryman_getmoduleinfo() {
	$info = array(
		...
		"settings"=>array(
			...
			"maxfavorgain"=>"Max amount of favor gained when Ramius greets them,int|15",
...
function ferryman_runevent($type) {
	...
	if ($op=="continueferry" || $op=="payferrymore") {
		...
		case 7:  // end up in underworld, gain some favor
			...
			output("`\$Ramius `2bids you welcome and seems pleased with your arrival.`n`n");
		...
		case 9:  // ferryman hits them
			output ("`2The ferryman starts moving towards you.");
			output("You start to you edge away from him and he swings his ferry pole at you!");
		...
		if ($randdrown < $drownchance) {
			...
			output("The ordeal has left you intact, though little tired.");
...
function ferryman_fight() {
	...
	if ($op == "fightferryman") {
		$badguy = array(
			"creaturename"=>"`7The Ferryman`0",
			"creaturelevel"=>$session['user']['level']+2,
			"creatureweapon"=>"Ferry Pole",
