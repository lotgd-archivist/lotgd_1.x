		...
		if ($op=="" && (!$detail || ($detail == $clanid))) {
			...
			if ($clanid && $session['user']['clanrank'] > CLAN_APPLICANT) {
				...
				if ($treepoints>=$treebonus) {
					...
					clantrees_buff($treereward*($bestree==$clanid?2:1));
					if (get_module_setting("competitive")) {
...
function clantrees_run(){
	...
	if ($op=="buytree") {
		...
		if ($size=="small") {
			...
			output("Though he struggles a bit with the awkwardness of it, be eventually makes it through the entrance and places it in a goodly-sized pot to one side of the entrance.");
