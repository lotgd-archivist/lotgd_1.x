		...
		if (($visits < $max) || ($max==0)){
			...
			if ($session['user']['gold'] < $cost){
				...
				if ($almost==5){
					...
					addnews("%s won %s gold after rolling 5 sixes in the Darkhorse Tavern.",$session['user']['name'],$prize);
					...
					addnews("%s won %s gold after rolling 4 sixes in the Darkhorse Tavern.",$session['user']['name'],$win);
					...
					debuglog("won $win gold at sixes game.");
					addnews("%s won %s gold after rolling 3 sixes in the Darkhorse Tavern.",$session['user']['name'],$win);
