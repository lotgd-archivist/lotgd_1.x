...
function customeq_dohook($hookname,$args){
	...
	case "footer-weapons":
		...
		if (($keep != 2 || $keepeq) && $weapon && $op == "buy"){
			...
			output("`n`n`7As you step out the door you realise, that your new weapon has magically turned into `5%s`7!`0`n", $session['user']['weapon']);
	...
	case "footer-armor":
		...
		if (($keep != 2 || $keepeq) && $armor && $op == "buy"){
			...
			output("`n`n`5As you put on your new armor, it magically changes to `&%s`5!`0`n", $session['user']['armor']);
		}elseif ($armor && $op == "buy"){
			if (is_module_active("costumeshop") &&
...
function customeq_run(){
	...
	if ($op == "buy"){
		...
		if ($newname){
			...
			addnav("C?Confirm","runmodule.php?module=customeq&op=confirm&subop=".$subop."&newname=".$newname);
		}else{
			output("`7You did not choose a valid name for your %s!`0`n", $eq);
		}
		addnav("a?Choose another name","runmodule.php?module=customeq&op=buy&subop=weapon");
	}elseif ($op == "confirm"){
		addnav("L?Return to the Lodge","lodge.php");
		$newname = httpget("newname");
		$eq = translate_inline($subop);
