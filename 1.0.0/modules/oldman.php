...
function oldman_bettinggame($from)
{
	...
	if ($bet<=0){
		...
		output("Embarrased, you think you'll head back into the forest.");
	}else{
...
function oldman_runevent($type)
{
	...
	if ($op=="" || $op=="search"){
		...
		if (e_rand(1,2)==2) {
			...
			output("`@You are quite sure that %s`@ would think you are a wuss for being scared of an old man.", ($session['user']['sex']?"`^Seth" : "`%Violet"));
		...
		case 5:
			...
			case 3:
				...
				output("Your vision goes dark and you feel the hand of `\$Ramius`@ closing around your heart.");
				...
				if ($session['user']['maxhitpoints'] >
					...
					output("`^You `bpermanently`b `\$lose`^ one hitpoint!`n");
				}
		...
		if ($session['user']['gems'] <= 0) {
			...
			case 3:
				...
				output("He turns to you and says, \"`#Since y' made such a fine bargain w' me, me %s, I'll be puttin' in a good word for y' with m' ol' friend `\$Ramius`#.`@\"`n`n", translate_inline($session['user']['sex']?"deary":"lad"));
