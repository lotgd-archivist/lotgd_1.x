...
if ($op == ""){
	...
	while($row = db_fetch_assoc($result)) {
		...
		// output("`^%s`0", $row['ref']);
		// output("</td><td>");
		output("`2%s`0",$row['male']);
		rawoutput("</td><td>");
		output("`6%s`0",$row['female']);
		rawoutput("</td></tr>");
		$i++;
	}
	rawoutput("</table>");
	...
	for ($i=0;$i<db_num_rows($result);$i++){
		...
		$newtitle = get_dk_title((int)$row['dragonkills'], (int)$row['sex']);
		...
		if ($oname != $newname) {
			...
			if ($session['user']['acctid']==$row['acctid']){
				...
				$sql = "UPDATE " . db_prefix("accounts") . " SET name='".addslashes($newname)."', title='".addslashes($newtitle)."' WHERE acctid='{$row['acctid']}'";
				db_query($sql);
			}
			...
			if ($session['user']['acctid']==$row['acctid']){
				...
				$sql = "UPDATE " . db_prefix("accounts") . " SET title='".addslashes($newtitle)."' WHERE acctid='{$row['acctid']}'";
				db_query($sql);
			}
		}
	}
