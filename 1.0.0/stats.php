...
if ($op=="stats" || $op==""){
	$sql = "SELECT sum(gentimecount) AS c, sum(gentime) AS t, sum(gensize) AS s, count(*) AS a FROM " . db_prefix("accounts");
	$result = db_query($sql);
	...
	for ($i=0;$i<db_num_rows($result);$i++){
		...
		for ($j = 0; $j < db_num_rows($res2); $j++) {
			...
			if ($j != db_num_rows($res2)-1) output_notl(",");
		}
		rawoutput("</td></tr>");
	}
	rawoutput("</table>");
}elseif($op=="graph"){
	$sql = "SELECT count(*) AS c, substring(laston,1,10) AS d FROM " . db_prefix("accounts") . " GROUP BY d DESC ORDER BY d DESC";
	$result = db_query($sql);
