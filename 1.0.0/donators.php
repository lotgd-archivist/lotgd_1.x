...
output("Character: ");
rawoutput("<input name='name' value=\"".htmlentities($name)."\">");
output("`nPoints: ");
rawoutput("<input name='amt' size='3' value=\"".htmlentities($amt)."\">");
output("`nReason: ");
rawoutput("<input name='reason' size='30' value=\"".htmlentities($reason)."\">");
rawoutput("<input type='hidden' name='txnid' value=\"".htmlentities($txnid)."\">");
output_notl("`n");
...
if ($op=="add2"){
	...
	$sql = "UPDATE " . db_prefix("accounts") . " SET donation=donation+'$amt' WHERE acctid='$id'";
	db_query($sql);
	modulehook("donation", array("id"=>$id, "amt"=>$amt, "manual"=>($txnid>""?false:true)));
	if ($txnid>""){
		$sql = "UPDATE ".db_prefix("paylog")." SET acctid='$id', processed=1 WHERE txnid='$txnid'";
		db_query($sql);
		debuglog("Received donator points for donating -- Credited manually [$reason]",false,$id,"donation",$amt,false);
		redirect("paylog.php");
	}else{
	...
	systemmail($id,"Donation Points Added","`2You have received $amt donation points for $reason.");
	httpset('op', "");
	$op = "";
}
