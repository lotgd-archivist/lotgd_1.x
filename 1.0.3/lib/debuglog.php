...
function debuglog($message,$target=false,$user=false,$field=false,$value=false,$consolidate=true){
	...
	global $session;
	$args = func_get_args();
	if ($needsdebuglogdelete){  //only test once per page hit.
		$r = e_rand(1,100);
		if ($r==2){
			$timestamp = date(
				"Y-m-d H:i:s",
