...
function showform($layout,$row,$nosave=false,$keypref=false){
	...
	rawoutput("</td></tr><tr><td>&nbsp;</tr></td><tr><td>");
	...
	while(list($key,$val)=each($layout)){
		...
		case "theme":
			...
			foreach($skins as $skin) {
				if ($skin == $row[$key]) {
					rawoutput("<option value='$skin' selected>".htmlentities(substr($skin, 0, strpos($skin, ".htm")))."</option>");
				} else {
					rawoutput("<option value='$skin'>".htmlentities(substr($skin, 0, strpos($skin, ".htm")))."</option>");
				}
			}
			rawoutput("</select>");
			break;
		case "location":
			...
			foreach($vloc as $loc=>$val) {
				if ($loc == $row[$key]) {
					rawoutput("<option value='$loc' selected>".htmlentities($loc)."</option>");
				} else {
					rawoutput("<option value='$loc'>".htmlentities($loc)."</option>");
				}
		...
		case "dayrange":
			...
			if ($cur && $cur < date("Y-m-d H:i:s", $start))
				rawoutput("<option value='$cur' selected>".htmlentities($cur)."</option>");
			for($j = $start; $j < $end; $j = strtotime($step, $j)) {
				$d = date("Y-m-d H:i:s", $j);
				rawoutput("<option value='$d'".($cur==$d?" selected":"").">".HTMLEntities("$d")."</option>");
			}
		...
		case "range":
			...
			for($j = $min; $j <= $max; $j += $step) {
				rawoutput("<option value='$j'".($row[$key]==$j?" selected":"").">".HTMLEntities("$j")."</option>");
			}
			rawoutput("</select>");
			break;
		case "floatrange":
			...
			for($j = $min; $j <= $max; $j = round($j+$step,2)) {
				rawoutput("<option value='$j'".($val==$j?" selected":"").">".HTMLEntities("$j")."</option>", true);
			}
		...
		case "datelength":
			...
			rawoutput("<select name='$keyout'>");
			while(list($k,$v)=each($vals)) {
				rawoutput("<option value=\"".htmlentities($v)."\"".($row[$key]==$v?" selected":"").">".htmlentities($v)."</option>");
			}
			rawoutput("</select>");
			break;
		case "enumpretrans":
		...
		case "enum":
			...
			while (list($k,$v)=each($info)){
				...
				$select.=("<option value='$optval'".($row[$key]==$optval?" selected":"").">".HTMLEntities("$optdis")."</option>");
			}
			$select.="</select>";
			rawoutput($select);
			break;
		case "password":
			...
			rawoutput("<input type='password' name='$keyout' value='".HTMLEntities($out)."'>");
			break;
		case "bool":
			tlschema("showform");
			...
			break;
		case "hidden":
			rawoutput("<input type='hidden' name='$keyout' value=\"".HTMLEntities($row[$key])."\">".HTMLEntities($row[$key]));
			break;
		case "viewonly":
		...
		case "int":
			...
			rawoutput("<input name='$keyout' value=\"".HTMLEntities($out)."\" size='5'>");
			break;
		case "float":
			rawoutput("<input name='$keyout' value=\"".htmlentities($row[$key])."\" size='8'>");
			break;
		case "string":
			$len = 50;
			$minlen = 50;
			...
			rawoutput("<input size='$minlen' maxlength='$len' name='$keyout' value=\"".HTMLEntities($val)."\">");
			break;
		default:
			if (array_key_exists($key, $row)) $val = $row[$key];
			else $val = "";
