...
if (getsetting("allowcreation",1)==0){ 
	...
	if ($op=="create"){
		...
		if (soap($shortname)!=$shortname){
			...
			// This is no longer really testable since we're not going to be
			// sending the passwords across the wire.  We'll test it
			// in the password form.
			if ((int)httppost('passlen')<=3){
			...
			if (!$blockaccount){
				...
				if (db_num_rows($result)>0){
					...
					$sql = "INSERT INTO " . db_prefix("accounts") . " 
						...
						VALUES 
						('$title $shortname', '".getsetting("defaultsuperuser",0)."', '$title', '$dbpass', '$sex', '$shortname', '".date("Y-m-d H:i:s",strtotime("-1 day"))."', '".$_COOKIE['lgi']."', '".$_SERVER['REMOTE_ADDR']."', ".getsetting("newplayerstartgold",50).", '$email', '$emailverification', '$referer')";
					db_query($sql);
