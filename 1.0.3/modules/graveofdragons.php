...
function graveofdragons_runevent($type,$from)
{
	...
	if ($op==""){
		output("`7In the distance in the vast fields of the dead, you see an object like a vast monolith in the distance through the fog.");
		...
		output("Indeed, tis better safe, then sorry, even for the dead.");
		...
		$session['user']['soulpoints']-=(((get_module_setting("lethality"))/100)*$session['user']['soulpoints']);
			if ($session['user']['soulpoints']==0) {
				output("`7You drift back to the shades, destroyed.");
				$session['user']['gravefights']=0;
				$session['user']['specialinc'] = "";
			} else {
				Output("`7Your barely able to drag your battered form back to the surface, haunted by the image of those terrible eyes.");
		...
		case 12:
			...
			debuglog("gained %s gold and %s gems digging in the Dragon's Graveyard.",$randgold,$randgems);
			$session['user']['specialinc'] = "";
			break;
		case 13:
		case 14:
			...
			if ($favor > 0) {
				output("`)You have `\$LOST `^%s`) favor with `\$Ramius`).",
						$favor);
		...
		output("`\$Ramius`7 thanks you for the sacrifice.  You gain `^%s`7 favor.",$favor);
		$session['user']['deathpower']+=$favor;
		...
		output("`7Whilst an impression of great fury fills your head, you realize that it was from `\$Ramius`7's altar which you stole!");
		output("Eventually, you drop the gold, are freed and return to the shades, shaken and not in `\$Ramius`7's best books.");
