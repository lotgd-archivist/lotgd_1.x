...
function mightyblogs_dohook($hookname,$args){
	...
	case "index":
		...
		if (get_module_pref("lastblog") < get_module_setting("lastblog")){
			addnav("$blogkey?`b$title`b","runmodule.php?module=mightyblogs&op=view");
		}else{
			addnav("$blogkey?$title","runmodule.php?module=mightyblogs&op=view");
		}
		break;
	}
	return $args;
...
function mightyblogs_run(){
	...
	while ($row = db_fetch_assoc($result)){
		addnav($row['name']." (".reltime(strtotime($row['date'])).")","runmodule.php?module=mightyblogs&op=view&author=".rawurlencode($row['login']));
	}
...
function mightyblogs_show($blog){
	...
	if ($session['user']['acctid']==$blog['author']){
		output("[ <a href='runmodule.php?module=mightyblogs&op=edit&id={$blog['blogid']}'>Edit</a>",true);
		addnav("","runmodule.php?module=mightyblogs&op=edit&id={$blog['blogid']}");
		output("| <a href='runmodule.php?module=mightyblogs&op=del&id={$blog['blogid']}' onClick=\"return(confirm('Are you sure you want to delete this blog?'));\">Delete</a>",true);
