...
function changename_run(){
	...
	if ($op=="") {
		...
		if (count($history) == 0) {
			...
			foreach ($history as $acctid => $acctinfoserialized) {
				...
				if (db_num_rows($res)) {
					...
					$buttonrevert = htmlentities(translate_inline("Revert"));
		...
		if ($op == "rename") {
			...
			$reasonld = stripslashes(httppost("reasonld"));
			$reasond = httppost("reasond");
			$reasondd = stripslashes(httppost("reasondd"));
			$reasont = httppost("reasont");
			$reasontd = stripslashes(httppost("reasontd"));
			$reasoni = httppost("reasoni");
			$reasonid = stripslashes(httppost("reasonid"));
			$reasonv = httppost("reasonv");
			$reasonvd = stripslashes(httppost("reasonvd"));
			$reasona = httppost("reasona");
			$reasonad = stripslashes(httppost("reasonad"));
			$reasono = httppost("reasono");
			$reasonod = stripslashes(httppost("reasonod"));
		...
		if (($confirm === "true") && ($confirmbox == true)) {
			...
			if (db_num_rows($res)) {
				...
				if ($op == "rename") {
					...
					rawoutput("<input name='reasonld' type='hidden' value='$reasonld'>");
					rawoutput("<input name='reasond' type='hidden' value='$reasond'>");
					rawoutput("<input name='reasondd' type='hidden' value='$reasondd'>");
					rawoutput("<input name='reasont' type='hidden' value='$reasont'>");
					rawoutput("<input name='reasontd' type='hidden' value='$reasontd'>");
					rawoutput("<input name='reasoni' type='hidden' value='$reasoni'>");
					rawoutput("<input name='reasonid' type='hidden' value='$reasonid'>");
					rawoutput("<input name='reasonv' type='hidden' value='$reasonv'>");
					rawoutput("<input name='reasonvd' type='hidden' value='$reasonvd'>");
					rawoutput("<input name='reasona' type='hidden' value='$reasona'>");
					rawoutput("<input name='reasonad' type='hidden' value='$reasonad'>");
					rawoutput("<input name='reasono' type='hidden' value='$reasono'>");
					rawoutput("<input name='reasonod' type='hidden' value='$reasonod'>");
				} else {
					addnav("", "runmodule.php?module=changename&op=revert");
				...
				$buttonconfirm = htmlentities(translate_inline("Confirm!"));
				...
				$buttoncancel = htmlentities(translate_inline("Aagh! No! Cancel! I messed up!"));
...
function changename_displaysetrangeform($lastacctid)
{
	...
	$buttonchangerange = htmlentities(translate_inline("Change Range"));
...
function changename_displayplayer($acctid)
{
	...
	if (db_num_rows($res)) {
		...
		if ($row['dragonkills'] > 0) {
			...
			if ($renamehistory == "") {
				$biourl = "bio.php?char=" . $row['login'] . "&ret=%2Frunmodule.php%3Fmodule%3Dchangename%26op%3Drefresh";
				...
				$buttonrename = htmlentities(translate_inline("Rename"));
...
function changename_displayplayerdetail($acctid,$frombio=false,$focusrefresh=false)
{
	...
	if (db_num_rows($res)) {
		...
		if ($row['name'] == $curname) {
			...
			$biourl = "bio.php?char=" . $row['login'] . "&ret=%2Frunmodule.php%3Fmodule%3Dchangename%26op%3D" . $returnop;
...
{
	$buttoncancel = htmlentities(translate_inline("Cancel"));
	addnav("", "runmodule.php?module=changename&op=cancel");
	rawoutput("<form action='runmodule.php?module=changename&op=cancel' method='post'>");
	rawoutput("<input type='submit' class='button' value='$buttoncancel'>");
	rawoutput("</form>");
