	...
	case "validateprefs":
		...
		if ($args['rentleft']<1) {
			...
			if (!($oldprefs['oldtitle'] || $oldprefs['oldweapon'] ||
						$oldprefs['oldarmor'])) {
				$args['oldtitle']=$row['ctitle'];
				$args['oldweapon']=$row['weapon'];
				$args['oldarmor']=$row['armor'];
			}
			...
			db_query($sql);
			if ($session['user']['acctid']==$userid) {
				$session['user']['ctitle']=$args['newtitle'];
				$session['user']['name']=$newname;
				$session['user']['weapon']=$args['weapon'];
				$session['user']['armor']=$args['armor'];
			}
		}
		break;
	}
