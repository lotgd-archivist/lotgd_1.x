...
function trollbridge_runevent($type,$link){
	...
	case "jump":
		...
		if (e_rand(1,7)>=6) {
			...
			addnews("%s`2's body was found broken by a river!",$session['user']['name']);
	...
	case "trollfight":
		$badguy = array(
			"creaturename"=>"Gigantic Troll",
			"creaturelevel"=>$session['user']['level']+2,
			"creatureweapon"=>"Knife and Fork",
	...
	case "trollfighting":
		...
		if ($victory ||
			...
			$session['user']['experience']*=0.9;
			$session['user']['alive'] = false;
			addnews("`2The remains of %s`2s chewed on bones were found in a river!",$session['user']['name']);
			$session['user']['specialinc'] = "";
			addnav("Return to the News","news.php");
		} else {
