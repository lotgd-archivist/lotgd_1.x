...
function thieves_runevent($type)
{
	...
	if ($op=="" || $op=="search"){
		...
		output("`7You may choose to meet their demands for `%%s`7 gems, stand and fight the thieves, using the skills you learned as a healthy `@%s`7 to try and defend yourself, or try to disappear into the foliage surrounding you (by throwing `%%s`7 %s to distract them).", $costwin, translate_inline($session['user']['race']), $costrun, translate_inline(($costrun > 1)?"gems" : "gem"));
