...
function jewelmonster_getmoduleinfo(){
	$info = array(
		...
		"author"=>"Shannon Brown, based on Chris Vorndran's tatmonster",
		"version"=>"1.0",
...
function jewelmonster_runevent($type,$link){
	...
	case "search":
		...
		output("`3From a scaly neck emerges the Gorgon's head of horror, huge tusks leaning towards you in a menacing manner.`n`n");
		if ($count > 0)
	...
	if ($battle){
		include("battle.php");
		if ($victory){
			output("`n`n`3You have overcome Gorgon!");
			...
			output("The fight earns you `^%s `3experience.`0",$exp);
			...
			$session['user']['badguy'] = "";
			$session['user']['specialinc'] = "";
			output("`n`n`3With one final crushing blow, Gorgon pins you to the ground.");
			...
			output("An evil smile comes across the Gorgon's grotesque face, before it leaves you lying on the ground rather than kill you.");
			$exp = round($session['user']['experience'] *
					get_module_setting("expgain"));
