...
if ($op==""){
	...
	if (db_num_rows($result)==0){
		...
		if ($row['value']>($session['user']['gold']+$tradeinvalue)){
			...
			//output("`b`&Ramius curses your cowardice!`n");
			//output("`4All gold on hand has been lost!`n");
			//output("`415% of experience has been lost!`n");
			//output("`4100 favor lost with Ramius!`n");
			//output("`4You have no more graveyard fights for today!`n");
			//output("You may begin fighting again tomorrow.");
			//addnav("Daily news","news.php");
			//addnews("`%%s`5 has been slain by `!MightyE`5 for trying to steal from `#Pegasus`5' Armor Wagon.",$session['user']['name']);
			...
			output("You begin to protest, \"`@Won't I look silly wearing nothing but a `&%s`@?`5\" you ask.", $row['armorname']);
