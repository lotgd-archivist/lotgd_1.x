...
if ($op == "list") {
	...
	while ($row = db_fetch_assoc($result)){
		rawoutput("<option value=\"".htmlentities($row['namespace'])."\"".((htmlentities($row['namespace']) == $namespace) ? "selected" : "").">".htmlentities($row['namespace'])." ({$row['c']})</option>");
	}
	rawoutput("</select>");
	...
	if ($mode == "edit") {
		...
		rawoutput("<textarea name='intext' cols='60' rows='5'>".htmlentities(stripslashes(httpget('intext')))."</textarea><br/>");
