	...
	if ($op==""){
		...
		while ($row=db_fetch_assoc($result)){
			...
			if ($session['user']['clanrank']>CLAN_MEMBER){
				...
				if ($row['clanrank']<$session['user']['clanrank']){
					rawoutput("[ <a href='clan.php?op=membership&setrank=".($row['clanrank']+1)."&who=".rawurlencode($row['login'])."&whoname=".rawurlencode($row['name'])."&whoacctid=".$row['acctid']."'>$promote</a> | ");
					addnav("","clan.php?op=membership&setrank=".($row['clanrank']+1)."&who=".rawurlencode($row['login'])."&whoname=".rawurlencode($row['name'])."&whoacctid=".$row['acctid']);
				}else{
					output_notl("[ %s | ", $promote);
				}
				if ($row['clanrank']<=$session['user']['clanrank'] && $row['clanrank']>CLAN_APPLICANT && $row['login']!=$session['user']['login']){
					rawoutput("<a href='clan.php?op=membership&setrank=".($row['clanrank']-1)."&who=".rawurlencode($row['login'])."&whoname=".rawurlencode($row['name'])."&whoacctid=".$row['acctid']."'>$demote</a> | ");
					addnav("","clan.php?op=membership&setrank=".($row['clanrank']-1)."&who=".rawurlencode($row['login'])."&whoname=".rawurlencode($row['name'])."&whoacctid=".$row['acctid']);
				}else{
		...
		if ($session['user']['clanrank']>=CLAN_LEADER){
			...
			if ($row['c']==0){
				...
				$sql = "SELECT name,acctid,clanrank FROM " . db_prefix("accounts") . " WHERE clanid={$session['user']['clanid']} AND clanrank > " . CLAN_APPLICANT . " AND acctid<>{$session['user']['acctid']} ORDER BY clanrank DESC, clanjoindate";
				$result = db_query($sql);
				if (db_num_rows($result)>0){
