...
function mountupgrade_showform($keyout, $val, $info){
	...
	while ($row = db_fetch_assoc($result)){
		...
		if ($val==$mountid){
			rawoutput("<option value='$mountid' selected>".htmlentities("$mountcategory: $mountname")."</option>");
		}else{
			rawoutput("<option value='$mountid'>".htmlentities("$mountcategory: $mountname")."</option>");
		}
	}
	rawoutput("</select>");
}
