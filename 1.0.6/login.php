...
if ($name!=""){
	if ($session['loggedin']){
		...
		$sql = "SELECT * FROM " . db_prefix("accounts") . " WHERE login = '$name' AND password='".addslashes($password)."' AND locked=0";
		$result = db_query($sql);
			...
			if (db_num_rows($result)>0){
				...
				while ($row=db_fetch_assoc($result)){
					...
					if ($c>=10){
						...
						$sql = "INSERT INTO " . db_prefix("bans") . " VALUES ('{$_SERVER['REMOTE_ADDR']}','','".date("Y-m-d H:i:s",strtotime("+".($c*3)." hours"))."','Automatic System Ban: Too many failed login attempts.','System','0000-00-00 00:00:00')";
						db_query($sql);
						if ($su){
