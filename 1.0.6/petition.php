...
if ($op=="primer"){
	...
	if (count($post)>0){
		...
		if ($abuse == "yes") {
			...
			rawoutput("<input type='hidden' name='abuse' value=\"".stripslashes(htmlentities(httpget("problem")))."\"><br><hr><pre>".stripslashes(htmlentities(httpget("problem")))."</pre><hr><br>");
		} else {
			rawoutput("<textarea name='description' cols='55' rows='7' class='input'>".stripslashes(htmlentities(httpget("problem")))."</textarea>");
		}
