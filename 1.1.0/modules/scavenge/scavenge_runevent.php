...
case "graveyard":
	if ($op=="" || $op=="search") {
		...
		case 4:
			...
			output("You feel like %s`7 is pleased with this act!`n`n",$deathoverlord);
			$session['user']['gold']+=$fnord;
			$session['user']['deathpower']+=10;
			debuglog("gained $gold scavenging");
			break;
		case 5:
		...
		case 15:
			...
			output("You could free it, but %s`7 would not be pleased if he heard that you had helped a soul escape his realm.",$deathoverlord);
			addnav("Lost Soul");
		...
		output("%s`\$ is angry at your temerity!`n`n",$deathoverlord);
		...
		addnews("%s`7 saved an innocent soul in the Graveyard from the clutches of %s`7.",$session['user']['name'],$deathoverlord);
	} elseif ($op=="running") {
		$session['user']['specialinc'] = "";
		...
		output("`7Thinking that the dead are better left in peace, you hurry away to seek favour with %s`7 elsewhere.`0",$deathoverlord);
		$session['user']['specialinc'] = "";
	}
	break;
case "village":
