...
function abigail_runevent($type, $link) {
	...
	}elseif($op == "shout")
	{
		...
		if ($session['user']['marriedto'] != INT_MAX && $session['user']['marriedto'] != 0) {
			...
			$subject = "`%Abigail liefert dir ein Geschenk!";
			$body = $session['user']['name']." `^hat dir ein(e) $trinket`^ als Geschenk schicken lassen.";
			$flirtingwith = get_module_pref("flirtingwith", "flirting");
			systemmail($flirtingwith, $subject, $body, 0);
		}
