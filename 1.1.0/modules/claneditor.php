...
function claneditor_run(){
	...
	if ($op==""||$op=="list"){
		...
		$result = db_query($sql);
		if (db_num_rows($result)>0){
			output("%s`7 steps in as if by magic, listing the clans for you.`n",$registrar);
			...
			for ($i=0;$i<db_num_rows($result);$i++){
				...
				if ($row['c']==0){
					...
					output_notl("&#149; <a href='runmodule.php?module=claneditor&op=mview&dt=%s'>%s</a></td><td>%s`n",
							$row['clanid'],
							htmlentities(full_sanitize($row['clanname']), ENT_COMPAT, getsetting("charset", "ISO-8859-1")),	$memb, true);
					rawoutput('</td></tr>');
		...
		if ($apply==1){
			...
			$result = db_query($sql);
			$e = array(translate_inline("{registrar}`7 hands you a form to start a clan."),
					translate_inline("{registrar}`7 looks over your form but informs you that your clan name must consist only of letters, spaces, apostrophes, or dashes.  Also, your short name can consist only of letters. She hands you a blank form."),
					translate_inline("{registrar}`7 looks over your form but informs you that you must have at least 5 and no more than 50 characters in your clan's name (and they must consist only of letters, spaces, apostrophes, or dashes), then hands you a blank form."),
					translate_inline("{registrar}`7 looks over your form but informs you that you must have at least 2 and no more than 5 characters in your clan's short name (and they must all be letters), then hands you a blank form."),
					translate_inline("{registrar}`7 looks over your form but informs you that the clan name %s is already taken, and hands you a blank form."),
					translate_inline("{registrar}`7 looks over your form but informs you that the short name %s is already taken, and hands you a blank form."),
			     );
			for ($i=0;$i<6;$i++) {
				$e[i]=str_replace("{registrar}",$registrar,$e[i]);
			}
			if ($ocs==""&&$ocn==""&&!httppostisset('clanname')&&!httppostisset('clanshort')) {
				...
				output_notl($e[4],stripslashes($clanname));
				clanform();
			}else{
				$sql = "SELECT * FROM " . db_prefix("clans") . " WHERE clanshort='$clanshort'";
				$result = db_query($sql);
				if (db_num_rows($result)>0){
					output_notl($e[5],stripslashes($clanshort));
					clanform();
				}else{
			...
			output("`7You approach %s`7 and mention that you would like to start a new clan.",$registrar);
