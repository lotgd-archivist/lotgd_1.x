	...
	case "newday":
		...
		if ($guilt > 1) {
			if (get_module_pref("guiltydk")) {
				...
				output("You are quite sure that, whatever it was,  you don't want to face %s`3 quite yet.",$barkeep);
			} else {
				output("`n`3You start to feel bad about breaking into the inn recently, and you aren't sure you can face %s`3 quite yet.",$barkeep);
			}
			if ($guilt == 2) {
...
function breakin_run(){
	...
	if ($op==""){
		...
		if ($bchance>$danger) {
			...
			output("%s`7 stands over you, a wine bottle in one hand, scowling angrily.`n`n",$barkeep);
			...
			addnews("`&%s `7tried to break into the Inn, and was caught by %s`7.", $session['user']['name'],$barkeep);
