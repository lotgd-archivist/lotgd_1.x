	...
	case "newday":
		...
		if ($blessed) {
			...
			if ($ramiusarmor) {
				...
				$ramiusbuff = array("name"=>array("%s``\$' Blessed Weapon",$deathoverlord),
									"rounds"=>-1,
									"atkmod"=>1.25,
									"lifetap"=>$multiplier,
				...
				debug("Error: $deathoverlord bless flagged, but not armor or weapon (gravebless)");
			}
		} elseif ($cursed) {
			if ($ramiusarmor) {
				$ramiusbuff = array("name"=>array("%s`4' Cursed Armor",$deathoverlord),
									"rounds"=>-1,
									"defmod"=>0.8,
									"badguyatkmod"=>1.25,
				...
				$ramiusbuff = array("name"=>array("Ramius`4' Cursed Weapon",$deathoverlord),
									"rounds"=>-1,
									"atkmod"=>0.8,
									"badguydefmod"=>1.25,
...
function gravebless_run() {
	global $session;
	page_header("Seek a Blessing from %s",$deathoverlord);
	...
	if ($blessop == "bless") {
		output("%s`) speaks, \"`7You are a most persistent mortal.",$deathoverlord);
		...
		output("%s`) looks over the battered armor you hand him and remarks, \"`7It is no small wonder you visit so often.\"",$deathoverlord);
		...
		if ($randcurse > $cursechance) {
			...
			debuglog("spent $blesscost favor on armor blessing from $deathoverlord");
			...
			output("`)You put on the armor and notice that it feels slightly warmer to the touch than you remember.");
			debuglog("spent $blesscost favor on armor curse from $deathoverlord");
			$session['user']['deathpower'] -= $blesscost;
		...
		output("%s`) looks over the puny weapon you hand him and comments, \"`7I am impressed that you make any progress at all.\"",$deathoverlord);
		...
		if ($randcurse > $cursechance) {
			...
			debuglog("spent $blesscost favor on weapon blessing from $deathoverlord");
			$session['user']['deathpower'] -= $blesscost;
			set_module_pref("ramiusblessed", 1);
			...
			output("\"`7Perhaps this will make you stronger when you next awaken.`)\"`n`n");
			output("`)You grasp the weapon and notice that it feels slightly warmer to the touch than you remember.");
			debuglog("spent $blesscost favor on weapon curse from $deathoverlord");
			$session['user']['deathpower'] -= $blesscost;
			set_module_pref("ramiuscursed", 1);
			set_module_pref("ramiusweapon", 1);
		}
		...
		output("As you turn to leave, %s`) speaks, ",$deathoverlord);
