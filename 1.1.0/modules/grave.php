...
function grave_runevent($type,$from)
{
	...
	if ($op==""){
		...
		output("`2As time passes you get the feeling that %s`2 is pleased with your gesture.`n",$deathoverlord);
		output("`)You gain %s favor with `4%s`).`0`n",$favorgain,$deathoverlord);
		$session['user']['turns']--;
