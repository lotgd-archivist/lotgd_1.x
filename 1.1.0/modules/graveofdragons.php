...
function graveofdragons_runevent($type,$from)
{
	...
	if ($op==""){
		...
		case 14:
			...
			if ($favor > 0) {
				output("`)You have `\$LOST `^%s`) favor with %s`).",
						$favor,$deathoverlord);
		...
		output("%s`7 thanks you for the sacrifice.  You gain `^%s`7 favor.",$deathoverlord,$favor);
		$session['user']['deathpower']+=$favor;
		...
		output("`7Whilst an impression of great fury fills your head, you realize that it was from %s`7's altar which you stole!",$deathoverlord);
		output("Eventually, you drop the gold, are freed and return to the shades, shaken and not in %s`7's best books.",$deathoverlord);
		debuglog("Lost all favour from steeling from the cursed altar of $deathoverlord");
