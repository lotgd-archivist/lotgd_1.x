...
function module_check_requirements($reqs, $forceinject=false){
	...
	while (list($key,$val)=each($reqs)){
		$info = explode("|",$val);
		if ($key=='lotgd') {
			$check=substr($logd_version,0,10);
			$checked=$info[0];
			$check=preg_replace("/[abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ-]/", "", $check);
			$checked=preg_replace("/[abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ-]/", "", $checked);
			$check=explode(".",$check);
			$checked=explode(".",$checked);
			$counter=count($checked);
			for ($i=0;$i<$counter;$i++) {
				if ((int)$check[$i]<(int)$checked[$i]) return false;
			}
			return true;
		}
...
function module_objpref_edit($type, $module, $id)
{
	...
	if (count($info['prefs-'.$type]) > 0) {
		...
		$sql = "SELECT value FROM " . db_prefix("module_objprefs") . " WHERE modulename='$module' AND objtype='$type' AND objid='$id'";
		$result = db_query($sql);
		while($row = db_fetch_assoc($result)) {
