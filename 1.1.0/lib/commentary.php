...
function talkform($section,$talkline,$limit=10,$schema=false){
	...
	function previewtext(t,l){
		...
		if (t.substr(0,2)=='::'){
			...
			x=3;
			out += '</span><span class=\'colLtWhite\'>';
		".(($session['user']['superuser']&SU_IS_GAMEMASTER)==SU_IS_GAMEMASTER?"}else if (t.substr(0,5)=='/game'){
			x=5;
			out = '<span class=\'colLtWhite\'>';":"")."
		}else{
			out += '</span><span class=\'colDkCyan\'>".addslashes(appoencode(translate_inline($talkline,$schema))).", \"</span><span class=\'colLtCyan\'>';
		...
		charsleft += '".translate_inline("You have ")."'+(l-t.length)+'".translate_inline(" characters left.")."<br>';
		if (l-t.length<0) charsleft +='</span>';
		document.getElementById('charsleft').innerHTML=charsleft+'<br/>';
		for (; x < t.length; x++){
			y = t.substr(x,1);
			if (y=='<'){
				out += '&lt;';
	...
	if (!strpos($req,"?")) $req = str_replace("&","?",$req);
	addnav("",$req);
	rawoutput("<span id='charsleft'></span>");
	output_notl("<form action=\"$req\" method='POST' autocomplete='false'>",true);
	output_notl("<input name='insertcommentary' id='commentary' onKeyUp='previewtext(document.getElementById(\"commentary\").value,".(200-$tll).");' size='40' maxlength='".(200-$tll)."'>",true);
