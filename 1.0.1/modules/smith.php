...
function smith_runevent($type, $link)
{
	...
	if ($op=="" || $op=="search"){
		...
		if ($previously_upgraded){
			...
			output("\"`6No, I'm afraid there is nothing I can do to improve this.  Fare thee well friend!`7\" he says as he disapears into the underbrush.");
			...
			output("`^Your %s has been upgraded!", $op);
			...
			if ($r <= $ichance){
				...
				output("He examines it for a moment, then pulls an anvil and forge from behind his back, and sets to work.");
				output("A short while later, he mutters, \"`6Oops.`7\" and hands your %s back to you.", $op);
				output("Your %s was downgraded!", $op);
				$session['user'][$op] = $session['user'][$op]." -1";
				$session['user'][$op.($op=="weapon"?"dmg":"def")]-=1;
				$session['user'][($op=="weapon"?"attack":"defense")]--;
				...
				// nothing (default 30%).
				output("\"`6Not much I can do here I'm afraid, friend.`7\" and hands it back to you.");
			}
		}
	}else{
