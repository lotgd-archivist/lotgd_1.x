...
function marquee_run() {
	...
	if($op==""){
		...
		output("%s is standing at the servery, handing out %s`7.`n`n", $talk ,$tucker);
		output("On a board near the door is a list of the dishes available, for %s gold each.",$price);
		if($eatentoday==1){
			output("`n`nYou really aren't hungry!");
			output("Maybe you'll come back tomorrow.");
		}elseif($session['user']['gold']<$price && $voucher){
			output("`n`nYou realize that most of the dishes here cost %s gold, but you have a voucher for free pizza.",$price);
			addnav("R?Redeem Pizza Voucher","runmodule.php?module=marquee&op=pizza");
		}elseif($session['user']['gold']<$price){
			output("`n`nYou then realize that dishes here cost %s gold.",$price);
