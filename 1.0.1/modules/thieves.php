...
function thieves_elf_help($from) {
	...
	$gems = $session['user']['gems'];
	$op = httpget("op");
	$salut = ($session['user']['sex'] ? "noble Lady":"dear Sire");
	...
	if ($gems == 0) {
		...
		rawoutput("</a><br />");
		if (is_module_active("specialtythief") &&
...
function thieves_fight($costlose) {
	...
	if ($op=="stand"){
		...
		$badguy = array(
			"creaturename"=>"`\$Lonestrider's Thieves`0",
			"creaturelevel"=>$session['user']['level']+1,
			"creatureweapon"=>"Many Stabbing Knives",
	...
	if ($battle){
		...
		if ($victory){
			...
			output("There, he purchases a room from Cedrik, and leaves coin for your care, departing before you fully gain consciousness, leaving no opportunity to thank him.`n`n");
...
function thieves_runevent($type)
{
	...
	if ($op=="" || $op=="search"){
		...
		output("`7You may choose to meet their demands for `%%s`7 gems, stand and fight the thieves, using the skills you learned as a healthy `@%s`7 to try and defend yourself, or try to disappear into the foliage surrounding you (by throwing `%%s`7 %s to distract them).", $costwin, $session['user']['race'], $costrun, translate_inline(($costrun > 1)?"gems" : "gem"));
