		...
		if (get_module_pref("homecity")!=$session['user']['location']){
			$info = modulehook("cities-usetravel",
				array(
					...
					"traveltext"=>array("`n`n`%Surprised at finding your master in %s`%, you feel a little less inclined to be galavanting around the countryside today.", $session['user']['location']),
					)
				);
			if ($info['success']){
...
function cities_run(){
	...
	if ($session['user']['specialinc'] != "" || httpget("eventhandler")){
		$in_event = handle_event("travel",
			"runmodule.php?module=cities&city=$city&d=$danger&continue=1&",
			"Travel");
		if ($in_event) {
			addnav("Continue","runmodule.php?module=cities&op=travel&city=$city&d=$danger&continue=1");
	...
	if ($op=="travel"){
		...
		if ($city==""){
			...
			output("`%Traveling the world can be a dangerous occupation.");
			...
			if ($continue!="1" && $su!="1"){
				if ($free > 0) {
					...
					debuglog("Traveled with out having any forest fights, how'd they swing that?");
				}
			}
			...
			if (e_rand(0,100)< $dlevel && $su!='1'){
				...
				if (module_events("travel", get_module_setting("travelspecialchance"),"runmodule.php?module=cities&city=$city&d=$danger&continue=1&") != 0) {
		...
		addnav("Continue your journey","runmodule.php?module=cities&op=travel&city=$city&continue=1&d=$danger");
		module_display_events("travel",
			"runmodule.php?module=cities&city=$city&d=$danger&continue=1");
		page_footer();
	}
	...
	if ($battle){
		...
		if ($victory){
			require_once("lib/forestoutcomes.php");
			forestvictory($badguy,"This fight would have yielded an extra turn except it was during travel.");
			addnav("Continue your journey","runmodule.php?module=cities&op=travel&city=$city&continue=1&d=$danger");
			module_display_events("travel",
				"runmodule.php?module=cities&city=$city&d=$danger&continue=1");
		}elseif ($defeat){
			...
			fightnav(true,true,"runmodule.php?module=cities&city=$city&d=$danger");
		}
		page_footer();
	}
...
function cities_faq() {
	...
	output("`^1. Why, oh why did you activate such a (choose one [wonderous, horrible]) feature?`n");
	output("`@For kicks, of course. We like to mess with your head.`n");
	output("But seriously, have you looked at the user list?  On lotgd.net, we've got over 4,000 people cramming themselves into the Village Square and trying to get their voices heard! Too much! Too much!`n");
