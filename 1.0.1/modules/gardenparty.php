...
function gardenparty_getmoduleinfo(){
	$info = array(
		...
		"settings"=>array(
			...
			"cedrikclothes"=>"What is Cedrik wearing?|a Hawaiian shirt",
...
function gardenparty_dohook($hookname, $args) {
	...
	case "gardens":
		...
		if ($start <= $now && $end >= $now) {
			...
			output("Cedrik is here, wearing (of all things) %s, serving food and drinks.`n`n", get_module_setting("cedrikclothes"));
			addnav("Party Treats!");
...
function gardenparty_run(){
	...
	if ($cantafford){
		page_header("Cedrik in %s", get_module_setting("cedrikclothes"));
		output("You wander over to where Cedrik is standing in the gardens, and ask to buy %s, but he tells you that you don't have enough gold to buy it.", $missed);
