...
function costumeshop_getmoduleinfo(){
	$info = array(
		...
		"prefs"=>array(
			...
			"oldtitle"=>"Original custom title before renting a costume|",
			"oldweapon"=>"Current actual weapon|",
			"oldarmor"=>"Current actual armor|",
		)
	);
	return $info;
}
...
function costumeshop_run(){
	...
	if ($rleft > 0){
		...
		if (get_module_setting("christmastxt")){
			...
			output("`7\"`)Greetings traveler! I am `\$Count Nesretep`), and welcome to my shop!`7\"`n`n");
		}
