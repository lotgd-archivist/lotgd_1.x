	...
	case graveyard:
		if ($op=="" || $op=="search") {
			...
			case 4:
				...
				output("You feel like Ramius is pleased with this act!`n`n");
			...
			case 15:
				...
				output("You could free it, but Ramius would not be pleased if he heard that you had helped a soul escape his realm.");
				addnav("Lost Soul");
			...
			output("`\$Ramius is angry at your temerity!`n`n");
			$session['user']['deathpower'] = $session['user']['deathpower']*0.5;
			$terror_wraith=get_module_pref("terror_wraith");
			set_module_pref("seen_wraith","-1");
			if ($terror_wraith>0) {
				set_module_pref("terror_wraith",get_module_pref("terror_wraith")-1);
			...
			addnews("%s saved an innocent soul in the Graveyard from the clutches of Ramius.",$session['user']['name']);
		} elseif ($op=="running") {
			$session['user']['specialinc'] = "";
			...
			output("`7Thinking that the dead are better left in peace, you hurry away to seek favour with Ramius elsewhere.`0");
			$session['user']['specialinc'] = "";
		}
		break;
	case village:
