...
function sethsong_dohook($hookname,$args){
	...
	case "inn":
		...
		if ($op == "" || $op == "strolldown" || $op == "fleedragon") {
			addnav("Things to do");
			addnav("L?Listen to Seth the Bard","runmodule.php?module=sethsong");
		}
		break;
	case "newday":
...
function sethsong_run(){
	...
	if ($been >= $visits) {
		output("Seth clears his throat and drinks some water.");
...
function sethsong_sing()
{
	...
	case 0:
		...
		break;
	case 1:
		output("Mireraband, I scoff at thee and tickeleth your toes.`n");
	...
	case 2:
		...
		output("You merely back away, and think you'll visit Seth when he's feeling better.");
	...
	case 3:
		...
		output("Though you don't really gain anything from the tale from Seth, you do happen to notice a few gold on the ground!");
	...
	case 6:
		...
		output("`0You think Seth is quite correct.`n");
	...
	case 7:
		...
		break;
	case 8:
		output("`0\"`^What is the sound of one hand clapping?`0\" asks Seth.");
		if ($session['user']['gold'] >=$gold ) {
			output("While you ponder this conundrum, Seth \"liberates\" a small entertainment fee from your purse.`n`n");
			output("You lose %s gold!",$gold);
			$session['user']['gold']-=$gold;
			debuglog("lost $gold gold to Seth");
		} else {
			output("While you ponder this conundrum, Seth attempts to \"liberate\" a small entertainment fee from your purse, but doesn't find enough to bother with.");
		}
		break;
	case 9:
		...
		output("A fsshh.`n`n");
		output("You groan as Seth laughs heartily.");
		if($gems==0){
		...
		break;
	case 10:
		output("Seth plays a soft but haunting melody.`n`n");
		output("You feel relaxed, and your wounds seem to fade away.");
		if ($session['user']['hitpoints'] < $session['user']['maxhitpoints'])
			$session['user']['hitpoints'] = $session['user']['maxhitpoints'];
		break;
	case 11:
		output("Seth plays a melancholy dirge for you.`n`n");
	...
	case 12:
		...
		output("`0Seth continues to sing, but not wishing to learn how high he can count, you quietly leave.`n`n");
	...
	case 13:
		...
		if ($session['user']['sex']==SEX_FEMALE){
			output("`0Seth is cut short by a curt slap across his face!");
			output("Feeling rowdy, you gain a forest fight.");
		}else{
			output("`0Seth is cut short as you burst out in laughter, not even having to hear the end of the rhyme.");
			output("Feeling inspired, you gain a forest fight.");
		}
		$session['user']['turns']++;
		break;
	case 14:
		output("Seth plays a rousing call-to-battle that wakes the warrior spirit inside of you.`n`n");
		output("`0You gain a forest fight!");
		$session['user']['turns']++;
		break;
	case 15:
		output("Seth seems preoccupied with your... eyes.`n`n");
		if ($session['user']['sex']==SEX_FEMALE){
		...
		break;
	case 16:
		output("Seth begins to play, but a lute string snaps, striking you square in the eye.`n`n");
		...
		break;
	case 17:
		output("Seth begins to play, but a rowdy patron stumbles past, spilling beer on you.");
		output("You miss the performance as you wipe the swill from your %s.", $session['user']['armor']);
		break;
	case 18:
		output("`0Seth stares at you thoughtfully, obviously rapidly composing an epic poem...`n`n");
		output("`^U-G-L-Y, You ain't got no alibi -- you ugly, yeah yeah, you ugly!`n`n");
		$session['user']['charm']--;
		if ($session['user']['charm']<0){
			output("`0If you had any charm, you'd have been offended, instead, Seth breaks a lute string.");
		}else{
