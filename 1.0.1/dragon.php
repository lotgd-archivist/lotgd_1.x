...
if ($op==""){
	...
	$badguy = array("creaturename"=>"`@The Green Dragon`0","creaturelevel"=>18,"creatureweapon"=>"Great Flaming Maw","creatureattack"=>45,"creaturedefense"=>25,"creaturehealth"=>300, "diddamage"=>0, 'type'=>'dragon');
	...
	for ($i=0;$i<db_num_rows($result);$i++){
		...
		if (isset($row['Field'],$nochange) && $nochange[$row['Field']]){
		}else{
	...
	output("`n`n`&Because you have slain the dragon %s times, you start with some extras.  You also keep additional hitpoints you've earned or purchased.`n",$session['user']['dragonkills']);
...
if ($battle){
	...
	if ($victory){
		...
		addnews("`&%s has slain the hideous creature known as `@The Green Dragon`&.  Across all the lands, people rejoice!",$session['user']['name']);
		tlschema("nav");
