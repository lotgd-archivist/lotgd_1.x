...
if ($op=="del"){
	...
	db_query($sql);
	invalidatecache("petition_counts");
	$op="";
}
...
if ($op==""){
	...
	$petitionsperpage = 50;
	$sql = "SELECT count(*) AS c from ".db_prefix("petitions");
	$result = db_query($sql);
	$row = db_fetch_assoc($result);
	...
	if ($totalpages != 1)  {
		...
		for ($x=1; $x <= $totalpages; $x++){
			if ($page == $x){
				addnav("`b`#Page $x`0`b","viewpetition.php?page=$x");
			}else{
				addnav("Page $x","viewpetition.php?page=$x");
			}
		}
	}
	if ($page > 1){
	...
	$last = translate_inline("Last Update");
	$view = translate_inline("View");
	$close = translate_inline("Close");
	$mark = translate_inline("Mark");

	rawoutput("<table border='0'><tr class='trhead'><td>$num</td><td>$ops</td><td>$from</td><td>$sent</td><td>$com</td><td>$last</td></tr>");
	$i=0;
	$laststatus=-1;
	...
	output("`@Body:`^`n");
	$body = htmlentities(stripslashes($row['body']));
	...
	output("`n`@Commentary:`0`n");
	viewcommentary("pet-$id","Add information",200);
	if ($viewpageinfo){
