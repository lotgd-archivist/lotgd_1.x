...
function saveuser(){
	...
	if ($session['loggedin'] && $session['user']['acctid']!=""){
		...
		$doesntcountforupdate = array(
			"gold"=>true,
			"goldinbank"=>true,
			// On any page where allowednavs changes normally, the output
			// and other things such as gentime, and gentimecount will 
			// change as well.  However, on motd page, due to a fix to 
			// handle village refresh, I needed to add a c=??? to the
			// refresh nav.  Now those navs change in the motd.
			"allowednavs"=>true,
		);
		while(list($key,$val)=each($session['user'])){
			...
			if ($baseaccount[$key]!=$val){
				...
				if (!isset($doesntcountforupdate[$key])) {
					$changes++;
				}
				$updates .= "$key, ";
			}
