...
$userinfo = array(
	...
	"clanrank"=>"Clan Rank,enum,".CLAN_APPLICANT.",Applicant,".CLAN_MEMBER.",Member,".CLAN_OFFICER.",Officer,".CLAN_LEADER.",Leader",
...
#		$sql = "UPDATE " . db_prefix("accounts") . " SET lasthit='$newdate' WHERE acctid='$userid'";
		...
		$sql = "UPDATE " . db_prefix("accounts") . " SET allowednavs='',output=\"\" WHERE acctid='$userid'";
		db_query($sql);
	...
	while (list($key,$val)=each($post)){
		if (isset($userinfo[$key])){
			if ($key=="newpassword" ){
				...
				$updates++;
				$tmp = sanitize_colorname(true, stripslashes($val));
				$tmp = preg_replace("/[`][cHw]/", "", $tmp);
				$tmp = sanitize_html($tmp);
				if ($tmp != stripslashes($val)) {
...
if ($op=="edit"){
	...
	if (httpget("subop")==""){
		...
		rawoutput("<input type='hidden' value=\"".htmlentities(serialize($info))."\" name='oldvalues'>");
		rawoutput("</form>");
	...
	output("IP: ");
	rawoutput("<input name='ip' id='ip' value=\"".HTMLEntities($row['lastip'])."\">");
	output_notl("`n");
	rawoutput("<input type='radio' value='id' name='type'>");
	output("ID: ");
	rawoutput("<input name='id' value=\"".HTMLEntities($row['uniqueid'])."\">");
	output("`nDuration: ");
	rawoutput("<input name='duration' id='duration' size='3' value='14'>");
	rawoutput("Days (0 for permanent)`n");
	...
	if ($row['name']!=""){
		...
		for ($x=strlen($ip); $x>0; $x--){
			...
			if (db_num_rows($result)>0){
				...
				rawoutput("</a>");
				if ($previpmatches)
					output(" (this filter would also catch the above names)");
				output_notl("`n");
				$previpmatches = true;
				while ($row=db_fetch_assoc($result)){
					output("&nbsp;&nbsp;",true);
	...
	for ($i=0;$i<db_num_rows($result);$i++){
		...
		$expire= sprintf_translate("%s days",
				round((strtotime($row['banexpire'])-strtotime("now"))/86400,0));
	...
	addnav("Debug Log");
	if ($next < $max) {
		addnav("Next page","user.php?op=debuglog&userid=$userid&start=$next");
	}
	if ($start > 0) {
		addnav("Previous page",
				"user.php?op=debuglog&userid=$userid&start=$prev");
	}
	$result = db_query($sql);
	$odate = "";
	for ($i=0; $i<db_num_rows($result); $i++) {
