...
if ($op=="money"){
	...
	$me = "SELECT count(*) AS count FROM ".db_prefix("accounts")." WHERE $standardwhere AND (goldinbank+gold+round((((rand()*10)-5)/100)*(goldinbank+gold))) $meop ".($session['user']['goldinbank'] + $session['user']['gold']);
	$adverb = "richest";
	...
	$me = "SELECT count(*) AS count FROM ".db_prefix("accounts")." WHERE $standardwhere AND gems $meop {$session['user']['gems']}";
	if ($subop == "least") $adverb = "least";
	else $adverb = "most";
	$title = "The warriors with the $adverb gems in the land";
	$table = array($title, $sql);
} elseif ($op=="charm"){
	$sql = "SELECT name,$sexsel AS data1,race AS data2 FROM " . db_prefix("accounts") . " WHERE $standardwhere ORDER BY charm $order, level $order, experience $order, acctid $order LIMIT $limit";
	$me = "SELECT count(*) AS count FROM ".db_prefix("accounts")." WHERE $standardwhere AND charm $meop {$session['user']['charm']}";
	...
	$sql = "SELECT name,level AS data2 ,race as data1 FROM " . db_prefix("accounts") . " WHERE $standardwhere ORDER BY maxhitpoints $order, level $order, experience $order, acctid $order LIMIT $limit";
	$me = "SELECT count(*) AS count FROM ".db_prefix("accounts")." WHERE $standardwhere AND maxhitpoints $meop {$session['user']['maxhitpoints']}";
	$adverb = "toughest";
	...
	$me = "SELECT count(*) AS count FROM ".db_prefix("accounts")." WHERE $standardwhere AND resurrections $meop {$session['user']['resurrections']}";
	...
	$sql = "SELECT name, IF(bestdragonage,bestdragonage,'Unknown') AS data1 FROM " . db_prefix("accounts") . " WHERE $standardwhere $extra ORDER BY bestdragonage $order, level $order, experience $order, acctid $order LIMIT $limit";
	$me = "SELECT count(*) AS count FROM ".db_prefix("accounts")." WHERE $standardwhere $extra AND bestdragonage $meop {$session['user']['bestdragonage']}";
	$adverb = "fastest";
	...
	$sql = "SELECT name,dragonkills AS data1,level AS data2,'&nbsp;' AS data3, IF(dragonage,dragonage,'Unknown') AS data4, '&nbsp;' AS data5, IF(bestdragonage,bestdragonage,'Unknown') AS data6 FROM " . db_prefix("accounts") . " WHERE $standardwhere $extra ORDER BY dragonkills $order,level $order,experience $order, acctid $order LIMIT $limit";
	if ($session['user']['dragonkills']>0) $me = "SELECT count(*) AS count FROM ".db_prefix("accounts")." WHERE $standardwhere $extra AND dragonkills $meop {$session['user']['dragonkills']}";
	$adverb = "most";
	if ($subop == "least") $adverb = "least";
	$title = "Heroes with the $adverb dragon kills in the land";
...
if (isset($table) && is_array($table)){
	...
	if ($me>"" && $totalplayers){
		...
		output("`c`7You rank within around the top `&%s`7%% in this listing.`0`c",round(100*$row['count']/$totalplayers,0));
	}
}
