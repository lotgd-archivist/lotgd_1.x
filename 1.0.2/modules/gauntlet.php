...
function gauntlet_getmoduleinfo(){
	$info = array(
		...
		"settings"=>array(
			...
			"dkvisits"=>"Visits allowed per dk (0 for unlimited),int|1",
...
function gauntlet_run(){
	...
	$dkseen=get_module_pref("seendk");
	$dkvisits = get_module_setting("dkvisits");
	...
	if ($op==""){
		...
		if ($hpadd) output("However, for all of that, you feel tougher.`n");
		villagenav();
	}
	page_footer();
}
