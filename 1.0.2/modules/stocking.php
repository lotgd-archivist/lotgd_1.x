...
function stocking_run(){
	...
	if ($days > 8 && $days < 30){
		output("`&You notice `%Violet the Barmaid`& and `^Seth the Bard`& are putting up a Christmas tree.");
		...
		output("`7You notice `%Violet the Barmaid `7busily hanging stockings on the mantle above the fireplace.");
		...
		if (!get_module_pref("gotbuff")) {
			$buff = array(
				...
				"atkmod"=>1.1,
				"defmod"=>1.1,
				"roundmsg"=>"`2You become a whirling dirvish with your EggNog Dementia!`0"
			);
			set_module_pref("gotbuff",1);
			apply_buff("magicweak", $buff);
			output("`%Violet `&hands you some EggNog which you drink eagerly.`n`n");
		} else {
		...
		if (!get_module_pref("gotbuff")) {
			...
			set_module_pref("gotbuff",1);
			apply_buff("magicweak", $buff);
			output("`%Violet `&hands you some Hot Cocoa which you drink eagerly.`n`n");
		} else {
			output("You stand for a while enjoying the spirit before wandering away.`n`n");
		}
	} elseif ($days > -3 && $days < 1) {
		if (get_module_pref("gotprez")) {
			...
			case 6:
				...
				$buff = array(
					...
					"atkmod"=>1.2,
					"defmod"=>1.2,
					"roundmsg"=>"`2Your joy increase's your fighting abilities!`0"
				);
				apply_buff("magicweak", $buff);
				break;
			case 7:
	...
	viewcommentary("fireplace", "Chat around the fire", 25, "chats");
