...
function azrael_runevent($type) {
	...
	$from = "village.php?";
	$city = get_module_setting("azrealloc");
	$op = httpget('op');
    if ($op == "") {
		...
		output("`7If only %s `7could see you now.",$session['user']['sex']?"`^Seth":"`%Violet");
		...
		if ($bad==1){
			...
			output("If only %s `7could see you now.`n`n",$session['user']['sex']?"`^Seth":"`%Violet");
