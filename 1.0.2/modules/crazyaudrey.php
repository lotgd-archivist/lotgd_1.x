...
function crazyaudrey_baskets($type)
{
	...
	$lcanimal = strtolower($animal);
	$lcplural = strtolower(get_module_setting("animals"));
	...
	if ($op == "" || $op == "search" || $op == "baskets") {
		...
		if ($c1==$c2 && $c2==$c3){
			if ($c1==4){
				...
				output("\"`%Hedgehogs?  HEDGEHOGS??  Hahahahaha, HEDGEHOGS!!!!`5\" shouts Crazy Audrey as she snatches them up and runs screaming into the %s.", $where);
				output("You notice that she has dropped a full BAG of those wonderful salves.`n`n");
				output("`^You gain FIVE forest fights!");
				$session['user']['turns']+=5;
			}else{
				output("\"`%Argh, you are ALL very bad %s`%!`5\" shouts Crazy Audrey before hugging her shoulder %s`5 and putting it back in the basket.", $lcplureal, $lcanimal);
			...
			output("Despite her threatening words, Crazy Audrey pets the %s`5 on her shoulder before placing it back in the basket, and giving you your salve, which you rub all over your toes.`n`n", $lcanimal);
