...
function scry_dohook($hookname, $args) {
	...
	case "gypsy":
		...
		foreach($vloc as $loc=>$val) {
			...
			addnav(array("Scry %s", $loc), "runmodule.php?module=scry&op=pay&area=".htmlentities($val)."&village=".htmlentities($loc));
		}
		break;
	}
	return $args;
...
function scry_run(){
	...
	if ($op == "pay") {
		if ($session['user']['gold']>=$cost) {
			...
			redirect("runmodule.php?module=scry&op=talk&area=".htmlentities($area)."&village=".htmlentities($village));
			page_footer();
		} else {
		...
		if ($times && (get_module_pref("talks") >= $times)) {
			...
			addnav(array("Scrying (%s gold)", $cost));
			addnav(array("Scry %s", $village), "runmodule.php?module=scry&op=pay&area=".htmlentities($area)."&village=".htmlentities($village));
			addnav("Other");
			...
			viewcommentary($area,"Project",25,"projects");
