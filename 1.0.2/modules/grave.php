...
function grave_getmoduleinfo(){
	$info = array(
		...
		"settings"=>array(
			"Traveler's Grave Module Settings,title",
...
function grave_runevent($type,$from)
{
	...
	if ($op==""){
		output("`2By the roadside you see the crudely marked grave of a dead traveler.`n`n");
		...
		output("`)You gain %s favor with `4Ramius`).`0`n",$favorgain);
		$session['user']['turns']--;
