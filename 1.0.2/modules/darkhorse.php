...
function darkhorse_bartender($from){
	...
	if ($what==""){
		...
		if ($testtext) {
			output("`0You entered %s`n", prevent_colors(HTMLEntities($testtext)),true);
		...
		if ($who==""){
			...
			if ($session['user']['gold']>=100){
				...
				output("`4`bGold:`b`6 Definitely richer than you`n");
...
function darkhorse_runevent($type, $link){
	...
	case "oldman":
		...
		if ($session['user']['sex'] == SEX_MALE) {
			output("\"`7Aah, I met an adventurer in the woods, and figured I'd play a little game with her, but she won, and took almost all of my money.\"`n`n");
		} else {
			output("\"`7Aah, I met an adventurer in the woods, and figured I'd play a little game with him, but he won, and took almost all of my money.\"`n`n");
		}
		$c = navcount();
		if ($c != 0) {
