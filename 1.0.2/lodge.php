...
if ($op==""){
	...
	if ($entry){
		output("Upon showing it to him, he says, `3\"Very good %s, welcome to the J. C. Petersen Hunting Lodge.", ($session['user']['sex']?"ma'am":"sir"));
		...
		output("Nearby some other rugged hunters talk:`n");
		viewcommentary("hunterlodge","Talk quietly",25);
		addnav("Use Points");
		modulehook("lodge");
	}else{
		$iname = getsetting("innname", LOCATION_INN);
		output("You pull out your Frequent Boozer Card from %s, with 9 out of the 10 slots punched out with a small profile of Cedrik's Head.`n`n", $iname);
		output("The guard glances at it, advises you not to drink so much, and directs you down the path.");
	}
}else if ($op=="points"){
	output("`b`3Points:`b`n`n");
	output("`7For each $1 donated, the account which makes the donation will receive 100 contributor points in the game.");
	...
	output("Also, you can gain contributor points for contributing in other ways that the administration may specify.");
