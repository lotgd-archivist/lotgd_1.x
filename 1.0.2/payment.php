...
function writelog($response){
	...
	if ($match[1]>""){
		...
		if ($acctid>0){
			...
			$sql = "UPDATE " . db_prefix("accounts") . " SET donation = donation + '".($donation*100)."' WHERE acctid=$acctid";
			$result = db_query($sql);
			debuglog("Received donator points for donating -- Credited Automatically",false,$acctid,"donation",$donation*100,false);
...
if ($payment_errors>"") {
	...
	mail($adminEmail,$subj,$payment_errors,"From: " . getsetting("gameadminemail", "postmaster@localhost.com"));
}
