...
if ($op==""){
	...
	addnav("Months");
	while ($row = db_fetch_assoc($result)){
		addnav(date("M Y",strtotime($row['month']."-01"))." (\${$row['profit']})","paylog.php?month={$row['month']}");
	}
	$month = httpget('month');
	...
	rawoutput("<tr class='trhead'><td>Date</td><td>$type</td><td>$gross</td><td>$fee</td><td>$net</td><td>$processed</td><td>$who</td></tr>");
	for ($i=0;$i<db_num_rows($result);$i++){
		...
		if ($row['name']>"") {
			...
			rawoutput("-=( <a href='$link' title=\"".htmlentities($info['item_number'])."\" alt=\"".htmlentities($info['item_number'])."\">[".htmlentities($info['memo'])."]</a> )=-");
			addnav("",$link);
		}
		rawoutput("</td></tr>");
	}
