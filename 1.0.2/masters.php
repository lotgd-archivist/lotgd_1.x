<?php
// Initially written as a module by Chris Vrondran.
...
if ($op == "del") {
	...
	if ($id != 0) {
		...
		$sql = "INSERT INTO " . db_prefix("masters") . " (creatureid,creaturelevel,creaturename,creatureweapon,creaturewin,creaturelose) VALUES ($id,$lev,'$name', '$weapon', '$win', '$lose')";
	}
	db_query($sql);
	if ($id == 0) {
	...
	rawoutput("<input id='input' name='level' value='".htmlentities($row['creaturelevel'])."' SIZE=5>");
	output_notl("`n");
	output("`^Master's name:`n");
	rawoutput("<input id='input' name='name' value='".htmlentities($row['creaturename'])."'>");
	output_notl("`n");
	output("`^Master's weapon:`n");
	rawoutput("<input id='input' name='weapon' value='".htmlentities($row['creatureweapon'])."'>");
	output_notl("`n");
	output("`^Master's speech when player wins:`n");
	rawoutput("<textarea name='lose' rows='5' cols='30' class='input'>".htmlentities($row['creaturelose'])."</textarea>");
	output_notl("`n");
	output("`^Master's speech when player loses:`n");
	rawoutput("<textarea name='win' rows='5' cols='30' class='input'>".htmlentities($row['creaturewin'])."</textarea>");
	output_notl("`n");
	...
	output("%w = Loser's name (can be specified as {goodguy}`n");
	output("%W = Winner's name (can be specified as {badguy}`n");
	output("%x = Loser's weapon (can be specified as {weapon}`n");
	output("%X = Winner's weapon (can be specified as {creatureweapon}`n");
	output("%a = Loser's armor (can be specified as {armor}`n");
	output("%s = Subjective pronoun for losing player (him her)`n");
	output("%p = Possessive pronoun for losing player (his her)`n");
	output("%o = Objective pronoun for losing player (he she)`n");
}
