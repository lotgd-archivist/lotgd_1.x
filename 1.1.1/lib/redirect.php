...
function redirect($location,$reason=false){
	...
	if (strpos($location,"badnav.php")===false) {
		...
		tlschema("badnav");
		$session['output']=
			"<a href=\"".HTMLEntities($location, ENT_COMPAT, getsetting("charset", "ISO-8859-1"))."\">".translate_inline("Click here.")."</a>";
		$session['output'].=translate_inline("<br><br>If you cannot leave this page, notify the staff via <a href='petition.php'>petition</a> and tell them where this happened and what you did. Thanks.");
		tlschema();
	}
	restore_buff_fields();
