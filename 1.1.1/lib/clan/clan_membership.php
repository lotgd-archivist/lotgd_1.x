		...
		while ($row=db_fetch_assoc($result)){
			...
			if ($session['user']['clanrank']>CLAN_MEMBER){
				...
				if ($row['clanrank'] <= $session['user']['clanrank'] && $row['login']!=$session['user']['login']){
					rawoutput("<a href='clan.php?op=membership&remove=".rawurlencode($row['login'])."&whoacctid=".$row['acctid']."' onClick=\"return confirm('$confirm');\">$remove</a> ]");
					addnav("","clan.php?op=membership&remove=".$row['acctid']."&whoacctid=".$row['acctid']);
				}else{
