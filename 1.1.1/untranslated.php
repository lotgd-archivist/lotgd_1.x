	...
	if ($mode == "edit") {
		...
		if (db_num_rows($result)>0){
			...
			while ($row = db_fetch_assoc($result)){
				...
				addnav("", "untranslated.php?op=list&mode=edit&ns=". rawurlencode($row['namespace'], ENT_COMPAT, getsetting("charset", "ISO-8859-1")) ."&intext=". rawurlencode($row['intext']), ENT_COMPAT, getsetting("charset", "ISO-8859-1"));
				rawoutput("</td><td>");
