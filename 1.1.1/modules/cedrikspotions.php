	...
	if($op=="gems"){
		if ($gemcount==""){
			...
			if (get_module_setting("ischarm") == 1) {
				...
				if ($mincost != $maxcost) {
					$cm = get_module_setting("charmcost");
					output_notl("(%s %s for %s charm)", $cm, translate_inline($cm==1?"gem":"gems"), get_module_setting("charmgain"));
				}
				output_notl("`n");
			}
			if (get_module_setting("ismax") == 1) {
				...
				if ($mincost != $maxcost) {
					...
					output_notl("(%s %s for %s %s max %s)", $cm,
			...
			if (get_module_setting("istemp") == 1) {
				...
				if ($mincost != $maxcost) {
					...
					output_notl("(%s %s for %s %s)", $cm, translate_inline($cm==1?"gem":"gems"), $hm, translate_inline($hm == 1? "hitpoint":"hitpoints"));
				}
				output_notl("`n");
			}
			...
			if ($gemcount>$session['user']['gems']){
				...
				if (($wish == 4 || $wish == 5) && $gemcount > $cost) {
					output("Cedrik, feeling sorry for you, prevents you from paying for multiple doses of a potion that only needs a single dose.");
					$gemcount = $cost;
				}
