...
if ($stage==0){
	...
	if (file_exists("dbconnect.php")){
		...
		rawoutput("<option value=\"1\" ".($session['dbinfo']['DB_USEDATACACHE']?selected:'')."\">".translate_inline("Yes")."</option>");
		rawoutput("<option value=\"0\" ".(!$session['dbinfo']['DB_USEDATACACHE']?selected:'')."\">".translate_inline("No")."</option>");
		rawoutput("</select>");
		tip("Do you want to use a datacache for the sql queries? Many internal queries produce the same results and can be cached. This feature is *highly* recommended to use as the MySQL server is usually high frequented.");
	...
	if (httppostisset("DB_HOST")) {
		...
		$session['dbinfo']['DB_USEDATACACHE']=httppost("DB_USEDATACACHE");
