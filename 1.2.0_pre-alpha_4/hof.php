...
{
	global $user, $from, $to, $page, $playersperpage, $totalplayers;
...
if ($op=="money"){
	$sql = "SELECT name,(CAST(gold as signed)+goldinbank+round((((rand()*10)-5)/100)*(CAST(gold as signed)+goldinbank))) AS data1 FROM " . DB::prefix("accounts") . " WHERE $standardwhere ORDER BY data1 $order, level $order, experience $order, acctid $order LIMIT $limit";
	$me = "SELECT count(acctid) AS count FROM ".DB::prefix("accounts")." WHERE $standardwhere AND (goldinbank+CAST(gold as signed)+round((((rand()*10)-5)/100)*(goldinbank+CAST(gold as signed)))) $meop ".($user->goldinbank + $user->gold);
	$adverb = "richest";
	...
	$me = "SELECT count(acctid) AS count FROM ".DB::prefix("accounts")." WHERE $standardwhere AND gems $meop {$user->gems}";
	...
	$me = "SELECT count(acctid) AS count FROM ".DB::prefix("accounts")." WHERE $standardwhere AND charm $meop {$user->charm}";
	...
	$me = "SELECT count(acctid) AS count FROM ".DB::prefix("accounts")." WHERE $standardwhere AND maxhitpoints $meop {$user->maxhitpoints}";
	$adverb = "toughest";
	...
	$me = "SELECT count(acctid) AS count FROM ".DB::prefix("accounts")." WHERE $standardwhere AND resurrections $meop {$user->resurrections}";
	...
	$me = "SELECT count(acctid) AS count FROM ".DB::prefix("accounts")." WHERE $standardwhere $extra AND bestdragonage $meop {$user->bestdragonage}";
	$adverb = "fastest";
	...
	$unk = translate_inline("Unknown");
	$sql = "SELECT name,dragonkills AS data1,level AS data2,'&nbsp;' AS data3, IF(dragonage,dragonage,'$unk') AS data4, '&nbsp;' AS data5, IF(bestdragonage,bestdragonage,'$unk') AS data6 FROM " . DB::prefix("accounts") . " WHERE $standardwhere $extra ORDER BY dragonkills $order,level $order,experience $order, acctid $order LIMIT $limit";
	if ($user->dragonkills>0) $me = "SELECT count(acctid) AS count FROM ".DB::prefix("accounts")." WHERE $standardwhere $extra AND dragonkills $meop {$user->dragonkills}";
	$adverb = "most";
	if ($subop == "least") $adverb = "least";
	$title = "Heroes with the $adverb dragon kills in the land";
