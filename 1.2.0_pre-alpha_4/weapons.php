...
$basetext=array(
	...
	"desc"			=>	array(
		...
		array("He may be a humble weapons merchant, but he still carries himself with the grace of a man who has used his weapons to kill mightier %s than you.`n`n",translate_inline($user->sex?"women":"men")),
	...
	"tradein"		=>	array(
		...
		array("`!MightyE`7 looks at you and says, \"`#I'll give you `^%s`# trade-in value for your `5%s`#.",$tradeinvalue,$user->weapon),
...
if ($op==""){
	...
	$sql = "SELECT max(level) AS level FROM " .  DB::prefix("weapons") . " WHERE level<=".(int)$user->dragonkills;
	$result = DB::query($sql);
	...
	if (DB::num_rows($result)==0){
		...
		if ($row['value']>($user->gold+$tradeinvalue)){
			...
			output($texts['payweapon'],$user->weapon,$row['weaponname']);
			tlschema();
			debuglog("spent " . ($row['value']-$tradeinvalue) . " gold on the " . $row['weaponname'] . " weapon");
			$user->gold+=$tradeinvalue;
			$user->gold-=$row['value'];
			$user->weapon = $row['weaponname'];
			$user->attack+=$row['damage'];
			$user->attack-=$user->weapondmg;
			$user->weapondmg = $row['damage'];
			$user->weaponvalue = $row['value'];
			villagenav();
		}
	}
}
