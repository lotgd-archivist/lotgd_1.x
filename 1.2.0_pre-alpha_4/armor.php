...
checkday();
$tradeinvalue = round(($user->armorvalue*.75),0);
$basetext=array(
	"title"			=>	"Pegasus Armor",
	"desc"			=>	array(
	...
	"tradein"		=>	array(
		...
		array("Noticing for a moment that you are browsing her wares, she glances at your `&%s`5 and says that she'll give you `^%s`5 for them.`0`n`n",$user->armor, $tradeinvalue),
	),
...
if ($op==""){
  	...
  	$sql = "SELECT max(level) AS level FROM " . DB::prefix("armor") . " WHERE level<=".$user->dragonkills;
	$result = DB::query($sql);
	...
	if (DB::num_rows($result)==0){
		...
		if ($row['value']>($user->gold+$tradeinvalue)){
			...
			output($texts['payarmor'],$user->armor,$row['armorname'],$row['armorname']);
			tlschema();
			debuglog("spent " . ($row['value']-$tradeinvalue) . " gold on the " . $row['armorname'] . " armor");
			$user->gold+=$tradeinvalue;
			$user->gold-=$row['value'];
			$user->armor = $row['armorname'];
			$user->defense+=$row['defense'];;
			$user->defense-=$user->armordef;
			$user->armordef = $row['defense'];
			$user->armorvalue = $row['value'];
			villagenav();
		}
	}
}
