			...
			if (httpget("flirt")==""){
				...
				case 4:
					if (e_rand($c,11)>=11){
						...
						output("%s`0 apologizes, \"`^I'm sorry m'" . ($user->sex == SEX_FEMALE ? 'lady': 'lord') . ", I have no money to spare,`0\" as he turns out his moth-riddled pocket.",$partner);
						if ($c>0 && $c<10) $c--;
					}
					break;
				case 5:
				...
				case 6:
					if (e_rand($c,18)>=18){
						...
						addnews("`@%s`@ and %s`@ were seen heading up the stairs in the inn together.`0",$user->name,$partner);
						if ($c<25) $c++;
					}else{
						output("\"`^I'm sorry m'" . ($user->sex == SEX_FEMALE ? 'lady': 'lord') . ", but I have a show in 5 minutes`0\"");
						if ($c>0) $c--;
					}
					break;
				case 7:
					...
					if ($c>=22){
						...
						addnews("`&%s`& and %s`& are joined today in joyous matrimony!!!",$user->name,$partner);
						$user->marriedto=INT_MAX;
						apply_buff('lover',lovers_getbuff());
					}else{
