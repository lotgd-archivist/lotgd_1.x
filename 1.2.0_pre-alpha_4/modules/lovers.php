...
function lovers_dohook($hookname, $args){
	...
	case "newday":
		...
		if ($user->marriedto == 4294967295){
			...
			if ($user->charm<=0){
				output("`bWhen  you  wake  up, you find a note next to you, reading`n`5Dear %s`5,`n",$user->name);
				...
				addnews("`\$%s`\$ has left %s`\$ to pursue \"other interests.\"`0",$partner, $user->name);
				$user->marriedto=0;
				$user->charm=0;
			}
		}
