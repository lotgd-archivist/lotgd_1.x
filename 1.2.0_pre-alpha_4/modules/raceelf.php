...
function raceelf_uninstall(){
	...
	DB::query($sql);
	if ($user->location == $gname)
		$user->location = $vname;
	// Force anyone who was a Elf to rechoose race
	$sql = "UPDATE  " . DB::prefix("accounts") . " SET race='" . RACE_UNKNOWN . "' WHERE race='Elf'";
	DB::query($sql);
	if ($user->race == 'Elf')
		$user->race = RACE_UNKNOWN;
	return true;
}
...
function raceelf_dohook($hookname,$args){
	...
	case "changesetting":
		...
		if ($args['setting'] == "villagename" && $args['module']=="raceelf") {
			if ($user->location == $args['old'])
				$user->location = $args['new'];
	...
	case "villagetext":
		...
		break;
	case "weaponstext":
		$tradeinvalue = round(($user->weaponvalue*.75),0);
		if ($user->location == $city) {
			$args['schemas']['title'] = "module-raceelf";
			...
			if ($user->race == $race) {
				$args['desc'] = array(
					...
					array("`7You warm to his pride in your shared Elven blood, and proudly display your %s.`n`n",$user->weapon),
				);
				$args['tradein'] = array(
					array("`5Gadriel`7 looks at you and says, \"`5I'll give you `^%s`5 trade-in value for your `~%s`5.", $tradeinvalue, $user->weapon),
				...
				$args['desc'] = array(
					...
					array("`7You attempt bravado in the face of such arrogance, and thrust forward your %s for inspection.`n`n",$user->weapon),
				);
				$args['tradein'] = array(
					array("`5Gadriel`7 takes your `5%s`7 and examines it again quitely. After some seconds he gives it back to you with a cold look in his eyes.", $user->weapon),
...
function raceelf_checkcity(){
	...
	if ($user->race==$race && is_module_active("cities")){
