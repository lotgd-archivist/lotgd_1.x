...
function dag_dohook_private($hookname,$args){
	...
	case "dragonkill":
		...
		$sql = "UPDATE " . DB::prefix("bounty") . " SET status=1,winner=0,windate='$windate' WHERE target={$user->acctid} AND status=0";
		DB::query($sql);
		break;
	case "inn-desc":
		if (getsetting("pvp",1)) {
	...
	case "delete_character":
		...
		$windate = date("Y-m-d H:i:s");
		$sql = "UPDATE " . DB::prefix("bounty") . " SET status=1,winner=0,windate='$windate' WHERE target={$args['acctid']} AND status=0";
		DB::query($sql);
		break;
	case "superuser":
