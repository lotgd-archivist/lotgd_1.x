...
function dag_manage(){
	...
	$sql = "DELETE FROM " . DB::prefix("bounty") . " WHERE status=1 AND windate <'".date("Y-m-d H:i:s",strtotime("-".(getsetting("expirecontent",180)/10)." days"))."'";
	DB::query($sql);
	...
	if ($op == "") {
		...
		$sql = "SELECT bountyid,amount,target,setter,setdate FROM " . DB::prefix("bounty") . " WHERE status=0 ORDER BY bountyid ASC";
		$result = DB::query($sql);
		...
		for($i=0;$i<DB::num_rows($result);$i++){
			$row = DB::fetch_assoc($result);
				...
				$sql = "SELECT name,alive,sex,level,laston,loggedin,lastip,uniqueid FROM " . DB::prefix("accounts") . " WHERE acctid={$row['target']}";
				$result2 = DB::query($sql);
				if (DB::num_rows($result2) == 0) {
					/* this person has been deleted, clear bounties */
					$sql = "UPDATE " . DB::prefix("bounty") . " SET status=1 WHERE target={$row['target']}";
					DB::query($sql);
					continue;
				}
		...
		for($i=0;$i<$totlist;$i++) {
			...
			if ($listing[$i]['loggedin']) $laston=translate_inline("Now");
		...
		if (httpget('subfinal')==1){
			$sql = "SELECT acctid,name,login,level,locked,age,dragonkills,pk,experience FROM " . DB::prefix("accounts") . " WHERE name='".addslashes(rawurldecode(stripslashes(httppost('contractname'))))."' AND locked=0";
		}else{
			...
			$sql = "SELECT acctid,name,login,level,locked,age,dragonkills,pk,experience FROM " . DB::prefix("accounts") . " WHERE name LIKE '".addslashes($name)."' AND locked=0";
		}
		$result = DB::query($sql);
		if (DB::num_rows($result) == 0) {
			...
			if ($amt <= 0) {
				...
				// All good!
				$sql = "INSERT INTO " . DB::prefix("bounty") . " (amount, target, setter, setdate) VALUES ($amt, ".$row['acctid'].", 0, '".date("Y-m-d H:i:s")."')";
				DB::query($sql);
				output("Bounty added!");
			}
		}
		...
		if ($type=='search'){
			...
			if (httppost('setter')>'') {
				...
				$sql = "SELECT acctid FROM " . DB::prefix("accounts") . " WHERE name LIKE '$setter'";
				$result = DB::query($sql);
				$ids = array();
			...
			if (httppost('getter')>'') {
				...
				$sql = "SELECT acctid FROM " . DB::prefix("accounts") . " WHERE name LIKE '$getter'";
				$result = DB::query($sql);
				$ids = array();
				while ($row = DB::fetch_assoc($result)){
					array_push($ids,$row['acctid']);
			...
			if (httppost('target')>'') {
				...
				$sql = "SELECT acctid FROM " . DB::prefix("accounts") . " WHERE name LIKE '$target'";
				$result = DB::query($sql);
				$ids = array();
				while ($row = DB::fetch_assoc($result)){
					array_push($ids,$row['acctid']);
		...
		$sql = "SELECT bountyid,amount,target,setter,setdate,status,winner,windate FROM " . DB::prefix("bounty").$t.$s.$d;
		$result = DB::query($sql);
		rawoutput("<table border=0 cellpadding=2 cellspacing=1 bgcolor='#999999'>");
		$id = translate_inline("ID");
		...
		for($i=0;$i<DB::num_rows($result);$i++){
			...
			if ($row['target']==0) {
				...
				$sql = "SELECT name FROM " . DB::prefix("accounts") . " WHERE acctid=".(int)$row['target'];
				$result2 = DB::query($sql);
				if (DB::num_rows($result2) == 0) {
					$target['name'] = translate_inline("`4Deleted Character");
				} else {
					$target = DB::fetch_assoc($result2);
				}
			}
			if ($row['setter']==0) {
				$setter['name'] = translate_inline("`2Green Dragon");
			}else {
				$sql = "SELECT name FROM " . DB::prefix("accounts") . " WHERE acctid=".(int)$row['setter'];
				$result3 = DB::query($sql);
				if (DB::num_rows($result3) == 0) {
					$setter['name'] = translate_inline("`4Deleted Character");
				} else {
					$setter = DB::fetch_assoc($result3);
				}
			}
			$winner['name'] = "";
			if (($row['winner']==0) && $row['status'] == 1) {
				...
				$sql = "SELECT name FROM " . DB::prefix("accounts") . " WHERE acctid=".(int)$row['winner'];
					$result4 = DB::query($sql);
				if (DB::num_rows($result4) == 0) {
					$winner['name'] = translate_inline("`2Deleted Character");
				} else {
					$winner = DB::fetch_assoc($result4);
				}
			}
			rawoutput("<tr class='".($i%2?"trdark":"trlight")."'><td>");
		...
		$sql = "UPDATE " . DB::prefix("bounty") . " SET status=1,winner=0,windate=\"$windate\" WHERE bountyid=$bountyid";
		DB::query($sql);
...
function dag_pvpwin($args){
	...
	// Check for Bounty
	$sql = "SELECT bountyid,amount,setter FROM " . DB::prefix("bounty") . " WHERE status=0 AND setdate<='".date("Y-m-d H:i:s")."' AND target=".$badguy['acctid'];
	$result = DB::query($sql);
	if (DB::num_rows($result) > 0) {
		$totgoodamt = 0;
		$totbadamt = 0;
		for($i=0;$i<DB::num_rows($result);$i++){
			$row = DB::fetch_assoc($result);
			if ($row['setter'] == $user->acctid) {
				$totbadamt += $row['amount'];
			} else {
				$totgoodamt += $row['amount'];
				$windate = date("Y-m-d H:i:s");
				$sql = "UPDATE " . DB::prefix("bounty") . " SET status=1,winner=".$user->acctid.",windate=\"$windate\" WHERE bountyid=".$row['bountyid'];
				DB::query($sql);
			}
		}
		if ($totgoodamt > 0) {
	...
	// Add Bounty Gold
	if ($totgoodamt > 0) {
		$user->gold+=$totgoodamt;
		debuglog("gained ".$totgoodamt." gold bounty for killing ", $badguy['acctid']);
	}
	// End Add Bounty Gold
	// Add Bounty Kill to News
	if ($totgoodamt > 0) {
		addnews("`4%s`3 collected `4%s`3 gold bounty by turning in `4%s`3's head!",$user->name,$totgoodamt,$badguy['creaturename']);
	}
