...
function dag_run_private(){
	...
	if ($op=="list"){
		...
		$sql = "SELECT bountyid,amount,target,setter,setdate FROM " . DB::prefix("bounty") . " WHERE status=0 AND setdate<='".date("Y-m-d H:i:s")."' ORDER BY bountyid ASC";
		$result = DB::query($sql);
		...
		for($i=0;$i<DB::num_rows($result);$i++){
			...
			$sql = "SELECT name,alive,sex,level,laston,loggedin,lastip,location FROM " . DB::prefix("accounts") . " WHERE acctid={$row['target']}";
		...
		if (DB::num_rows($result) == 0) {
			...
			if ($row['locked']) {
				...
				$curbounty = 0;
				$sql = "SELECT sum(amount) AS total FROM " . DB::prefix("bounty") . " WHERE status=0 AND target={$row['acctid']}";
				$result = DB::query($sql);
				if (DB::num_rows($result) > 0) {
					$nrow = DB::fetch_assoc($result);
				...
				if ($amt < $min) {
					...
					$sql = "INSERT INTO ". DB::prefix("bounty") . " (amount, target, setter, setdate) VALUES ($amt, ".$row['acctid'].", ".(int)$user->acctid.", '".date("Y-m-d H:i:s",$setdate)."')";
					DB::query($sql);
					// ***END ADD***
		...
		// By Andrew Senger
		// Adding for new Bounty Code
		$sql = "SELECT sum(amount) as total FROM " . DB::prefix("bounty") . " WHERE status=0 AND setdate<='".date("Y-m-d H:i:s")."' AND target=".$user->acctid;
		$result = DB::query($sql);
		$curbounty = 0;
		if (DB::num_rows($result) != 0) {
			$row = DB::fetch_assoc($result);
