...
function sethsong_sing()
{
	...
	case 2:
		...
		output("Membrane man hates %s`^ man.`n", $user->name);
	...
	case 4:
		...
		$user->hitpoints=round(max($user->maxhitpoints,$user->hitpoints)*(($lgain/100)+1),0);
		break;
	case 5:
	...
	case 7:
		...
		$user->hitpoints-=round($user->maxhitpoints * ($bloss/100),0);
		if ($user->hitpoints<=0)
			$user->hitpoints=1;
		break;
	case 8:
	...
	case 16:
		...
		$user->hitpoints-=round($user->maxhitpoints*($sloss/100),0);
		if ($user->hitpoints<1)
			$user->hitpoints=1;
		break;
	case 17:
		output("%s`0 begins to play, but a rowdy patron stumbles past, spilling beer on you.", getsetting("bard", "`^Seth"));
		output("You miss the performance as you wipe the swill from your %s.", $user->armor);
		break;
	case 18:
