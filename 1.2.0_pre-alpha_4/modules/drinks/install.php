...
function drinks_install_private(){
	...
	// buffactivate field
	$sql = "DESCRIBE ". DB::prefix("drinks");
	$result = DB::query($sql);
	while($row = DB::fetch_assoc($result)) {
		if ($row['Field']=="buffactivate"){
			debug("Dropping buffactivate from the drinks table.");
			$sql = "ALTER TABLE " . DB::prefix("drinks") . " DROP buffactivate";
			DB::query($sql);
		} // end if
		if ($row['Field']=="hppercent" && $row['Type']=="float") {
			debug("Altering {$row['Field']} from float to int in the drinks table.");
			$sql = "UPDATE " . DB::prefix("drinks") . " SET hppercent=hppercent*100";
			DB::query($sql);
			$sql = "ALTER TABLE " . DB::prefix("drinks") . " CHANGE {$row['Field']} {$row['Field']} int(11) NOT NULL DEFAULT 0";
			DB::query($sql);
		}
		if (($row['Field']=="buffatkmod" || $row['Field']=="buffdefmod" ||
			...
			$sql = "ALTER TABLE " . DB::prefix("drinks") . " CHANGE {$row['Field']} {$row['Field']} text NOT NULL";
			DB::query($sql);
		}
	} // end while
