...
function drinks_dohook_private($hookname,$args) {
	...
	case "ale":
		...
		$drinktexts = modulehook("drinks-text",$texts);
		...
		if (get_module_pref('harddrinks')>=get_module_setting('hardlimit')) {
			...
			$remark = str_replace("{barkeep}", $drinktexts['barkeep']."`0", $remark);
		...
		while ($row = DB::fetch_assoc($result)) {
			...
			if ($row['allowdrink']) {
				$drinkcost = $row['costperlevel']*$user->level;
	...
	case "soberup":
		...
		break;
	case "commentary":
		if (($user->superuser & SU_IS_GAMEMASTER) && substr($args['commentline'], 0, 5) == "/game") break;
		...
		break;
	case "superuser":
		if (($user->superuser & SU_EDIT_USERS) || get_module_pref("canedit")) {
