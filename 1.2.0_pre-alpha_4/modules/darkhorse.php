...
function darkhorse_install(){
	...
	while($row = DB::fetch_assoc($result)) {
		if ($row['Field'] == "tavern") {
			...
			$sql = "INSERT INTO " . DB::prefix("module_objprefs") . " (modulename,objtype,setting,objid,value) SELECT 'darkhorse','mounts','findtavern',mountid,tavern FROM " . DB::prefix("mounts");
			DB::query($sql);
			debug("Dropping tavern field from mounts table");
			$sql = "ALTER TABLE " . DB::prefix("mounts") . " DROP tavern";
			DB::query($sql);
		}
	}
...
function darkhorse_checkday(){
	...
	checkday();
	// And now set it back.
	$user->specialinc="module:darkhorse";
}
...
function darkhorse_bartender($from){
	...
	if ($what==""){
		...
		$dname = translate_inline($user->sex?"lasshie":"shon");
		...
		if ($who==""){
			...
			if ($user->gold>=100){
				$sql = "SELECT name,acctid,alive,location,maxhitpoints,gold,sex,level,weapon,armor,attack,race,defense,charm FROM " . DB::prefix("accounts") . " WHERE login='$who'";
				$result = DB::query($sql);
...
function darkhorse_runevent(){
	global $user;
	$from = func_get_arg(1);
	$gameret = substr($from, 0, -1);
	$user->specialinc="module:darkhorse";
