		...
		if (($visits < $max) || ($max==0)){
			...
			if ($user->gold < $cost){
				...
				if ($almost==5){
					...
					addnews("%s won %s gold after rolling 5 sixes in the %s.",$user->name,$prize, get_module_setting("tavernname","darkhorse"));
					...
					addnews("%s won %s gold after rolling 4 sixes in the %s.",$user->name,$win, get_module_setting("tavernname", "darkhorse"));
					...
					debuglog("won $win gold at sixes game.");
					addnews("%s won %s gold after rolling 3 sixes in the %s.",$user->name,$win,get_module_setting("tavernname", "darkhorse"));
