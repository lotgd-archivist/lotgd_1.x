...
function game_dice_run(){
	...
	if ($user->gold>0){
		...
		if ($bet<=0){
			...
			output("`3\"`!How much would you bet young %s?`3\"", translate_inline($user->sex?"lady":"man"));
			...
			output("Desperate to really show him good, you open up your purse and spill out its contents: `^%s`3 gold.`n`n", $user->gold);
			...
			if ($what!="keep"){
				...
				output("You roll your %s die, and it comes up as `b%s`b`n`n", $die, $user->specialmisc);
				...
				output("Your final roll was `b%s`b, the old man will now try to beat it:`n`n", $user->specialmisc);
				$r = e_rand(1,6);
				output("The old man rolls a %s...`n", $r);
				if ($r>$user->specialmisc || $r==6){
					output("\"`7I think I'll stick with that roll!`0\" he says.`n");
				}else{
					$r = e_rand(1,6);
					output("The old man rolls again and gets a %s...`n", $r);
					if ($r>=$user->specialmisc){
						output("\"`7I think I'll stick with that roll!`0\" he says.`n");
					}else{
						$r = e_rand(1,6);
