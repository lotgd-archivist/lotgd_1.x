...
if ($op=="edit"){
	...
	if ($tauntid!=""){
		$sql = "UPDATE " . DB::prefix("taunts") . " SET taunt=\"$taunt\",editor=\"".addslashes($user->login)."\" WHERE tauntid=\"$tauntid\"";
	}else{
		$sql = "INSERT INTO " . DB::prefix("taunts") . " (taunt,editor) VALUES (\"$taunt\",\"".addslashes($user->login)."\")";
	}
	DB::query($sql);
	$op = "";
	httpset("op", "");
