...
$logd_version = "1.2.0 Dragonprime Edition Pre-Alpha 4";
$logd_version_release_date = date('M d Y');
header("Cache-Control: no-cache, must-revalidate"); // HTTP/1.1
header("Expires: Sat, 26 Jul 1997 05:00:00 GMT");
// Make handler functions if we don't have zlib
...
if(isset($DB_HOST)) {
	...
	Settings::setUseCache($DB_USEDATACACHE);
	Settings::setCachePath($DB_DATACACHEPATH);
}
...
if ($link===false){
 	if (!IS_INSTALLER){
		...
		if (file_exists("lib/smsnotify.php")) {
			$smsmessage = "No DB Server: " . DB::error();
...
if (!DB_CONNECTED || !DB::select_db()){
	if (!IS_INSTALLER && DB_CONNECTED){
		// Ignore this bit.  It's only really for Eric's server
		if (file_exists("lib/smsnotify.php")) {
			$smsmessage = "Cant Attach to DB: " . DB::error();
			require_once("lib/smsnotify.php");
		}
		// And tell the user it died.  No translation here, we need the DB for
		// translation.
...
// Initates settings
$settings = Settings::getSingleon();



header("Content-Type: text/html; charset=".getsetting('charset','ISO-8859-1'));

if (strtotime("-".getsetting("LOGINTIMEOUT",900)." seconds") > Session::get('lasthit') && Session::get('lasthit')>0 && Session::get('loggedin')){
	...
	translator_setup();
	Session::set('message', translate_inline("`nYour session has expired!`n","common"));
}
Session::set('lasthit', strtotime('now'));
if(Session::get('output') && @gzuncompress(Session::get('output'))) Session::set('output', @gzuncompress(Session::get('output')));
...
if (!isset($user->loggedin)) $user->loggedin=false;
Session::set('loggedin', (bool)$user->loggedin);

if ($user->loggedin!=true && !ALLOW_ANONYMOUS){
	redirect("login.php?op=logout");
}

if (!isset($user->gentime)) $user->gentime = 0;
if (!isset($user->gentimecount)) $user->gentimecount = 0;
if (!isset($user->gensize)) $user->gensize = 0;
if (!isset($user->acctid)) $user->acctid = 0;
if (!Session::exists('counter')) Session::set('counter', 0);
...
if ($logd_version != getsetting("installer_version","-1") && !IS_INSTALLER){
	...
	rawoutput("<meta http-equiv='refresh' content='30; url={$user->restorepage}'>");
...
if (isset($user->bufflist) && is_array(unserialize($user->bufflist))) {
	Session::set('bufflist', unserialize($user->bufflist));
} else {
	Session::set('bufflist', array());
}
$user->lastip=$REMOTE_ADDR;
if (!isset($_COOKIE['lgi']) ||  strlen($_COOKIE['lgi'])<32){
	if (strlen($user->uniqueid)<32){
		$u=md5(microtime());
		setcookie("lgi",$u,strtotime("+365 days"));
		$_COOKIE['lgi']=$u;
		$user->uniqueid=$u;
	}else{
		setcookie("lgi",$user->uniqueid,strtotime("+365 days"));
	}
}else{
	$user->uniqueid=$_COOKIE['lgi'];
}
	...
	if ($site != $host){
		...
		if ($row['refererid']>""){
			...
			if (e_rand(1,100)==2){
				...
				DB::query("DELETE FROM ".DB::prefix("referers")." WHERE last < '$timestamp' LIMIT 300");
...
if (!isset($user->superuser)) $user->superuser=0;
...
if (!isset($user->hashorse)) $user->hashorse=0;
$playermount = getmount($user->hashorse);
$temp_comp = @unserialize($user->companions);
$companions = array();
...
$sql = "SELECT * FROM " . DB::prefix("clans") . " WHERE clanid='{$user->clanid}'";
$result = DB::query_cached($sql, "clandata-{$user->clanid}", 3600);
