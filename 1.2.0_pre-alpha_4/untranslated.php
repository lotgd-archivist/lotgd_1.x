...
if ($op == "list") {
	...
	$sql = "SELECT namespace,count(*) AS c FROM " . DB::prefix("untranslated") . " WHERE language='".$user->prefs['language']."' GROUP BY namespace ORDER BY namespace ASC";
	$result = DB::query($sql);
	...
	if ($mode == "edit") {
		...
		$sql = "SELECT * FROM " . DB::prefix("untranslated") . " WHERE language='".$user->prefs['language']."' AND namespace='".$namespace."'";
		$result = DB::query($sql);
	...
	if ($count['count'] > 0) {
		$sql = "SELECT * FROM " . DB::prefix("untranslated") . " WHERE language = '" . $user->prefs['language'] . "' ORDER BY rand(".e_rand().") LIMIT 1";
		$result = DB::query($sql);
