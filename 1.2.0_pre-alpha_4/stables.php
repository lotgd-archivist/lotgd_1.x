...
$basetext=array(
	...
	"desc"=>array(
		...
		array("You approach, and he whirls around, pointing a pitchfork in your general direction, \"`&Ach, sorry m'%s, I dinnae hear ya' comin' up on me, an' I thoht fer sure ye were %s`&; he what been tryin' to improve on his dwarf tossin' skills. ",translate_inline($user->sex?'lass':'lad'),getsetting('barkeep','`tCedrik')),
...
if ($playermount) {
	...
	$grubprice = round($user->level*$playermount['mountfeedcost'], 0);
}
$confirm = 0;
...
if ($op == 'confirmbuy') {
	...
	if (DB::num_rows($result)<=0){
		...
		$mount = DB::fetch_assoc($result);
		if (($user->gold+$repaygold) < $mount['mountcostgold'] ||
			($user->gems+$repaygems) < $mount['mountcostgems']){
	...
	if (getsetting("allowfeed", 0) == 0) {
		...
		output($texts['nofeed'],
				($user->sex?$texts["lass"]:$texts["lad"]));
		tlschema();
	} elseif($user->gold>=$grubprice) {
		$buff = unserialize($playermount['mountbuff']);
		if (!isset($buff['schema']) || $buff['schema'] == "") $buff['schema'] = "mounts";
		$mountbuff = Session::getNested('bufflist', 'mount');
		if ($mountbuff && $mountbuff['rounds'] == $buff['rounds']) {
			tlschema($schemas['nothungry']);
			output($texts['nothungry'],$name);
			tlschema();
		} else {
			if ($mountbuff && $mountbuff['rounds'] > $buff['rounds']*.5) {
...
if ($confirm == 0) {
	if ($user->hashorse>0){
		...
		if (getsetting("allowfeed", 0) && $user->fedmount==0) {
			addnav(array("Feed %s`0 (`^%s`0 gold)", $lcname, $grubprice),
					"stables.php?op=feed");
		}
	}

	$sql = "SELECT mountname,mountid,mountcategory,mountdkcost FROM " . DB::prefix("mounts") .  " WHERE mountactive=1 AND mountlocation IN ('all','{$user->location}') ORDER BY mountcategory,mountcostgems,mountcostgold";
	$result = DB::query($sql);
	$category="";
	...
	for ($i=0;$i<$number;$i++){
		...
		if ($row['mountdkcost'] <= $user->dragonkills)
