...
if ($category > "") {
	$cat = "&cat=$category";
	$sqlcat = "WHERE ".DB::prefix("gamelog").".category = '$category'";
} else {
	$cat='';
	$sqlcat='';
}
...
$sql = "SELECT ".DB::prefix("gamelog").".*, ".DB::prefix("accounts").".name AS name FROM ".DB::prefix("gamelog")." LEFT JOIN ".DB::prefix("accounts")." ON ".DB::prefix("gamelog").".who = ".DB::prefix("accounts").".acctid $sqlcat LIMIT $start,500";
$next = $start+500;
$prev = $start-500;
addnav("Operations");
