...
if ($op==""){
	...
	$sql = "SELECT * FROM " .  DB::prefix("companions") . "
				WHERE companioncostdks<={$user->dragonkills}
				AND (companionlocation = '{$user->location}' OR companionlocation = 'all')
	...
	while ($row = DB::fetch_assoc($result)) {
		...
		if ($row['companioncostgold'] && $row['companioncostgems']) {
			if ($user->gold >= $row['companioncostgold'] && $user->gems >= $row['companioncostgems'] && !isset($companions[$row['name']])) {
			...
			if ($user->gold >= $row['companioncostgold'] && !isset($companions[$row['name']])) {
			...
			if ($user->gems >= $row['companioncostgems'] && !isset($companions[$row['name']])) {
	...
	if ($row = DB::fetch_assoc($result)) {
		$row['attack'] = $row['attack'] + $row['attackperlevel'] * $user->level;
		$row['defense'] = $row['defense'] + $row['defenseperlevel'] * $user->level;
		$row['maxhitpoints'] = $row['maxhitpoints'] + $row['maxhitpointsperlevel'] * $user->level;
		...
		if (apply_companion($row['name'], $row)) {
			...
			$user->gold -= $row['companioncostgold'];
			$user->gems -= $row['companioncostgems'];
...
function healnav($companions, $texts, $schemas) {
	...
	foreach ($companions as $name => $companion) {
		if (isset($companion['cannotbehealed']) && $companion['cannotbehealed'] == true) {
			...
			if ($pointstoheal > 0) {
				$healable = true;
				$costtoheal = round(log($user->level+1) * ($pointstoheal + 10)*1.33);
				if ($user->gold >= $costtoheal) {
