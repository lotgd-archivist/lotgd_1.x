...
$cost = log($user->level) * (($user->maxhitpoints-$user->hitpoints) + 10);
...
if ($op==""){
	...
	}elseif($user->hitpoints == $user->maxhitpoints){
	...
	if ($user->gold>=$newcost){
		...
		$diff = round(($user->maxhitpoints-$user->hitpoints)*$pct/100,0);
		$user->hitpoints += $diff;
		if($newcost){
...
foreach($companions as $name => $companion){
	if(isset($companion['cannotbehealed']) && $companion['cannotbehealed'] == true){
		...
		if($points > 0){
			$compcost = round(log($user->level+1) * ($points + 10)*1.33);
