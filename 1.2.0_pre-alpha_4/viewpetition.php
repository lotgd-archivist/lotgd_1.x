...
if (trim(httppost('insertcommentary'))!="") {
	...
	$sql = "UPDATE " . DB::prefix("petitions") . " SET closeuserid='{$user->acctid}',closedate='".date("Y-m-d H:i:s")."' WHERE petitionid='$id'";
	DB::query($sql);
}
...
if ($op==""){
	...
	if ($setstat!=""){
		...
		if ($row['status']!=$setstat){
			$sql = "UPDATE " . DB::prefix("petitions") . " SET status='$setstat',closeuserid='{$user->acctid}',closedate='".date("Y-m-d H:i:s")."' WHERE petitionid='$id'";
			DB::query($sql);
			invalidatedatacache("petition_counts");
		}
	}
	...
	$page = httpget("page");
	if ($page == "") {
		if (Session::exists('petitionPage')){
			$page = (int)Session::get('petitionPage');
		}else{
			$page = 1;
		}
	}
	...
	while($row = DB::fetch_assoc($result)){
		...
		if (isset($row['status']) && $row['status']!=$laststatus){
	...
	if ($row['acctid']>0 && $user->superuser & SU_EDIT_USERS){
		addnav("User Ops");
		addnav("R?Edit User Record","user.php?op=edit&userid={$row['acctid']}&returnpetition=$id");
	}
	if ($row['acctid']>0 && $user->superuser & SU_EDIT_DONATIONS){
		addnav("User Ops");
