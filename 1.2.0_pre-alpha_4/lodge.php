...
$pointsavailable =
	$user->donation-$user->donationspent;
$entry = ($user->donation > 0) || ($user->superuser & SU_EDIT_COMMENTS);
...
if ($op==""){
	...
	if ($entry){
		output("Upon showing it to him, he says, `3\"Very good %s, welcome to the J. C. Petersen Hunting Lodge.", translate_inline($user->sex?"ma'am":"sir"));
		output("You have earned `^%s`3 points and have `^%s`3 points available to spend,\"`7 and admits you in.`n`n", $user->donation, $pointsavailable);
