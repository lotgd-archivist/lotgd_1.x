...
if ($op=="val"){
	$id = httpget('id');
	$sql = "SELECT acctid,login,password,name FROM ". DB::prefix("accounts") . " WHERE emailvalidation='$id' AND emailvalidation!=''";
	$result = DB::query($sql);
...
if ($op=="forgot"){
	...
	if ($charname!=""){
		$sql = "SELECT acctid,login,emailaddress,emailvalidation,password FROM " . DB::prefix("accounts") . " WHERE login='$charname'";
		$result = DB::query($sql);
		if (DB::num_rows($result)>0){
			$row = DB::fetch_assoc($result);
			if (trim($row['emailaddress'])!=""){
				if ($row['emailvalidation']==""){
					...
					$sql = "UPDATE " . DB::prefix("accounts") . " SET emailvalidation='{$row['emailvalidation']}' where login='{$row['login']}'";
					DB::query($sql);
				}
	...
	if ($op=="create"){
		...
		if (soap($shortname)!=$shortname){
			...
			if (getsetting("blockdupeemail",0)==1 && getsetting("requireemail",0)==1){
				$sql = "SELECT login FROM " . DB::prefix("accounts") . " WHERE emailaddress='$email'";
				$result = DB::query($sql);
				if (DB::num_rows($result)>0){
					$blockaccount=true;
			...
			if (!$blockaccount){
				...
				if (DB::num_rows($result)>0){
					...
					$refer = httpget('r');
					if ($refer>""){
						$sql = "SELECT acctid FROM " . DB::prefix("accounts") . " WHERE login='$refer'";
						$result = DB::query($sql);
