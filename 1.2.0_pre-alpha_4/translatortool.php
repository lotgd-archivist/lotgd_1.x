...
if ($op==""){
	...
	if ($trans>""){
		...
		if (DB::num_rows($result)==0){
			$sql = "INSERT INTO ".DB::prefix("translations")." (language,uri,intext,outtext,author,version) VALUES ('".LANGUAGE."','$uri','$text','$trans','{$user->login}','$logd_version ')";
			...
			if ($row['intext'] == $text){
				$sql = "UPDATE ".DB::prefix("translations")." SET author='{$user->login}', version='$logd_version', uri='$uri', outtext='$trans' WHERE tid={$row['tid']}";
			}else{
				$sql = "INSERT INTO " . DB::prefix("translations") . " (language,uri,intext,outtext,author,version) VALUES ('" . LANGUAGE . "','$uri','$text','$trans','{$user->login}','$logd_version ')";
			...
			$sql = "UPDATE ".DB::prefix("translations")." SET author='{$user->login}', version='$logd_version', uri='$page', outtext='$trans' WHERE tid IN (".join(",",$rows).")";
		}
	}
	DB::query($sql);
