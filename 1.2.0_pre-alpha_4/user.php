...
$sql = "SELECT clanid FROM " . DB::prefix("accounts") . " WHERE acctid='$userid'";
$row = DB::fetch_assoc(DB::query($sql));
$clanid = $row['clanid'];
$ranks = modulehook("clanranks", array("ranks"=>$ranks, "clanid"=>$clanid));
$ranks = $ranks['ranks'];
$rankstring = "";
...
$userinfo = array(
	...
	"referer"=>"ID of player who referred this player," .
		(($user->superuser & SU_EDIT_DONATIONS) ? "int" : "viewonly"),
	...
	"marriedto"=>"Is married to the player with AcctID," .
		(($user->superuser & SU_MEGAUSER) ? "int" : "viewonly"),
	...
	"superuser"=>"Superuser Permissions".
		...
		"bitfield,".
		($user->superuser | SU_ANYONE_CAN_SET |
		  ($user->superuser&SU_MEGAUSER ? 0xFFFFFFFF : 0)).",".
		...
		(Settings::doShowAdvance()? SU_EDIT_ADVANCE_CONFIG . ",Edit Advance Game Configurations*," : "") .
