...
if ($lastexpire < $needtoexpire){
	...
	$old-=5;
	$sql = "SELECT acctid,emailaddress FROM " . DB::prefix("accounts") . " WHERE 1=0 ".($old>0?"OR (laston < \"".date("Y-m-d H:i:s",strtotime("-$old days"))."\")\n":"")." AND emailaddress!='' AND sentnotice=0 AND (superuser&".NO_ACCOUNT_EXPIRATION.")=0";
	$result = DB::query($sql);
	$subject = translate_inline("LoGD Character Expiration");
	$body = sprintf_translate("One or more of your characters in " . Settings::getsetting('gtitle', 'Legend of the Green Dragon') . " at %s is about to expire.  If you wish to keep this character, you should log on to him or her soon!",getsetting("serverurl","http://".$_SERVER['SERVER_NAME'].($_SERVER['SERVER_PORT'] == 80?"":":".$_SERVER['SERVER_PORT']).dirname($_SERVER['REQUEST_URI'])));
	while ($row = DB::fetch_assoc($result)) {
		mail($row['emailaddress'],$subject,
		$body,
		"From: ".getsetting("gameadminemail","postmaster@localhost.com")
		);
		$sql = "UPDATE " . DB::prefix("accounts") . " SET sentnotice=1 WHERE acctid='{$row['acctid']}'";
		DB::query($sql);
	}
}
?>
