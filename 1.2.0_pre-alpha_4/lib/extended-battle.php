...
function show_enemies($enemies) {
	...
	if ($user->alive){
		output("`2YOUR Hitpoints: `6%s`0`n",$user->hitpoints);
	}else{
		output("`2YOUR Soulpoints: `6%s`0`n",$user->hitpoints);
	}
}
...
function unsuspend_companions($susp, $nomsg=false) {
	...
	if (is_array($companions)) {
		foreach ($companions as $name => $companion) {
			if (isset($companion['suspended']) && $companion['suspended'] == true && !(isset($companion[$susp]) && $companion[$susp] == true)) {
				$notify = true;
...
function report_companion_move($companion, $activate="fight") {
	global $badguy, $user;
	global $count,$defended,$needtosstopfighting;
	...
	if ($activate == "fight" && isset($companion['abilities']['fight']) && $companion['abilities']['fight'] == true && $companion['used'] == false) {
		...
		if ($user->hitpoints < $user->maxhitpoints) {
			$hptoheal = min($companion['abilities']['heal'], $user->maxhitpoints - $user->hitpoints);
			$user->hitpoints += $hptoheal;
			$companion['used'] = true;
...
function rollcompaniondamage($companion){
	...
	global $creaturedefmod,$compdefmod,$compatkmod,$buffset,$atk;
...
function load_ai_script($scriptid) {
	if ($scriptid == 0) {
		...
		$sql = "SELECT script, file FROM ".DB::prefix("scripts")." WHERE scriptid = $scriptid";
		$result = DB::query($sql);
		$row = DB::fetch_assoc($result);
		if ($row['file']) return file_get_contents('scripts/' . $row['file']);
		return $row['script'];
	}
}
