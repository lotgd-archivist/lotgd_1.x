...
function logd_error_handler($errno, $errstr, $errfile, $errline){
	...
	case E_USER_NOTICE:
		if (getsetting('show_notices', 0) &&
				$user->superuser & SU_SHOW_PHPNOTICE) {
	...
	case E_USER_WARNING:
		...
		if (getsetting("notify_on_warn",0) > ""){
			...
			logd_error_notify($errstr, $errfile, $errline, $backtrace);
		}
		break;
	case E_ERROR:
	case E_USER_ERROR:
...
function logd_error_notify($errstr, $errfile, $errline, $backtrace){
	global $user;
	...
	if ($data['firstrun']){
		...
		if ($do_notice){
			...
			$userstr = "";
			if ($user && isset($user->name) && isset($user->acctid)) {
				$userstr = "Error triggered by user " . $user->name . " (" . $user->acctid . ")\n";
			}
