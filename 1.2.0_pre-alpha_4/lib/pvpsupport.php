...
function setup_target($name) {
	...
	if (DB::num_rows($result)>0){
		...
		if (abs($user->level-$row['creaturelevel'])>2){
			...
			$row['playerstarthp'] = $user->hitpoints;
...
function pvpvictory($badguy, $killedloc, $options)
{
	...
	if ($user->level == Settings::getsetting('maxlevel', 15)) $winamount = 0;
	output("`#You receive `^%s`# gold!`n", $winamount);
	$user->gold+=$winamount;

	$exp = round(getsetting("pvpattgain",10)*$badguy['creatureexp']/100,0);
	if ($user->level == Settings::getsetting('maxlevel', 15)) $exp = 0;
	$expbonus = round(($exp *
...
//	debuglog("gained $winamount ({$badguy['creaturegold']} base) gold and $wonexp exp (loser lost $lostexp) for killing ", $badguy['acctid']);
	...
	debuglog("started the fight and defeated {$badguy['creaturename']} in $killedloc (earned $winamount of {$badguy['creaturegold']} gold and $wonexp of $lostexp exp)",false,$user->acctid);
	debuglog("was victim and has been defeated by {$user->name} in $killedloc (lost {$badguy['creaturegold']} gold and $lostexp exp, actor tooks $winamount gold and $wonexp exp)",false,$badguy['acctid']);
	...
	$mailmessage = array($msg,
			$killedloc, $user->name,
			$user->weapon, $badguy['playerstarthp'],
			$user->hitpoints, getsetting("pvpdeflose", 5),
	...
	$sql = "UPDATE " . DB::prefix("accounts") . " SET alive=0, goldinbank=(goldinbank+IF(gold<{$badguy['creaturegold']},gold-{$badguy['creaturegold']},0)),gold=IF(gold<{$badguy['creaturegold']},0,gold-{$badguy['creaturegold']}), experience=experience-$lostexp WHERE acctid=".(int)$badguy['acctid']."";
	DB::query($sql);
	return $args['handled'];
}
...
function pvpdefeat($badguy, $killedloc, $taunt, $options)
{
	...
	// their advantage, sometimes against.  The basic idea is to prevent
	// exhorbitant amounts of money from being transferred this way.
	$winamount = round(10 * $user->level *
			log(max(1,$user->gold)),0);
	if ($badguy['creaturelevel'] == Settings::getsetting('maxlevel', 15))	$winamount = 0;

	$sql = "SELECT level FROM " . DB::prefix("accounts") . " WHERE acctid={$badguy['acctid']}";
	$result = DB::query($sql);
	$row = DB::fetch_assoc($result);

	$wonexp = round($user->experience*getsetting("pvpdefgain",10)/100,0);
	if ($badguy['creaturelevel'] == Settings::getsetting('maxlevel', 15))	$wonexp = 0;

	$lostexp = round($user->experience * getsetting("pvpattlose",15) / 100,0);
	...
	} elseif ($badguy['creaturelevel'] == Settings::getsetting('maxlevel', 15)) {
	...
	systemmail($badguy['acctid'],
			...
			array($msg, $user->name, $killedloc, $wonexp,
	...
	$user->alive=false;
	//debuglog("lost {$user->gold} ($winamount to winner) gold and $lostexp exp ($wonexp to winner) being slain by ", $badguy['acctid']);
	
	debuglog("started the fight and has been defeated by {$badguy['creaturename']} in $killedloc (lost {$user->gold} gold and $lostexp exp, victim tooks $winamount gold and $wonexp exp)",false,$user->acctid);
	debuglog("was the victim and won aginst {$user->name} in $killedloc (earned $winamount gold and $wonexp exp)",false,$badguy['acctid']);	
