...
function translate($indata,$namespace=FALSE){
	...
	global $translation_table,$translation_namespace;
	...
	if ($namespace != "notranslate") {
		if (!isset($translation_table[$namespace]) ||
			...
			$translation_table[$namespace] =
				translate_loadnamespace($namespace,Session::get('tlanguage') or false);
		}
	}
...
function translate_mail($in,$to=0){
	...
	if ($to>0){
		...
		Session::set('tlanguage', $language['prefs']['language']?$language['prefs']['language']:getsetting("defaultlanguage","en"));
	}
	reset($in);
...
function tlbutton_push($indata,$hot=false,$namespace=FALSE){
	...
	global $translation_is_enabled,$seentlbuttons,$user;
	if (!$translation_is_enabled) return false;
	if (!$namespace) $namespace="unknown";
	if ($user->superuser & SU_IS_TRANSLATOR){
		if (preg_replace("/[ 	\n\r]|`./",'',$indata)>""){
