...
function page_header(){
	global $header,$SCRIPT_NAME,$template, $runheaders, $nopopups, $user;
	...
	if (!$arguments || count($arguments) == 0) {
		$arguments = array(Settings::getsetting('gtitle', 'Legend of the Green Dragon'));
	}
...
function page_footer($saveuser=true){
	global $output,$header,$nav,
		$pagestarttime,$quickkeys,$template,$y2,$z2,
		$logd_version,$copyright,$SCRIPT_NAME,$nopopups, $footer,$user;
	...
	if (!isset($_SESSION['logdnet']) || !isset($_SESSION['logdnet']['']) || $_SESSION['logdnet']['']=="" || !isset($user->laston) || date("Y-m-d H:i:s",strtotime("-1 hour"))>$user->laston){
		$already_registered_logdnet = false;
	}else{
		$already_registered_logdnet = true;
	}
	if (getsetting("logdnet",0) && $user->loggedin && !$already_registered_logdnet){
		//account counting, just for my own records, I don't use this in the calculation for server order.
		$sql = "SELECT count(*) AS c FROM " . DB::prefix("accounts");
...
<input type="hidden" name="item_name" value="Legend of the Green Dragon Author Donation from '.full_sanitize($user->name).'">
<input type="hidden" name="item_number" value="'.htmlentities($user->login, ENT_COMPAT, getsetting("charset", "ISO-8859-1")).":".$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI'].'">
...
<input type="image" src="images/paypal2.gif" border="0" name="submit" alt="Donate!">
	...
	if (isset($user->acctid) && $user->acctid>0 && $user->loggedin) {
	...
	if ($user->superuser & SU_EDIT_PETITIONS){
	...
	//Failsave: without this the error class reports an object error
	if (!isset($user->gentime)) $user->gentime = 0;
	
	$user->gentime+=$gentime;
	$user->gentimecount++;
	if ($user->gentimecount==0) $roundedtime=0;
	else $roundedtime=round($user->gentime/$user->gentimecount,3);
	$footer=str_replace("{pagegen}","Page gen: ".round($gentime,3)."s / ".$dbinfo['queriesthishit']." queries (".round($dbinfo['querytime'],3)."s), Ave: ".$roundedtime."s - ".round($user->gentime,3)."/".round($user->gentimecount,3)."",$footer);
...
function popup_footer(){
	global $output,$header,$y2,$z2,$copyright, $template;
 ...
 */
function charstats(){
	global $user, $playermount, $companions;

	wipe_charstats();

	if (Session::get('loggedin')){
		$user->hitpoints=round($user->hitpoints,0);
		$user->experience=round($user->experience, 0);
		$user->maxhitpoints=round($user->maxhitpoints,0);
		$spirits=array(-6=>"Resurrected",-2=>"Very Low",-1=>"Low","0"=>"Normal",1=>"High",2=>"Very High");
		if ($user->alive){ }else{ $spirits[(int)$user->spirits] = "DEAD"; }
		//calculate_buff_fields();
		$atk=$user->attack;
		$def=$user->defense;
		$buffcount = 0;
		$buffs = "";
		foreach (Session::get('bufflist') as $key=>$val) {
			...
			if ($val['name'] > "" || $user->superuser & SU_DEBUG_OUTPUT){
				tlschema($val['schema']);
				if ($val['name']=="")
					$val['name'] = "DEBUG: {$key}";
		...
		$def = round($def, 2);
		if ($atk < $user->attack){
			$atk = round($user->attack,1)."`\$".round($atk-$user->attack,1);
		}else if($atk > $user->attack){
			$atk = round($user->attack,1)."`@+".round($atk-$user->attack,1);
		} else {
		...
		if ($def < $user->defense){
			$def = round($user->defense,1)."`\$".round($def-$user->defense,1);
		}else if($def > $user->defense){
			$def = round($user->defense,1)."`@+".round($def-$user->defense,1);
		} else {
			// They are equal, display in the 1 signifigant digit format.
			$def = round($def, 1);
		...
		addcharstat("Name", $user->name);
		addcharstat("Level", "`b".$user->level.check_temp_stat("level",1)."`b");
		if ($user->alive) {
			addcharstat("Hitpoints", $user->hitpoints.check_temp_stat("hitpoints",1).
									"`0/".$user->maxhitpoints.check_temp_stat("maxhitpoints",1));
			addcharstat("Turns", $user->turns.check_temp_stat("turns",1));
			addcharstat("Attack", $atk.check_temp_stat("attack",1));
			addcharstat("Defense", $def.check_temp_stat("defense",1));
		} else {
			$maxsoul = $user->level * 5 + 50;
			addcharstat("Soulpoints", $user->soulpoints.check_temp_stat("soulpoints",1)."`0/".$maxsoul);
			addcharstat("Torments", $user->gravefights.check_temp_stat("gravefights",1));
			addcharstat("Psyche", 10+round(($user->level-1)*1.5));
			addcharstat("Spirit", 10+round(($user->level-1)*1.5));
		}
		addcharstat("Spirits", translate_inline("`b".$spirits[(int)$user->spirits]."`b"));
		if ($user->race != RACE_UNKNOWN) {
			addcharstat("Race", translate_inline($user->race,"race"));
		}else {
		...
		if ($user->alive) {
			addcharstat("Gold", number_format($user->gold).check_temp_stat("gold",1));
		} else {
			addcharstat("Favor", $user->deathpower.check_temp_stat("deathpower",1));
		}
		addcharstat("Gems", number_format($user->gems).check_temp_stat("gems",1));
		addcharstat("Experience", number_format($user->experience).check_temp_stat("experience",1));
		addcharstat("Equipment Info");
		addcharstat("Weapon", $user->weapon);
		addcharstat("Armor", $user->armor);
		if ($user->hashorse)
		...
		if (!is_array(Session::get('bufflist'))) Session::set('bufflist', array());
		return $charstat;
	}else{
		$ret = "";
...
function loadtemplate($templatename){
	...
	if (!($template = datacache('parsed-template-' . $templatename, 86400))) {
		...
		updatedatacache('parsed-template' . $templatename, $template);
	}
	return $template;
}
...
function maillink(){
	global $user;
	$sql = "SELECT sum(if(seen=1,1,0)) AS seencount, sum(if(seen=0,1,0)) AS notseen FROM " . DB::prefix("mail") . " WHERE msgto=\"".$user->acctid."\"";
	$result = DB::query_cached($sql,"mail-{$user->acctid}",86400);
	$row = DB::fetch_assoc($result);
	DB::free_result($result);
