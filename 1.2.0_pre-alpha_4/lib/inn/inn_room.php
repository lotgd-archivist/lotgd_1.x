<?php
$config = unserialize($user->donationconfig);
$expense = round(($user->level*(10+log($user->level))),0);
$pay = httpget('pay');
if ($pay){
	if ($pay == 2 || $user->gold>=$expense ||
			$user->boughtroomtoday){
		if ($user->loggedin){
			if (!$user->boughtroomtoday) {
				if ($pay == 2) {
					...
					if (strpos($fee, "%"))
						...
						$expense += $fee;
					$user->goldinbank-=$expense;
				} else {
					$user->gold-=$expense;
				}
				$user->boughtroomtoday=1;
	...
	if ($user->boughtroomtoday){
		...
		if ($user->goldinbank >= $bankexpense && $bankexpense != $expense) {
		...
		if ($user->goldinbank >= $bankexpense) {
