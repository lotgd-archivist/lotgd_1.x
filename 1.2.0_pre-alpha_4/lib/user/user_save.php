	...
	if (isset($userinfo[$key])){
		if ($key=="newpassword" ){
			if ($val>"") {
				...
				debuglog($user->name."`0 changed password to $val",$userid);
				if ($user->acctid==$userid) {
					$user->password=md5(md5($val));
				}
			}
			...
			$stripfield = ((int)$oldvalues['superuser'] | $user->superuser | SU_ANYONE_CAN_SET | ($user->superuser & SU_MEGAUSER ? 0xFFFFFFFF : 0));
			$value = $value & $stripfield;
				//put back on privs that the user used to have but the
			//current user can't set.
			$unremovable = ~ ((int)$user->superuser | SU_ANYONE_CAN_SET | ($user->superuser & SU_MEGAUSER ? 0xFFFFFFFF : 0));
			...
			if ((int)$value != (int)$oldvalues['superuser']){
				...
				debuglog($user->name."`0 changed superuser to ".show_bitfield($value),$userid);
			...
			debuglog($user->name . "`0 changed player name to $newname`0", $userid);
			...
			if ($newname != $oldvalues['name']) {
				...
				debuglog($user->name . "`0 changed player name to $newname`0 due to changed dragonkill title", $userid);
				$oldvalues['name']=$newname;
				if ($user->acctid==$userid) {
					$user->name = $newname;
				}
			...
			if ($newname != $oldvalues['name']) {
				...
				debuglog($user->name . "`0 changed player name to $newname`0 due to changed custom title", $userid);
				$oldvalues['name']=$newname;
				if ($user->acctid==$userid) {
					$user->name = $newname;
				}
			...
			debuglog($user->name."`0 changed $key to $val",$userid);
			if ($user->acctid==$userid) {
