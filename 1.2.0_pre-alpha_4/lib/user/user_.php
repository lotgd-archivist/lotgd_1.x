...
if ($display == 1){
	...
	$rn=0;
	$oorder = "";
	$number3=DB::num_rows($searchresult);
	for ($i=0;$i<$number3;$i++){
		$row=DB::fetch_assoc($searchresult);
		...
		rawoutput("<td nowrap>");
		rawoutput("[ <a href='user.php?op=edit&userid={$row['acctid']}$m'>$ed</a> | ");
		if($user->superuser & SU_MEGAUSER) {
			rawoutput("<a href='user.php?op=del&userid={$row['acctid']}' onClick=\"return confirm('$conf');\">$del</a> | ");
			addnav("","user.php?op=del&userid={$row['acctid']}");
		}
		rawoutput("<a href='user.php?op=setupban&userid={$row['acctid']}'>$ban</a> | <a href='user.php?op=debuglog&userid={$row['acctid']}'>$log</a> ]");
