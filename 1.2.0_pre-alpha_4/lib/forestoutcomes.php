...
function forestvictory($enemies,$denyflawless=false){
	...
	foreach ($enemies as $badguy) {
		...
		tlschema("battle");
		if(isset($badguy['creaturelose'])) $msg = substitute(translate_inline($badguy['creaturelose']));
		tlschema();
		...
		$expbonus += round(($badguy['creatureexp'] * (1 + .25 * ($badguy['creaturelevel']-$user->level))) - $badguy['creatureexp'],0);
	}
	$multibonus = $count>1?1:0;
	$expbonus += $user->dragonkills * $user->level * $multibonus;
	$totalexp = 0;
	...
	if ($user->level < Settings::getsetting('maxlevel', 15) && e_rand(1,$gemchances) == 1) {
	...
	if (!$diddamage) {
		...
		}elseif ($user->level<=$creaturelevel){
...
function forestdefeat($enemies,$where="in the forest"){
	...
	foreach ($enemies as $badguy) {
		...
		if (isset($badguy['creaturewin']) && $badguy['creaturewin'] > "") {
			$msg = substitute(translate_inline($badguy['creaturewin'],"battle"));
	...
	addnews("`%%s`5 has been slain %s by %s.`n%s",$user->name,$where,$badguy['creaturename'],$taunt);
	$user->alive=false;
	debuglog("lost gold when they were slain $where",false,false,"forestlose",-$user->gold);
	$user->gold=0;
	$user->hitpoints=0;
	$user->experience=round($user->experience*(1-($percent/100)),0);
