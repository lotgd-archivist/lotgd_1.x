...
function injectmodule($modulename,$force=false){
	...
	if (file_exists($modulefilename)){
		...
		$result = DB::query_cached($sql, "inject-$modulename", 3600);
		if (!$force) {
	...
	if (file_exists($modulefilename)) {
		...
		if (DB::num_rows($result) > 0) {
			...
			if ($row['active']) {
				...
				if (isset($injected_modules[0][$modulename]) &&
						$injected_modules[0][$modulename])
					$status |= MODULE_INJECTED;
				if (isset($injected_modules[1][$modulename]) &&
...
function blockmodule($modulename) {
	global $blocked_modules, $block_all_modules;
...
function modulehook($hookname, $args=false, $allowinactive=false, $only=false){
	...
	global $output, $user, $modulehook_queries;
	global $currenthook;
	...
	if ($user->superuser & SU_DEBUG_OUTPUT && !isset($hookcomment[$hookname])){
	...
	if (isset($modulehook_queries[$hookname]) //This data was pre fetched in mass_module_prepare
		...
		$sql =
			...
			ORDER BY
				...
				" . DB::prefix("module_hooks") . ".modulename";
		$result = DB::query_cached($sql,"hook-".$hookname);
	}
	...
	while ($row = DB::fetch_assoc($result)){
		...
		if (!isset($blocked_modules[$row['modulename']])){
			$blocked_modules[$row['modulename']] = false;
		}
		if (!isset($unblocked_modules[$row['modulename']])){
		...
		if (injectmodule($row['modulename'], $allowinactive)) {
			...
			if (!isset($row['whenactive'])) $row['whenactive'] = '';
...
function load_module_settings($module){
	...
	if (!isset($module_settings[$module])){
		...
		$result = DB::query_cached($sql,"modulesettings-$module");
		while ($row = DB::fetch_assoc($result)){
...
function get_module_objpref($type, $objid, $name, $module=false){
	...
	$result = DB::query_cached($sql, "objpref-$type-$objid-$name-$module", 86400);
	if (DB::num_rows($result)>0){
		$row = DB::fetch_assoc($result);
		return $row['value'];
	}
...
function module_delete_userprefs($userid){
	$sql = "DELETE FROM " . DB::prefix("module_userprefs") . " WHERE userid='$userid'";
	DB::query($sql);
}

$module_prefs=array();
function get_all_module_prefs($module=false,$userid=false){
	global $module_prefs,$mostrecentmodule,$user;
	if ($module === false) $module = $mostrecentmodule;
	if ($userid === false) $userid = $user->acctid;
	load_module_prefs($module,$userid);

	return $module_prefs[$userid][$module];
}

function get_module_pref($name,$module=false,$userid=false){
	global $module_prefs,$mostrecentmodule,$user;
	if ($module === false) $module = $mostrecentmodule;
	if ($userid===false) {
		if(isset($user->loggedin) && $user->loggedin) $userid = $user->acctid;
		else $userid = 0;
	}

	if (isset($module_prefs[$userid][$module][$name])) {
		return $module_prefs[$userid][$module][$name];
	}
	...
	if (isset($info['prefs'][$name])){
		...
		if (isset($x[1])){
			set_module_pref($name,$x[1],$module,$userid);
			return $x[1];
		}
	}
	return NULL;
...
function set_module_pref($name,$value,$module=false,$userid=false){
	global $module_prefs,$mostrecentmodule,$user;
	if ($module === false) $module = $mostrecentmodule;
	if ($userid === false) $uid=$user->acctid;
	else $uid = $userid;
...
function increment_module_pref($name,$value=1,$module=false,$userid=false){
	global $module_prefs,$mostrecentmodule,$user;
	$value = (float)$value;
	if ($module === false) $module = $mostrecentmodule;
	if ($userid === false) $uid=$user->acctid;
...
function clear_module_pref($name,$module=false,$userid=false){
 	global $module_prefs,$mostrecentmodule,$user;
	if ($module === false) $module = $mostrecentmodule;
	if ($userid === false) $uid=$user->acctid;
	else $uid = $userid;
...
function load_module_prefs($module, $userid=false){
	global $module_prefs,$user;
	if ($userid===false) $userid = $user->acctid;
	if (!isset($module_prefs[$userid])) $module_prefs[$userid] = array();
	if (!isset($module_prefs[$userid][$module])){
		$module_prefs[$userid][$module] = array();
		$sql = "SELECT setting,value FROM " . DB::prefix("module_userprefs") . " WHERE modulename='$module' AND userid='$userid'";
		$result = DB::query($sql);
		while ($row = DB::fetch_assoc($result)){
			$module_prefs[$userid][$module][$row['setting']] = $row['value'];
		}//end while
	}//end if
}//end function
...
function module_collect_events($type, $allowinactive=false)
{
	...
	$result = DB::query_cached($sql,"event-".$type);
	while ($row = DB::fetch_assoc($result)){
		...
		if (($block_all_modules || isset($blocked_modules[$row['modulename']]) && $blocked_modules[$row['modulename']]) &&
				(!isset($unblocked_modules[$row['modulename']]) || !$unblocked_modules[$row['modulename']])) {
			$chance = 0;
		}
...
function module_display_events($eventtype, $forcescript=false) {
	...
	if (!($user->superuser & SU_DEVELOPER)) return;
...
function get_module_install_status(){
	...
	if ($result !== false){
		while ($row = DB::fetch_assoc($result)) {
			...
			if (!isset($seencats[$row['category']]))
...
function get_racename($thisuser=true) {
	if ($thisuser === true) {
		global $user;
		return translate_inline($user->race,"race");
	} else {
