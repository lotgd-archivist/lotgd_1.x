...
function rolldamage(){
	global $badguy,$user,$creatureattack,$creatureatkmod,$adjustment;
	global $creaturedefmod,$defmod,$atkmod,$buffset,$atk,$options;
	...
	if ($badguy['creaturehealth']>0 && $user->hitpoints>0){
		...
		$adjustedselfdefense = ($user->defense * $adjustment * $defmod);
...
function suspend_buff_by_name($name, $msg=false) {
	...
	$buffs = Session::get('bufflist');
	if ($buffs[$name] &&
			!$buffs[$name]['suspended']) {
		$buffs[$name]['suspended'] = 1;
		Session::set('bufflist', $buffs);
		// And notify.
		$schema = false;
		if ($msg === false) {
...
function is_buff_active($name) {
	// If it's not already suspended.
	$buffs = Session::get('bufflist');
	return (($buffs[$name] && !$buffs[$name]['suspended'])?1:0);
}
...
function unsuspend_buffs($susp=false,$msg=false) {
	...
	$buffs = Session::get('bufflist');
	foreach ($buffs as $key=>$buff) {
		if (isset($buff['expireafterfight']) && $buff['expireafterfight']) unset($buffs[$key]);
		elseif (isset($buff['suspended']) && $buff['suspended'] && $susp && (!isset($buff[$susp]) || !$buff[$susp])) {
			$buffs[$key]['suspended'] = 0;
			$unsuspendnotify=1;
		}
	}
	Session::set('bufflist', $buffs);
...
function apply_bodyguard($level){
	if (!Session::getNested('bufflist', 'bodyguard')) {
		switch($level){
		case 1:
			$badguyatkmod=1.05;
			$defmod=0.95;
