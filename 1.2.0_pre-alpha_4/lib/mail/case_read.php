...
$sql = "SELECT $mail.*, $accounts.name FROM $mail LEFT JOIN $accounts ON $accounts.acctid=$mail.msgfrom WHERE msgto=\"".$user->acctid."\" AND messageid=\"".$id."\"";
$result = DB::query($sql);
if (DB::num_rows($result)>0){
	...
	$sql = "UPDATE " . DB::prefix("mail") . " SET seen=1 WHERE  msgto=\"".$user->acctid."\" AND messageid=\"".$id."\"";
	DB::query($sql);
	...
	rawoutput("</tr><tr>");
	$sql = "SELECT messageid FROM $mail WHERE msgto='{$user->acctid}' AND messageid < '$id' ORDER BY messageid DESC LIMIT 1";
	$result = DB::query($sql);
	if (DB::num_rows($result)>0){
		$row = DB::fetch_assoc($result);
		$pid = $row['messageid'];
	...
	$sql = "SELECT messageid FROM $mail WHERE msgto='{$user->acctid}' AND messageid > '$id' ORDER BY messageid  LIMIT 1";
	$result = DB::query($sql);
	if (DB::num_rows($result)>0){
		$row = DB::fetch_assoc($result);
		$nid = $row['messageid'];
