...
if (httppostisset("DB_HOST")) {
	Session::setNested('dbinfo', 'DB_HOST', httppost('DB_HOST'));
	Session::setNested('dbinfo', 'DB_USER', httppost('DB_USER'));
	Session::setNested('dbinfo', 'DB_PASS', httppost('DB_PASS'));
	Session::setNested('dbinfo', 'DB_NAME', httppost('DB_NAME'));
	Session::setNested('dbinfo', 'DB_TYPE', httppost('DB_TYPE'));
	Session::setNested('dbinfo', 'DB_USEDATACACHE', (bool)httppost('DB_USEDATACACHE'));
	Session::setNested('dbinfo', 'DB_DATACACHEPATH', httppost('DB_DATACACHEPATH'));
}
output("`@`c`bTesting the Database Connection`b`c`2");
output("Trying to establish a connection with the database:`n");
ob_start();
DB::setHost(Session::getNested('dbinfo', 'DB_HOST'));
DB::setUser(Session::getNested('dbinfo', 'DB_USER'));
DB::setPass(Session::getNested('dbinfo', 'DB_PASS'));
DB::setType(Session::getNested('dbinfo', 'DB_TYPE'));
$link = DB::connect();
...
if (!$link){
	...
	if (httpget("op")=="trycreate"){
		...
		create_db(Session::getNested('dbinfo', 'DB_NAME'));
	}
	if (!DB::select_db(Session::getNested('dbinfo', 'DB_NAME'))){
		$error = DB::error($link);
		output("`\$Rats!  I was not able to connect to the database.");
		if ($error=="Unknown database '" . Session::getNested('dbinfo', 'DB_NAME') . "'"){
		...
		if (!Session::getNested('dbinfo', 'DB_USEDATACACHE')) {
			output("-----skipping, not selected-----`n");
		} else {
			$fp = file_put_contents(Session::getNested('dbinfo', 'DB_DATACACHEPATH')."/dummy.php", $dbconnect);
			if ($fp){
				output("`2Result: `@Pass`n");
				@unlink(Session::getNested('dbinfo', 'DB_DATACACHEPATH')."/dummy.php");
			}else{
				output("`2Result: `\$Fail`n");
