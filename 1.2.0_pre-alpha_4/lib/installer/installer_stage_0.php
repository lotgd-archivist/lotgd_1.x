...
if (DB_CHOSEN){
	...
	if (httppost("username")>""){
		...
		if ($version == "-1") {
			...
			$sql = "SELECT * FROM ".DB::prefix("accounts")." WHERE login='".DB::escape_string(httppost("username"))."' AND password='".mysql_real_escape_string(httppost("password"))."' AND superuser & ".SU_MEGAUSER;
		}else {
			$password = stripslashes(httppost("password"));
			if (substr($password, 0, 5) == "!md5!") {
				$password = md5(substr($password, 5));
			} else {
				$password = md5(md5($password));
			}
			$sql = "SELECT * FROM ".DB::prefix("accounts")." WHERE login='".DB::escape_string(httppost("username"))."' AND password='".$password."' AND superuser & ".SU_MEGAUSER;
		}
		$result = DB::query($sql);
...
if ($user->superuser & SU_MEGAUSER) $needsauthentication=false;
