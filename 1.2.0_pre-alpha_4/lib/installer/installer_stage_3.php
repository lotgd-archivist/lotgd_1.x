...
if (file_exists("dbconnect.php")){
	...
	if(count($wrappers) == 0) {
		output("`nThere doesn't appear to be a valid wrapper that you can use. The game is unable to install.");
		Session::set('stagecompleted', Session::get('stagecompleted') - 1);
	} else {
		...
		rawoutput("<input name='DB_HOST' value=\"".htmlentities(Session::getNested('dbinfo', 'DB_HOST'), ENT_COMPAT, getsetting("charset", "ISO-8859-1"))."\">");
		...
		rawoutput("<input name='DB_USER' value=\"".htmlentities(Session::getNested('dbinfo', 'DB_USER'), ENT_COMPAT, getsetting("charset", "ISO-8859-1"))."\">");
		...
		rawoutput("<input name='DB_PASS' value=\"".htmlentities(Session::getNested('dbinfo', 'DB_PASS'), ENT_COMPAT, getsetting("charset", "ISO-8859-1"))."\">");
		...
		rawoutput("<input name='DB_NAME' value=\"".htmlentities(Session::getNested('dbinfo', 'DB_NAME'), ENT_COMPAT, getsetting("charset", "ISO-8859-1"))."\">");
		...
		output("`nWhat method do you want to connect to the database with?`n");
		rawoutput("<select name='DB_TYPE'>");
		foreach ($wrappers as $type=>$desc) {
			rawoutput("<option value='$type'>$desc</option>");
		}
		rawoutput("</select>");
		tip("These lets you choose which database type and group of functions is used for the game.");

		output("`nDo you want to use datacaching (high load optimization)?`n");
		rawoutput("<select name='DB_USEDATACACHE'>");
		rawoutput("<option value=\"1\" ".(Session::getNested('dbinfo', 'DB_USEDATACACHE')?'selected=\"selected\"':'').">".translate_inline("Yes")."</option>");
		rawoutput("<option value=\"0\" ".(!Session::getNested('dbinfo', 'DB_USEDATACACHE')?'selected=\"selected\"':'').">".translate_inline("No")."</option>");
		rawoutput("</select>");
		...
		rawoutput("<input name='DB_DATACACHEPATH' value=\"".htmlentities(Session::getNested('dbinfo', 'DB_DATACACHEPATH'), ENT_COMPAT, getsetting("charset", "ISO-8859-1"))."\">");
