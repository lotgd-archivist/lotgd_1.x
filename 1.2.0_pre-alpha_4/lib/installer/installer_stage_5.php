...
Session::setNested('dbinfo', 'DB_PREFIX', httppost('DB_PREFIX'));
if (Session::getNested('dbinfo', 'DB_PREFIX') > "" && substr(Session::getNested('dbinfo', 'DB_PREFIX'),-1)!="_")
Session::setNested('dbinfo', 'DB_PREFIX', Session::getNested('dbinfo', 'DB_PREFIX') . "_");

$descriptors = descriptors(Session::getNested('dbinfo', 'DB_PREFIX'));
$unique=0;
$game=0;
$missing=0;
$conflict = array();
...
DB::setName(Session::getNested('dbinfo', 'DB_NAME'));
...
Session::setNested('dbinfo', 'upgrade', $upgrade);
if ($upgrade){
	...
	if (count($conflict)>0){
		...
		if (httpget("op")=="confirm_overwrite") Session::set('sure i want to overwrite the tables', true);
		if (!Session::get('sure i want to overwrite the tables')){
			Session::set('stagecompleted', 4);
...
rawoutput("<input name='DB_PREFIX' value=\"".htmlentities(Session::getNested('dbinfo', 'DB_PREFIX'), ENT_COMPAT, getsetting("charset", "ISO-8859-1"))."\"><br>");
