...
if (DB::num_rows($result)>0){
	...
	if ($row['hauntedby']!=""){
		...
		$roll2 = e_rand(0,$user->level);
		if ($roll2>$roll1){
			output("You have successfully haunted `7%s`)!", $row['name']);
			$sql = "UPDATE " . DB::prefix("accounts") . " SET hauntedby='".addslashes($user->name)."' WHERE login='$name'";
			DB::query($sql);
			addnews("`7%s`) haunted `7%s`)!",$user->name,$row['name']);
			$subj = array("`)You have been haunted");
			$body = array("`)You have been haunted by `&%s`).",$user->name);
			require("lib/systemmail.php");
			systemmail($row['acctid'], $subj, $body);
		}else{
			addnews("`7%s`) unsuccessfully haunted `7%s`)!",$user->name,$row['name']);
			switch (e_rand(0,5)){
			case 0:
