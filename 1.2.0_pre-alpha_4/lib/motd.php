...
function pollitem($id,$subject,$body,$author,$date,$showpoll=true){
	global $user;
	$sql = "SELECT count(resultid) AS c, MAX(choice) AS choice FROM " . DB::prefix("pollresults") . " WHERE motditem='$id' AND account='{$user->acctid}'";
	$result = DB::query($sql);
	...
	if ($subject=="" || $body=="" || $preview>""){
		...
		if (!$id || DB::affected_rows()==0){
			...
			if ($doinsert){
				...
				$sql = "INSERT INTO " . DB::prefix("motd") . " (motdtitle,motdbody,motddate,motdauthor) VALUES (\"$subject\",\"$body\",'".date("Y-m-d H:i:s")."','{$user->acctid}')";
				DB::query($sql);
				invalidatedatacache("motd");
				invalidatedatacache("lastmotd");
				invalidatedatacache("motddate");
...
function motd_poll_form() {
	...
	if ($subject=="" || $body==""){
		...
		$sql = "INSERT INTO " . DB::prefix("motd") . " (motdtitle,motdbody,motddate,motdtype,motdauthor) VALUES (\"$subject\",\"".addslashes(serialize($body))."\",'".date("Y-m-d H:i:s")."',1,'{$user->acctid}')";
		DB::query($sql);
		invalidatedatacache("motd");
		invalidatedatacache("lastmotd");
		invalidatedatacache("motddate");
