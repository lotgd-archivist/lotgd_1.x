...
//		);
		...
		addnews("`&%s was smitten down for attempting to defile the gods (they tried to hack superuser pages).",$user->name);
		debuglog("Lost {$user->gold} and ".($user->experience*0.25)." experience trying to hack superuser pages.");
		$user->hitpoints=0;
		$user->alive=0;
		$user->soulpoints=0;
		$user->gravefights=0;
		...
		while ($row = DB::fetch_assoc($result)) {
			...
			$subj = sprintf($subj, $user->name);
			$body = "Bad, bad, bad %s, they are a hacker!`n`nTried to access %s from %s.";
			$body = sprintf($body, $user->name, $_SERVER['REQUEST_URI'], $_SERVER['HTTP_REFERER']);
