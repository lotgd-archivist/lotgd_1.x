...
if ($op=="pay"){
	if ($user->gold>=$cost){ // Gunnar Kreitz
		$user->gold-=$cost;
		...
		villagenav();
		output("`5You offer the old gypsy woman your `^%s`5 gold for your gen-u-wine say-ance, however she informs you that the dead may be dead, but they ain't cheap.", $user->gold);
	}
}elseif ($op=="talk"){
	...
	output("\"`!For you, %s, the price is a trifling `^%s`! gold.`5\", she rasps.", translate_inline($user->sex?"my pretty":"my handsome"), $cost);
	addnav("Seance");
