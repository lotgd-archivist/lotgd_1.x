...
case "run":
	if (e_rand(0,2)==1) {
		...
		$favor = 5 + e_rand(0, $user->level);
		if ($favor > $user->deathpower)
			$favor = $user->deathpower;
		if ($favor > 0) {
			output("`)You have `\$LOST `^%s`) favor with `\$%s`).",$favor,$deathoverlord);
			$user->deathpower-=$favor;
		}
		tlschema("nav");
...
if ($battle){
	...
	//with the undead guy.
	$originalhitpoints = $user->hitpoints;
	$user->hitpoints = $user->soulpoints;
	$originalattack = $user->attack;
	$originaldefense = $user->defense;
	$user->attack =
		10 + round(($user->level - 1) * 1.5);
	$user->defense =
		10 + round(($user->level - 1) * 1.5);
	...
	$user->attack = $originalattack;
	$user->defense = $originaldefense;
	$user->soulpoints = $user->hitpoints;
	$user->hitpoints = $originalhitpoints;
	...
	if ($victory) {
		...
		$user->deathpower+=$badguy['creatureexp'];
		$op = "";
		httpset('op', "");
		...
		if ($defeat){
			...
			addnews("`)%s`) has been defeated in the graveyard by %s.`n%s",$user->name,$badguy['creaturename'],$taunt);
