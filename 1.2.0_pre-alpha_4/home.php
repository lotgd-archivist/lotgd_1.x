...
// Die if no PHP 5. Only done here and installer.
if(version_compare(PHP_VERSION, '5.0.0') < 0) {
	die('PHP 5 is required. You are running ' . PHP_VERSION . '. Please upgrade.');
}
...
if (!Session::exists('loggedin')) Session::set('loggedin', false);
...
page_header();
output('`cWelcome to ' . Settings::getsetting('gtitle', 'Legend of the Green Dragon') . ', a browser based role playing game, based on Seth Able\'s Legend of the Red Dragon.`n');
...
if ($onlinecount<getsetting("maxonline",0) || getsetting("maxonline",0)==0){
	...
	if ($op=="timeout"){
		Session::set('message', Session::get('message') . translate_inline(" Your session has timed out, you must log in again.`n"));
	}
	if (!isset($_COOKIE['lgi'])){
		Session::set('message', Session::get('message') . translate_inline("It appears that you may be blocking cookies from this site.  At least session cookies must be enabled in order to use this site.`n")
		. translate_inline("`b`#If you are not sure what cookies are, please <a href='http://en.wikipedia.org/wiki/WWW_browser_cookie'>read this article</a> about them, and how to enable them.`b`n"));
	}
	if (Session::get('message')) {
		output_notl("`b`\$%s`b`n", Session::get('message'),true);
	}
	...
	if (Session::get('message')) output("`b`\$%s`b`n", Session::get('message'),true);
