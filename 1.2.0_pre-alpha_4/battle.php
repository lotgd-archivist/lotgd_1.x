...
global $badguy,$enemies,$newenemies,$creatureattack,$creatureatkmod, $beta;
...
$newcompanions = array();
$attackstack = @unserialize($user->badguy);
...
if ($enemycounter > 0) {
	...
	foreach ($enemies as $index=>$badguy) {
		if ($badguy['creaturehealth']>0 && $user->hitpoints>0) {
...
if ($op != "run" && $op != "fight" && $op != "newtarget") {
	if (count($enemies) > 1) {
		...
		if (!isset($options['didsurprise']) || !$options['didsurprise']) {
...
if ($op != "newtarget") {
	...
	do {
			...
			if ($badguy['dead'] == false && $badguy['creaturehealth'] > 0) {
				...
				if (($roundcounter > $options['maxattacks']) && $badguy['istarget'] == false) {
					...
					if ($badguy['creaturehealth']<=0 || $user->hitpoints<=0){
						...
						if ($badguy['creaturehealth']>0 && $user->hitpoints>0 && $badguy['istarget']){
						...
						if ($op=="fight" || $op=="run" || $surprised){
							...
							if ($op=="fight" && !$surprised){
								$ggchancetodouble = $user->dragonkills;
								$bgchancetodouble = $user->dragonkills;
								...
								if ($badguy['creaturehealth']>0 && $user->hitpoints>0) {
									...
									if ($badguy['creaturehealth']<=0 || $user->hitpoints<=0){
										...
										do {
											...
											$r = mt_rand(0,100);
											if ($r < $ggchancetodouble && $badguy['creaturehealth']>0 && $user->hitpoints>0 && !$needtostopfighting){
							...
							//-- Gunnar Kreitz
							if ($badguy['creaturehealth']>0 && $user->hitpoints>0 && $roundcounter <= $options['maxattacks']){
								...
								do {
									...
									if ($r < $bgchancetodouble && $badguy['creaturehealth']>0 && $user->hitpoints>0 && !$needtostopfighting){
										$additionalattack = true;
						...
						if($badguy['dead'] == false && isset($badguy['creatureaiscriptid']) && $badguy['creatureaiscriptid']) {
							global $unsetme;
							execute_ai_script((int)$badguy['creatureaiscriptid']);
						}else if($badguy['dead'] == false && isset($badguy['creatureaiscript']) && $badguy['creatureaiscript']) {
							global $unsetme;
		...
		if (($count != 1 || ($needtostopfighting && $count > 1)) && $user->hitpoints > 0 && count($enemies) > 0) {
		...
		if (count($newenemies) > 0) {
			...
			foreach ($newenemies as $index => $badguy) {
				if ($badguy['dead'] == true || $badguy['creaturehealth'] <= 0){
					...
					if (getsetting("instantexp",false) == true && $user->alive && $options['type'] != "pvp" && $options['type'] != "train") {
						if (!isset($badguy['expgained']) || $badguy['expgained'] == false) {
							if (!isset($badguy['creatureexp'])) $badguy['creatureexp'] = 0;
							$user->experience += round($badguy['creatureexp']/count($newenemies));
			...
			$enemiesflown = false;
			if ($alive == $fleeable && $user->hitpoints > 0) {
				$defeat=false;
				$victory=true;
				$enemiesflown=true;
				$needtostopfighting=true;
...
if ($user->hitpoints>0 && count($newenemies)>0 && ($op=="fight" || $op=="run")){
	output("`2`bEnd of Round:`b`n");
	show_enemies($newenemies);
}

if ($user->hitpoints < 0) $user->hitpoints = 0;
...
function battle_player_attacks() {
	global $badguy, $user;
	global $buffset,$atk,$options;
	global $roll,$count,$needtostopfighting;
