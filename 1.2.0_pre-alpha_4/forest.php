...
if ($op=="search"){
	...
	if ($user->turns<=0){
		...
		if (module_events("forest", getsetting("forestchance", 15)) != 0) {
			...
			$multi = 1;
			$targetlevel = ($user->level + $plev - $nlev );
			$mintargetlevel = $targetlevel;
			if (getsetting("multifightdk", 10) <= $user->dragonkills) {
			...
			if ($targetlevel>$settings->maxlevel + 2) {
				$multi += $targetlevel - ($settings->maxlevel + 2);
				$targetlevel=$settings->maxlevel + 2;
			}
			...
			if (DB::num_rows($result) == 0) {
				...
				$badguy = array();
				$badguy['creaturename']=
					"An evil doppleganger of ".$user->name;
				$badguy['creatureweapon']=$user->weapon;
				$badguy['creaturelevel']=$user->level;
				$badguy['creaturegold']=0;
				$badguy['creatureexp'] =
				round($user->experience/10, 0);
				$badguy['creaturehealth']=$user->maxhitpoints;
				$badguy['creatureattack']=$user->attack;
				$badguy['creaturedefense']=$user->defense;
				$stack[] = $badguy;
			} else {
				...
				if ($packofmonsters == true) {
					...
					for($i=0;$i<$multi;$i++) {
						...
						$initialbadguy['playerstarthp']=$user->hitpoints;
					...
					while ($badguy = DB::fetch_assoc($result)) {
						$badguy['playerstarthp']=$user->hitpoints;
						$badguy['diddamage']=0;
