...
if (!isset($valid_loc[$user->location])) {
	$user->location=$vname;
}

$newestname = "";
$newestplayer = getsetting("newestplayer", "");
if ($newestplayer == $user->acctid) {
...
$texts = modulehook("villagetext-{$user->location}",$texts);
...
if (getsetting("automaster",1) && $user->seenmaster!=1){
	//masters hunt down truant students
	$level = $user->level+1;
	$dks = $user->dragonkills;
	$expreqd = exp_for_next_level($level, $dks);
	if ($user->experience>$expreqd &&
			$user->level<Settings::getsetting('maxlevel', 15)){
...
if (!$skipvillagedesc) {
	modulehook("collapse{", array("name"=>"villagedesc-".$user->location));
	tlschema($schemas['text']);
	output($texts['text']);
	tlschema();
	modulehook("}collapse");
	modulehook("collapse{", array("name"=>"villageclock-".$user->location));
	...
	modulehook("village-desc-{$user->location}",$texts);
	if ($texts['newestplayer'] > "" && $texts['newest']) {
		modulehook("collapse{", array("name"=>"villagenewest-".$user->location));
		tlschema($schemas['newest']);
		output($texts['newest'], $texts['newestplayer']);
		tlschema();
		$id = $texts['newestid'];
		if ($user->superuser & SU_EDIT_USERS && $id) {
...
modulehook("village-{$user->location}",$texts);
...
//		viewcommentary($texts['section'],"Speak",25,$texts['sayline'], $schemas['sayline']);
}
