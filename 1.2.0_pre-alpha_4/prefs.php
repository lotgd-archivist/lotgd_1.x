...
if ($op=="suicide" && getsetting("selfdelete",0)!=0) {
	...
	addnews("`#%s quietly passed from this world.",$user->name);
	...
	if (count($post)){
		...
		if ($pass1!=$pass2){
			...
			if ($pass1!=""){
				if (strlen($pass1)>3){
					$user = PrefsUser::getUser(Session::get('user')->acctid);
					$user->password=$pass1;
	...
	if (!isset($user->prefs['timeformat'])) $user->prefs['timeformat'] = "[m/d h:ia]";
	...
	$form=array(
		...
		"topcomment"=>"Show newest comments at the top,bool",
		...
		"timeformat"=>array("Timestamp format (currently displaying time as %s whereas default format is \"[m/d h:ia]\"),string,20",
			date($user->prefs['timeformat'],
				strtotime("now") + ($user->prefs['timeoffset'] * 60 * 60))),
		...
		'attractedTo' => 'Gender you prefer?,enum,' . SEX_MALE . ',Male,' . SEX_FEMALE . ',Female',
	);
	...
	if ($foundmodules!=array()) {
		$sql = "SELECT * FROM " . DB::prefix("module_userprefs") . " WHERE modulename IN ('".implode("','",$foundmodules)."') AND (setting LIKE 'user_%' OR setting LIKE 'check_%') AND userid='".$user->acctid."'";
	...
	// monster killed them.
	if ($user->alive && getsetting("selfdelete",0)!=0) {
		rawoutput("<form action='prefs.php?op=suicide&userid={$user->acctid}' method='POST'>");
		...
		rawoutput("</form>");
		addnav("","prefs.php?op=suicide&userid={$user->acctid}");
	}
}
page_footer();
?>
