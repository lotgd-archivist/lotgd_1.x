...
if ($user->loggedin){
	...
	output_notl("%sreferral.php?r=%s`n`n",$url,rawurlencode($user->login));
	...
	$sql = "SELECT name,level,refererawarded FROM " . DB::prefix("accounts") . " WHERE referer={$user->acctid} ORDER BY dragonkills,level";
	$result = DB::query($sql);
