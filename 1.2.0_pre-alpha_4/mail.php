...
$id = (int)httpget('id');
if($op=="del"){
	$sql = "DELETE FROM " . DB::prefix("mail") . " WHERE msgto='".$user->acctid."' AND messageid='$id'";
	DB::query($sql);
	invalidatedatacache("mail-{$user->acctid}");
	header("Location: mail.php");
	exit();
}elseif($op=="process"){
	$msg = httppost('msg');
	if (!is_array($msg) || count($msg)<1){
		Session::set('message', "`\$`bYou cannot delete zero messages!  What does this mean?  You pressed \"Delete Checked\" but there are no messages checked!  What sort of world is this that people press buttons that have no meaning?!?`b`0");
		header("Location: mail.php");
		exit();
	}else{
		$sql = "DELETE FROM " . DB::prefix("mail") . " WHERE msgto='".$user->acctid."' AND messageid IN ('".join("','",$msg)."')";
		DB::query($sql);
		invalidatedatacache("mail-{$user->acctid}");
		header("Location: mail.php");
		exit();
	...
	$sql = "UPDATE " . DB::prefix("mail") . " SET seen=0 WHERE msgto='".$user->acctid."' AND messageid='$id'";
	DB::query($sql);
	invalidatedatacache("mail-{$user->acctid}");
	header("Location: mail.php");
	exit();
