...
if ($op==""){
  ...
  output("`@Elessa`6 blinks for a moment and then smiles, \"`@Hmm, `&%s`@, let's see.....`6\" she mutters as she scans down a page in her ledger.",$user->name);
	if ($user->goldinbank>=0){
		output("`6\"`@Aah, yes, here we are.  You have `^%s gold`@ in our prestigious bank.  Is there anything else I can do for you?`6\"",$user->goldinbank);
	}else{
		output("`6\"`@Aah, yes, here we are.  You have a `&debt`@ of `^%s gold`@ in our prestigious bank.  Is there anything else I can do for you?`6\"",abs($user->goldinbank));
	}
}elseif($op=="transfer"){
	...
	if ($user->goldinbank>=0){
		...
		$maxout = $user->level*getsetting("maxtransferout",25);
		output("Similarly, you may transfer no more than `^%s`@ gold total during the day.`6\"`n",$maxout);
		if ($user->amountouttoday > 0) {
			output("`6She scans her ledgers briefly, \"`@For your knowledge, you have already transferred `^%s`@ gold today.`6\"`n",$user->amountouttoday);
		}
		output_notl("`n");
	...
	$sql = "SELECT name,acctid FROM " . DB::prefix("accounts") . " WHERE name LIKE '".addslashes($string)."' AND locked=0 ORDER by login='$to' DESC, name='$to' DESC, login";
	$result = DB::query($sql);
	...
	if (DB::num_rows($result)==1){
		...
		rawoutput("<input type='hidden' name='to' value='".$row['acctid']."'><input type='hidden' name='amount' value='$amt'><input type='submit' class='button' value='$msg'></form>",true);
	...
	if ($user->gold+$user->goldinbank<$amt){
		output("`@Elessa`6 stands up to her full, but still diminutive height and glares at you, \"`@How can you transfer `^%s`@ gold when you only possess `^%s`@?`6\"",$amt,$user->gold+$user->goldinbank);
	}else{
		$sql = "SELECT name,acctid,level,transferredtoday FROM " . DB::prefix("accounts") . " WHERE acctid=$to";
		$result = DB::query($sql);
		if (DB::num_rows($result)==1){
			$row = DB::fetch_assoc($result);
			$maxout = $user->level*getsetting("maxtransferout",25);
			$maxtfer = $row['level']*getsetting("transferperlevel",25);
			if ($user->amountouttoday+$amt > $maxout) {
			...
			}else if($row['acctid']==$user->acctid){
				...
				if ($user->gold<0){
					...
					$user->goldinbank+=$user->gold;
					$user->gold=0;
				}
				$user->amountouttoday+= $amt;
				...
				$body = array("`&%s`6 has transferred `^%s`6 gold to your bank account!",$user->name,$amt);
	...
	output_notl($user->goldinbank>=0?$balance:$debt,abs($user->goldinbank));
	output("`6Searching through all your pockets and pouches, you calculate that you currently have `^%s`6 gold on hand.`n`n", $user->gold);
	$dep = translate_inline("`^Deposit how much?");
	$pay = translate_inline("`^Pay off how much?");
	output_notl($user->goldinbank>=0?$dep:$pay);
  ...
  addnav("","bank.php?op=depositfinish");
	...
	if ($amount>$user->gold){
		output_notl($notenough,$user->gold,$amount);
	}else{
		debuglog("deposited " . $amount . " gold in the bank");
		$user->goldinbank+=$amount;
		$user->gold-=$amount;
		output_notl($user->goldinbank>=0?$depositbalance:$depositdebt,$amount,$user->name, abs($user->goldinbank),$user->gold);
	}
}elseif($op=="borrow"){
	$maxborrow = $user->level*getsetting("borrowperlevel",20);
	...
	output_notl($user->goldinbank>=0?$balance:$debt,abs($user->goldinbank));
	output("`6\"`@How much would you like to borrow `&%s`@?  At your level, you may borrow up to a total of `^%s`@ from the bank.`6\"`n`n",$user->name, $maxborrow);
	...
	rawoutput("<form action='bank.php?op=withdrawfinish' method='POST'>");
	output_notl($user->goldinbank>=0?$balance:$debt,abs($user->goldinbank));
	output("`6\"`@How much would you like to withdraw `&%s`@?`6\"`n`n",$user->name);
	...
	$amount=abs((int)httppost('amount'));
	if ($amount==0){
		$amount=abs($user->goldinbank);
	}
	if ($amount>$user->goldinbank && httppost('borrow')=="") {
		output("`\$ERROR: Not enough gold in the bank to withdraw.`^`n`n");
		output("`6Having been informed that you have `^%s`6 gold in your account, you declare that you would like to withdraw all `^%s`6 of it.`n`n", $user->goldinbank, $amount);
	...
	}else if($amount>$user->goldinbank){
		$lefttoborrow = $amount;
		$didwithdraw = 0;
		$maxborrow = $user->level*getsetting("borrowperlevel",20);
		if ($lefttoborrow<=$user->goldinbank+$maxborrow){
			if ($user->goldinbank>0){
				output("`6You withdraw your remaining `^%s`6 gold.", $user->goldinbank);
				$lefttoborrow-=$user->goldinbank;
				$user->gold+=$user->goldinbank;
				$user->goldinbank=0;
			...
			if ($lefttoborrow-$user->goldinbank > $maxborrow){
				...
				$user->goldinbank-=$lefttoborrow;
				$user->gold+=$lefttoborrow;
				debuglog("borrows $lefttoborrow gold from the bank");
				output("`@Elessa`6 records your withdrawal of `^%s `6gold in her ledger. \"`@Thank you, `&%s`@.  You now have a debt of `\$%s`@ gold to the bank and `^%s`@ gold in hand.`6\"", $amount,$user->name, abs($user->goldinbank),$user->gold);
			}
		}else{
			output("`6Considering the `^%s`6 gold in your account, you ask to borrow `^%s`6. `@Elessa`6 peers through her ledger, runs a few calculations and then informs you that, at your level, you may only borrow up to a total of `^%s`6 gold.", $user->goldinbank, $lefttoborrow-$user->goldinbank, $maxborrow);
		}
	}else{
		$user->goldinbank-=$amount;
		$user->gold+=$amount;
		debuglog("withdrew $amount gold from the bank");
		output("`@Elessa`6 records your withdrawal of `^%s `6gold in her ledger. \"`@Thank you, `&%s`@.  You now have a balance of `^%s`@ gold in the bank and `^%s`@ gold in hand.`6\"", $amount,$user->name, abs($user->goldinbank),$user->gold);
	}
}
villagenav();
addnav("Money");
...
if (getsetting("allowgoldtransfer",1)){
	if ($user->level>=getsetting("mintransferlev",3) || $user->dragonkills>0){
