...
if ((int)httpget("stage")>0)
	...
	$stage = 0;
if (!Session::exists('stagecompleted')) {
	Session::set('stagecompleted', -1);
}
if ($stage > Session::get('stagecompleted')+1) $stage = Session::get('stagecompleted');
if (!Session::exists('dbinfo')) {
	Session::set('dbinfo', array("DB_HOST"=>"","DB_USER"=>"","DB_PASS"=>"","DB_NAME"=>""));
}
if (file_exists("dbconnect.php") && (
	$stage==3 ||
	$stage==4 ||
...
if ($stage > Session::get('stagecompleted')) {
...
if (!$noinstallnavs){
	if ($user->loggedin) addnav("Back to the game",$user->restorepage);
	addnav("Install Stages");

	for ($x=0; $x<=min(count($stages)-1,Session::get('stagecompleted')+1); $x++){
