...
if ($op=="" && $act!="attack"){
	...
	output($args['atkmsg'], $user->playerfights);
	tlschema();
...
if ($battle){
	...
	if ($victory){
		...
		if (!$handled) {
			if ($killedin==$iname){
				addnews("`4%s`3 defeated `4%s`3 by sneaking into their room in the inn!",$user->name,$badguy['creaturename']);
			}else{
				addnews("`4%s`3 defeated `4%s`3 in fair combat in the fields of %s.", $user->name,$badguy['creaturename'], $killedin);
			}
		}
		...
		if (!$handled) {
			if ($killedin == $iname) {
				addnews("`%%s`5 has been slain while breaking into the inn room of `^%s`5 in order to attack them.`n%s`0", $user->name, $badguy['creaturename'], $taunt);
			}else {
				addnews("`%%s`5 has been slain while attacking `^%s`5 in the fields of `&%s`5.`n%s`0", $user->name, $badguy['creaturename'], $killedin, $taunt);
			}
		}
	}else{
		$extra = "";
