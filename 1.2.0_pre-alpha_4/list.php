...
if ($page=="" && $op==""){
	...
	$sql = "SELECT acctid,name,login,alive,location,race,sex,level,laston,loggedin,lastip,uniqueid FROM " . DB::prefix("accounts") . " WHERE locked=0 AND loggedin=1 AND laston>'".date("Y-m-d H:i:s",strtotime("-".getsetting("LOGINTIMEOUT",900)." seconds"))."' AND clanid='{$user->clanid}' ORDER BY level DESC, dragonkills DESC, login ASC";
	$result = DB::query($sql);
}else{
