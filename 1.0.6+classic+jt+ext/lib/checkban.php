...
function checkban($login=false){
	...
	$sql = "SELECT * FROM " . db_prefix("bans") . " where ((substring('$ip',1,length(ipfilter))=ipfilter AND ipfilter<>'') OR (uniqueid='$id' AND uniqueid<>'')) AND (banexpire is null OR banexpire>='".date("Y-m-d")."')";
	$result = db_query($sql);
	if (db_num_rows($result)>0){
		...
		while ($row = db_fetch_assoc($result)) {
			...
			if (!isset($row['banexpire']) || $row['banexpire']=='')
