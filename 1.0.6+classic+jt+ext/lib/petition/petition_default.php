...
//		$body = output_array($post);
//		if (preg_match('/viagra/i', $body) ||
//				preg_match('/cialis/i', $body) ||
//				preg_match('/levitra/i', $body) ||
//				preg_match('/http:\/\/.*http:\/\//i', $body) ||
//			    preg_match('/http:\/\/[^\/]+\.(info|biz)/i', $body)) {
//			output("Get lost you stupid spambot!");
//		} else {
			...
			$post['cancelreason'] = "The admins have prevented you from submitting a petition for some unknown reason.";
			$post['ipaddress'] = $ip;
			$post = modulehook("addpetition", $post);
			unset($post['ipaddress']);
			if (!$post['cancelpetition']) {
				unset($post['cancelpetition']);
				unset($post['cancelreason']);
				$post = modulehook("cleanuppetition", $post);
...
//		}
	...
	if ($abuse == "yes") {
		...
		rawoutput("<input type='hidden' name='abuse' value=\"".stripslashes_deep(htmlentities(httpget("problem"), ENT_COMPAT, getsetting("charset", "ISO-8859-1")))."\"><br><hr><pre>".stripslashes(htmlentities(httpget("problem"), ENT_COMPAT, getsetting("charset", "ISO-8859-1")))."</pre><hr><br>");
	} else {
