...
if ($replyto){
	...
	$sql = "SELECT ".$mail.".body,".$mail.".msgfrom,".$mail.".subject,".$accounts.".login, superuser, ".$accounts.".name FROM ".$mail." LEFT JOIN ".$accounts." ON ".$accounts.".acctid=".$mail.".msgfrom WHERE msgto=\"".$session['user']['acctid']."\" AND messageid=\"".$replyto."\"";
	$result = db_query($sql);
...
if (is_array($row)){
	...
	if (isset($row['body']) && $row['body']){
		$body="\n\n---".sprintf_translate(array("Original Message from %s",sanitize($row['name'])))."---\n".$row['body'];
	}
}
