...
if (isset($userid) && $userid) {
	...
	debug("I am here with: (user: $userid) $row");
}
...
output("IP: ");
$lastip = (isset($row['lastip']) && $row['lastip']) ? $row['lastip'] : "";
rawoutput("<input name='ip' id='ip' value=\"".HTMLEntities($lastip, ENT_COMPAT, getsetting("charset", "ISO-8859-1"))."\">");
output_notl("`n");
rawoutput("<input type='radio' value='id' name='type'>");
output("ID: ");
$uniq = (isset($row['uniqueid']) && $row['uniqueid']) ? $row['uniqueid'] : "";
rawoutput("<input name='id' value=\"".HTMLEntities($uniq, ENT_COMPAT, getsetting("charset", "ISO-8859-1"))."\">");
output("`nDuration: ");
