...
if ($display == 1){
	...
	for ($i=0;$i<$number3;$i++){
		...
		rawoutput("<td nowrap>");
		rawoutput("[ <a href='user.php?op=edit&userid={$row['acctid']}$m'>$ed</a> | <a href='user.php?op=del&userid={$row['acctid']}' onClick=\"return confirm('$conf');\">$del</a> | <a href='user.php?op=setupban&userid={$row['acctid']}'>$ban</a> | <a href='user.php?op=debuglog&userid={$row['acctid']}'>$log</a> | <a href='bio.php?char={$row['acctid']}&ret=".urlencode($_SERVER['REQUEST_URI'].$q)."'>$bio</a> ]");
		...
		addnav("","bio.php?char={$row['acctid']}&ret=".urlencode($_SERVER['REQUEST_URI'].$q));
		rawoutput("</td><td>");
