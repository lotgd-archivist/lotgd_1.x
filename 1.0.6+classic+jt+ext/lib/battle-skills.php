...
function rolldamage(){
	global $badguy,$session,$creatureattack,$creatureatkmod;
	...
	if ($badguy['creaturehealth']>0 && $session['user']['hitpoints']>0){
		...
		*/
		...
		while(!isset($creaturedmg) || !isset($selfdmg) || $creaturedmg==0 && $selfdmg==0){
			...
			if (e_rand(1,20)==1 && $badguy['type'] != "pvp") $atk*=3;
			/*
...
function is_buff_active($name) {
	global $session;
	// If it's not already suspended.
	if (!isset($session['bufflist'][$name])) return 0;
	if (isset($session['bufflist'][$name]['suspended']) &&
			$session['bufflist'][$name]['suspended']) return 0;
	return 1;
}
