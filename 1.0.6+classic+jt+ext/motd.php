...
if ($op=="") {
	...
	*/
	$m = httpget("month");
	if ($m !="" && !validateDate($m)) $m = "";
	if ($m > ""){
		list($y, $m) = explode("-", $m);
		$eom = cal_days_in_month(CAL_GREGORIAN, $m, $y);

		$sql = "SELECT " . db_prefix("motd") . ".*,name AS motdauthorname FROM " . db_prefix("motd") . " LEFT JOIN " . db_prefix("accounts") . " ON " . db_prefix("accounts") . ".acctid = " . db_prefix("motd") . ".motdauthor WHERE motddate >= '{$y}-{$m}-01' AND motddate <= '{$y}-{$m}-${eom}' ORDER BY motddate DESC";
