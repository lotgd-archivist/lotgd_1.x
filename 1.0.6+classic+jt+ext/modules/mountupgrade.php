...
function mountupgrade_dohook($hookname, $args){
	...
	case "battle-victory":
		if ($upgradeto){
			if ($reqffs &&
					($args['type']=="forest" || $args['type']=="travel")) {
				set_module_pref("metffs",$metffs+1);
			}
			if ($reqlevels && $args['type']=="train") {
	...
	case "gainprizemount":
		...
		if ($prizemount!=$oldmount){
			while($prizemount){
				$prizemount=get_module_objpref("mounts",
						$prizemount,"upgradeto");
				if ($prizemount==$oldmount){
		...
		$upgradeto = get_module_objpref("mounts", $prizemount,"upgradeto");
...
function mountupgrade_showform($keyout, $val, $info){
	...
	while ($row = db_fetch_assoc($result)){
		...
		if ($val==$mountid){
			rawoutput("<option value='$mountid' selected>".htmlentities("$mountcategory: $mountname [$mountid]", ENT_COMPAT, getsetting("charset", "ISO-8859-1"))."</option>");
		}else{
			rawoutput("<option value='$mountid'>".htmlentities("$mountcategory: $mountname [$mountid]", ENT_COMPAT, getsetting("charset", "ISO-8859-1"))."</option>");
		}
	}
	rawoutput("</select>");
}
