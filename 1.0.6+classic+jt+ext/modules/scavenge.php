	...
	case "graveyard":
		if ($op=="" || $op=="search") {
			...
			case 15:
				...
				output("You could free it, but %s`7 would not be pleased if he heard that you had helped a soul escape his realm.", $death);
				addnav("Lost Soul");
			...
			addnews("`#%s`^ saved an innocent soul in the Graveyard from the clutches of %s`^.",$session['user']['name'], $death);
		} elseif ($op=="running") {
			$session['user']['specialinc'] = "";
