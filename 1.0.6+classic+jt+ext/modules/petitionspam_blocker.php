...
function petitionspam_blocker_install(){
	...
	module_addhook("cleanuppetition");
...
function petitionspam_blocker_dohook($hookname,$args){
	...
	case "petitionform":
		...
		rawoutput("<p style='display: none;'><b>Please note, do not change these fields</b>, they are an anti-spam measure.  " .
				...
				"</p>");
		break;
	case "cleanuppetition":
		unset($args[get_module_setting("hidden_reqname")]);
		break;
	case "addpetition":
		...
		if (!array_key_exists($reqname,$args) ||
			array_key_exists($noname,$args) ||
			$args[$reqname] != $reqvalue ||
			preg_match("/^202\.75\.49/", $args['ipaddress']) ||
			preg_match("/^195\.5\.116/", $args['ipaddress'])
		){
		...
		break;
	case "newday-runonce":
		# Once per newday change the special fields!  This prevents bots
		# bots from hard coding them.
		set_module_setting("hidden_reqname", md5(getmicrotime(true).e_rand()));
		set_module_setting("hidden_reqvalue", md5(getmicrotime(true).e_rand()));
		set_module_setting("hidden_noname", md5(getmicrotime(true).e_rand()));
		set_module_setting("hidden_novalue", md5(getmicrotime(true).e_rand()));
		break;
	}
	return $args;
}
