	...
	if ($act=="editor"){
		...
		if ($drunk > get_module_setting("maxdrunk")) {
			...
			if ($session['user']['gold'] >= $drinkcost) {
				$drunk = get_module_pref("drunkeness");
				$drunkadd = modulehook("modify-drunkeness",$row);
				$drunk += $drunkadd['drunkeness'];
		...
		if ($drinktext['return']>""){
			...
			addnav(array("Go back to talking to %s`0", getsetting('barkeep', '`)Cedrik')),"inn.php?op=bartender");
		}
