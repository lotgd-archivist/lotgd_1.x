...
function lottery_dohook($hookname,$args){
	...
	case "inn":
		addnav("Things to do");
		addnav(array("%s`0's Lottery", getsetting('barkeep', '`)Cedrik')),"runmodule.php?module=lottery&op=store");
		break;
	}
	return $args;
}
...
function lottery_run(){
	...
	if ($op=="store"){
		...
		page_header("%s's Lottery", color_sanitize(getsetting('barkeep', '`)Cedrik')));
