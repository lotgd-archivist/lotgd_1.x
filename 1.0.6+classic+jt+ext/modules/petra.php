...
function petra_run() {
	...
	if ($op == "" && $lasttat==0) {
		...
		$tatname = httpget("des");
		$args = modulehook("petrareadablename", array('tatname'=>$tatname));
		$printname = $tatname;
		if ($args['readname']) {
			$printname = $args['readname'];
		}
		...
		if ($feather==1 && $session['user']['gems']>=$costfeather) {
			output("Oh! That feather in your hair, it's lovely! I'll do the %s for 1 gem less, if you'll give me that feather!\"`n`n", translate_inline($printname));
			...
			output("Oh! Never mind! Now, let's get this %s organized. The price is %s gems.\"`n`n", translate_inline($printname), $cost);
			addnav(array("Pay %s gems", $cost),"runmodule.php?module=petra&op=full&des=$tatname");
			addnav("Don't get a tattoo today","runmodule.php?module=petra&op=nope");
		} else {
			output("Oh! Never mind! Now, let's get this %s organized. The price is %s gems.`n`n", translate_inline($printname), $cost);
		...
		output("Finally, she announces that the %s is finished.", translate_inline($printname));
