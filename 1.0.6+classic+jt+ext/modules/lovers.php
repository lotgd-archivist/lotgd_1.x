...
function lovers_dohook($hookname, $args){
	...
	case "newday":
		...
		if ($session['user']['marriedto'] == 4294967295){
			...
			// 1.0 was too much.
			$dk = min(max(1, round(.85 * sqrt($dk), 0)), 9);
			$charmloss= e_rand(1,$dk);
			$session['user']['charm'] -= $charmloss;
			output("`n`%You're  married,  so there's no reason to keep up that perfect image, and you let yourself go a little today.`0`n");
