...
function giftshoppe_run(){
	...
	if ($op =="editor"){
		...
		villagenav();
		addnav("Return to Gifte Shoppe", "runmodule.php?module=giftshoppe");
		output("`&The Sales-elf wanders over to the large leather-bound ledger book and flips through the pages.`n`n");
		$recip = translate_inline("Recipient");
		$gname = translate_inline("Gift");
		$gnote = translate_inline("Gift Note");
		$anon = translate_inline("Anonymously?");
		$sql = "SELECT count(*) as c FROM " . db_prefix("gifts") . " WHERE giftfrom = " . $session['user']['acctid'];
		$res = db_query($sql);
		$row = db_fetch_assoc($res);
		$gifttotal = $row['c'];
		if ($gifttotal == 0) {
			output("\"`\$Sorry,`&\" he says, \"`\$but you haven't bought anything yet!`&\"`n`n");
		} else {
			output("\"`\$Aha!`&\" he exclaims, \"`\$here is the list of the gifts you've bought so far!`&\"`n`n");
			rawoutput("<table border=0 cellpadding=2 cellspacing=1 bgcolor='#999999'>");
			rawoutput("<tr class='trhead'>");
			rawoutput("<td>$recip</td><td>$gname</td><td>$gnote</td><td>$anon</td>");
			rawoutput("</tr>");
			$sql = "SELECT g.giftto as recipid, g.name as gift, g.note as note, g.anonymous as anon, u.name as recipname from " . db_prefix("gifts") . " g INNER JOIN " . db_prefix("accounts") . " u ON g.giftto = u.acctid WHERE giftfrom = " . $session['user']['acctid'];
			$res = db_query($sql);
			$i = 0;
			while ($row = db_fetch_assoc($res)) {
				rawoutput("<tr class='".($i%2?"trdark":"trlight")."'><td>");
				output_notl("%s", $row['recipname']);
				rawoutput("</td><td>");
				output_notl("%s", $row['gift']);
				rawoutput("</td><td>");
				output_notl("%s", $row['note']);
				rawoutput("</td><td>");
				output_notl("%s", translate_inline($row['anon']?"Yes":"No"));
				rawoutput("</tr>");
				$i++;
			}
			rawoutput("</table>");
		...
		addnav("Ask the Elf");
		addnav("To Check Your Shopping List", "runmodule.php?module=giftshoppe&op=checklist");
		addnav("Gifts");
