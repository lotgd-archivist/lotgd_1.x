...
function racehuman_getmoduleinfo(){
	$info = array(
		...
		"settings"=>array(
			...
			"bonus"=>"How many extra forest fights for humans?,range,1,5,1|1",
		),
	);
	return $info;
}
...
function racehuman_dohook($hookname,$args){
	...
	case "setrace":
		if ($session['user']['race']==$race) {
			...
			output("`&As a human, your size and strength permit you the ability to effortlessly wield weapons, tiring much less quickly than other races.`n");
			output("`^You gain %s extra forest %s each day!", $bonus, ($bonus==1)?$fight:$fights);
			if (is_module_active("cities")) {
	...
	case "newday":
		if ($session['user']['race']==$race){
			...
			$fight = translate_inline("fight");
			$fights= translate_inline("fights");
			output("`n`&Because you are human, you gain `^%s extra`& forest %s for today!`n`0", $bonus, ($bonus==1)?$fight:$fights);
		}
		break;
	case "validforestloc":
	case "validlocation":
