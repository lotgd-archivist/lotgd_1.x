	...
	case "newday":
		...
		if ($guilt > 1) {
			if (get_module_pref("guiltydk")) {
				...
				output("You are quite sure that, whatever it was,  you don't want to face %s`3 quite yet.", getsetting("barkeep", "`)Cedrik"));
			} else {
				output("`n`3You start to feel bad about breaking into the inn recently, and you aren't sure you can face %s`3 quite yet.", getsetting("barkeep", "`)Cedrik"));
			}
			if ($guilt == 2) {
...
function breakin_run(){
	...
	if ($op==""){
		...
		if ($bchance>$danger) {
			...
			output("%s`3 stands over you, a wine bottle in one hand, scowling angrily.`n`n", getsetting("barkeep", "`)Cedrik"));
			...
			addnews("`&%s `7tried to break into the Inn, and was caught by %s`7.", $session['user']['name'], getsetting("barkeep", "`)Cedrik"));
