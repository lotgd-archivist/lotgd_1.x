...
/* gifts can be extended by other modules */
...
function abigail_runevent($type, $link) {
	...
	if ($op == "") {
		...
		$ret = modulehook("abigail-gifts", array("gifts"=>$gifts));
		$gifts = $ret['gifts'];
		$gifts[SEX_FEMALE] = translate_inline($gifts[SEX_FEMALE]);
		$gifts[SEX_MALE] = translate_inline($gifts[SEX_MALE]);
		...
		blocknav("runmodule.php?module=kissingbooth");
	}
	elseif ($op == "leave") {
