...
function gardenparty_getmoduleinfo(){
	$info = array(
		...
		"settings"=>array(
			...
			"cedrikclothes"=>array("What is %s`0 wearing?|a Hawaiian shirt", getsetting('barkeep', '`)Cedrik')),
...
function gardenparty_dohook($hookname, $args) {
	...
	case "gardens":
		...
		if ($start <= $now && $end >= $now) {
			...
			output("%s`0 is here, wearing (of all things) %s, serving food and drinks.`n`n", getsetting('barkeep', '`)Cedrik'), get_module_setting("cedrikclothes"));
			addnav("Party Treats!");
...
function gardenparty_run(){
	...
	if ($cantafford){
		page_header("%s`0 in %s", getsetting('barkeep', '`)Cedrik'), get_module_setting("cedrikclothes"));
		output("You wander over to where %s`0 is standing in the gardens, and ask to buy %s, but he tells you that you don't have enough gold to buy it.", getsetting('barkeep', '`)Cedrik'), $missed);
