...
function claneditor_run(){
	...
	addcommentary();
	$ranks = array(CLAN_APPLICANT=>"`!Applicant`0",CLAN_MEMBER=>"`#Member`0",CLAN_OFFICER=>"`^Officer`0",CLAN_LEADER=>"`&Leader`0",CLAN_FOUNDER=>"`\$Founder`0");
	...
	if ($op==""||$op=="list"){
		...
		if ($op=="mview"){
			...
			if (httpget("sop")=="yes") {
				...
				$sql = "UPDATE ".db_prefix("accounts")." SET clanid=0,clanrank=".CLAN_APPLICANT.",clanjoindate=null WHERE clanid=$dt";
				db_query($sql);
			...
			if ($remove>""){
				$sql = "UPDATE " . db_prefix("accounts") . " SET clanrank=".CLAN_APPLICANT.",clanid=0,clanjoindate=null WHERE login='$remove' AND clanrank<={$session['user']['clanrank']}";
				db_query($sql);
