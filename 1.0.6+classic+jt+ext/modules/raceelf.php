		...
		if ($session['user']['location'] == $city){
			...
			if ($new != 0) {
				...
				if ($row && isset($row['name'])) {
					...
					set_module_setting("newest-$city", 0, "cities");
					$args['newestplayer'] = 0;
					$args['newestid']="";
				}
			} else {
	...
	case "weaponstext":
		...
		if ($session['user']['location'] == $city) {
			...
			if ($session['user']['race'] == $race) {
				...
				$args['tradein'] = array(
					array("`5Gadriel`7 looks at you and says, \"`5I'll give you `^%s`5 trade-in value for your `7%s`5.", $tradeinvalue, $session['user']['weapon']),
			...
			$args['payweapon'] = "With a flourish, he takes your `5%s`7 and spins it expertly in his hands.  He then turns around and picks up your new `5%s`7, deftly demonstrating its use, before handing it to you with gallantry and grace.";
		}
		break;
	}
	return $args;
